...
let sort_migrations (migrations : migration list) : migration list =
  List.sort
    (fun (a : migration) (b : migration) -> compare a.version b.version)
    migrations
;;
  ...
  let lines =
    List.map
      (fun s ->
         ...
         let checksum_warning =
           if not s.checksum_match then " ⚠ checksum mismatch" else ""
         in
         let applied_info =
           match s.applied_at with
...
let pp_result = function
  ...
  | Partial { applied; failed_at; error } ->
    Printf.sprintf
      "✗ Partial: %d applied, failed at version %d: %s"
      applied
      failed_at
      error
...
let record_migration (module C : Db.CONNECTION) (migration : migration) =
  let sql =
    Printf.sprintf "INSERT INTO %s (version, name, checksum) VALUES (?, ?, ?)" table_name
  in
  let open Caqti_request.Infix in
...
let rollback_one (module C : Db.CONNECTION) (migration : migration)
  : (unit, string) result
  =
  let open Caqti_request.Infix in
...
let up pool migrations ~scheme : migration_result =
  match
    Caqti_eio.Pool.use
      (fun (module C : Db.CONNECTION) ->
         match ensure_table (module C) scheme with
         | Stdlib.Error err ->
           Stdlib.Ok
             (Stdlib.Error
                (Printf.sprintf
                   "Failed to create migrations table: %s"
           ...
           (match get_applied (module C) with
            | Stdlib.Error err ->
              Stdlib.Ok
                (Stdlib.Error
                   (Printf.sprintf
                      "Failed to get applied migrations: %s"
                      (Caqti_error.show err)))
            | Stdlib.Ok applied ->
              ...
              let pending =
                List.filter
                  (fun (m : migration) -> not (List.mem m.version applied_versions))
                  sorted
              in
              let rec apply_all count = function
                ...
                | (m : migration) :: rest ->
                  (match apply_one (module C) m with
                   | Stdlib.Error e ->
                     Partial { applied = count; failed_at = m.version; error = e }
         ...
         | Stdlib.Ok () ->
           (match get_applied (module C) with
            ...
            | Stdlib.Ok applied ->
              ...
              let to_rollback =
                List.filter
                  (fun (m : migration) -> List.mem m.version applied_versions)
                  sorted
              in
              let limited =
                match steps with
...
let status pool migrations ~scheme : (migration_status list, string) Stdlib.result =
  match
    Caqti_eio.Pool.use
      (fun (module C : Db.CONNECTION) ->
         match ensure_table (module C) scheme with
         | Stdlib.Error err ->
           Stdlib.Ok (Stdlib.Error (Printf.sprintf "Failed: %s" (Caqti_error.show err)))
         | Stdlib.Ok () ->
           (match get_applied (module C) with
            | Stdlib.Error err ->
              Stdlib.Ok
                (Stdlib.Error (Printf.sprintf "Failed: %s" (Caqti_error.show err)))
            | Stdlib.Ok applied ->
              let applied_map =
                List.fold_left
                  (fun acc (v, _, checksum, applied_at) ->
                     (v, (checksum, applied_at)) :: acc)
                  []
                  applied
              in
              let sorted = sort_migrations migrations in
  ...
  | Stdlib.Error err ->
    Stdlib.Error (Printf.sprintf "Pool error: %s" (Caqti_error.show err))
;;
