...
let default_not_found _req = Response.html ~status:`Not_found "<h1>404 Not Found</h1>"
...
(** Create registry *)
let create_registry () = { handlers = Hashtbl.create 32 }
...
let find_handler route_id registry = Hashtbl.find_opt registry.handlers route_id
...
let handle_loader ~ctx_factory ~registry route_match req =
  ...
  | Some handler ->
    (match handler.loader with
     | None -> Response.json (`Assoc [ "data", `Null ])
     | Some loader ->
       ...
       (match loader loader_ctx with
        | Ok data ->
          Response.json (`Assoc [ "data", data; "routeId", `String route.Manifest.id ])
        | Error msg ->
          ...
          (match Loader.parse_redirect msg with
           ...
           | None ->
             if Loader.is_not_found msg
             then
               Response.json (`Assoc [ "error", `String "Not found" ])
               |> Response.with_status `Not_found
             else if Loader.is_unauthorized msg
             then
    ...
    (match handler.action with
     ...
     | Some action ->
       ...
       (match action action_ctx with
        | Ok data -> Response.json (`Assoc [ "success", `Bool true; "data", data ])
        | Error msg ->
          Response.json (`Assoc [ "success", `Bool false; "error", `String msg ])
...
let create_handler ~config ~ctx_factory ~registry =
  ...
  let is_loader_request =
    Request.header "X-Loader-Request" req = Some "1" || method_ = `GET
  in
  let is_action_request =
    method_ = `POST || method_ = `PUT || method_ = `PATCH || method_ = `DELETE
  in
  match Manifest.match_path path config.manifest with
  | None -> config.not_found_handler req
  | Some route_match ->
    if is_action_request && Request.header "X-Action-Request" req = Some "1"
    then handle_action ~ctx_factory ~registry route_match req
    else if is_loader_request
    then handle_loader ~ctx_factory ~registry route_match req
    else (
...
|}
           ...
           (Yojson.Safe.to_string
              (`Assoc (List.map (fun (k, v) -> k, `String v) route_match.Manifest.params)))))
;;
...
let manifest_route manifest =
  get "/_routes/manifest.json" (fun _req -> Response.json (Manifest.to_json manifest))
;;
