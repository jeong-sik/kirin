...
let generate_params_type (route : Manifest.route_entry) =
  ...
  let params =
    List.filter_map
      (fun seg ->
         if String.length seg > 0 && String.get seg 0 = ':'
         then (
           let name = String.sub seg 1 (String.length seg - 1) in
           let is_optional =
             String.length name > 0 && String.get name (String.length name - 1) = '?'
           in
           let name =
             if is_optional then String.sub name 0 (String.length name - 1) else name
           in
           Some (name, is_optional))
         else if String.length seg > 0 && String.get seg 0 = '*'
         then Some ("*", false)
         else None)
      segments
  in
  ...
  then "Record<string, never>"
    ...
    let fields =
      List.map
        (fun (name, optional) ->
           if optional
           then Printf.sprintf "  %s?: string;" name
           else Printf.sprintf "  %s: string;" name)
        params
    in
    Printf.sprintf "{\n%s\n}" (String.concat "\n" fields))
;;
...
let generate_route_definition (route : Manifest.route_entry) =
  let loader_import =
    if route.has_loader
    then Printf.sprintf "() => import('./%s/loader').then(m => m.loader)" route.id
    else "undefined"
  in
  Printf.sprintf
    {|
...
let generate_search_params_type (route_id : string) (params : (string * string) list) =
  if params = []
  then Printf.sprintf "export type %sSearchParams = Record<string, never>;" route_id
  else (
    let fields =
      List.map (fun (name, ts_type) -> Printf.sprintf "  %s?: %s;" name ts_type) params
    in
    Printf.sprintf
      "export interface %sSearchParams {\n%s\n}"
      route_id
      (String.concat "\n" fields))
;;
