...
let default_meta = { title = None; description = None; tags = [] }
...
let is_param_segment s = String.length s > 0 && String.get s 0 = ':'

(** Check if segment is a catch-all *)
let is_catchall_segment s = String.length s > 0 && String.get s 0 = '*'

(** Extract param name from segment *)
let param_name s =
  if is_param_segment s
  then String.sub s 1 (String.length s - 1)
  else if is_catchall_segment s
  then String.sub s 1 (String.length s - 1)
  else s
;;
  ...
  let pattern_segments =
    List.map
      (fun seg ->
         ...
         then ":param"
         else if is_catchall_segment seg
         then "*"
         else seg)
      segments
  in
    ...
    let fields =
      List.map
        (fun p ->
           ...
           if p.optional
           then Printf.sprintf "  %s?: %s;" p.name ts_type
           else Printf.sprintf "  %s: %s;" p.name ts_type)
        params
    in
    Printf.sprintf "{\n%s\n}" (String.concat "\n" fields))
;;
