...
let ok ?(headers = []) ?(status = 200) data = { data; headers; status }
...
(** Get search param *)
let search_param name ctx = List.assoc_opt name ctx.search_params

(** Get all search params with same name *)
let search_params_all name ctx =
  List.filter_map (fun (k, v) -> if k = name then Some v else None) ctx.search_params
;;

(** {1 Loader Composition} *)

(** Run multiple loaders in parallel (simulated) *)
let parallel loaders ctx = List.map (fun loader -> loader ctx) loaders
...
let cache_key (ctx : _ loader_context) =
  let params_str = List.map (fun (k, v) -> k ^ "=" ^ v) ctx.params |> String.concat "&" in
  let search_str =
    List.map (fun (k, v) -> k ^ "=" ^ v) ctx.search_params |> String.concat "&"
  in
...
(** Redirect response *)
let redirect ?(status = 302) url = Error (Printf.sprintf "REDIRECT:%d:%s" status url)

(** Parse redirect from error *)
let parse_redirect error =
  if String.length error > 9 && String.sub error 0 9 = "REDIRECT:"
  then (
    let rest = String.sub error 9 (String.length error - 9) in
    match String.split_on_char ':' rest with
    | [ status_str; url ] ->
      (try Some (int_of_string status_str, url) with
       | Failure _ -> None)
    | _ -> None)
  else None
;;
...
(** Is not found error *)
let is_not_found error = error = "NOT_FOUND"

(** Unauthorized response *)
let unauthorized () = Error "UNAUTHORIZED"

(** Is unauthorized error *)
let is_unauthorized error = error = "UNAUTHORIZED"

(** {1 Loader Middleware} *)

(** Loader middleware type *)
type ('ctx, 'a, 'b) middleware = ('ctx, 'a) loader -> ('ctx, 'b) loader

(** Add authentication check *)
let with_auth (check : 'ctx -> bool) (loader : ('ctx, 'a) loader) : ('ctx, 'a) loader =
  fun ctx -> if check ctx.ctx then loader ctx else unauthorized ()
;;
