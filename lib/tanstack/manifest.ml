...
let empty = { version = 1; routes = []; root_id = None }

(** Add route to manifest *)
let add_route entry manifest = { manifest with routes = entry :: manifest.routes }

(** Set root route *)
let set_root id manifest = { manifest with root_id = Some id }

(** Find route by ID *)
let find_route id manifest = List.find_opt (fun r -> r.id = id) manifest.routes
...
(** Get child routes *)
let children_of id manifest = List.filter (fun r -> r.parent_id = Some id) manifest.routes

(** Get root routes *)
let root_routes manifest = List.filter (fun r -> r.parent_id = None) manifest.routes
...
let path_matches pattern path =
  ...
  let rec match_segments params pattern_segs path_segs =
    ...
    | p :: ps, s :: ss ->
      if String.length p > 0 && String.get p 0 = ':'
      then (
        (* Dynamic segment *)
        let param_name = String.sub p 1 (String.length p - 1) in
        let is_optional =
          String.length param_name > 0
          && String.get param_name (String.length param_name - 1) = '?'
        in
        let param_name =
          if is_optional
          then String.sub param_name 0 (String.length param_name - 1)
          else param_name
        in
        match_segments ((param_name, s) :: params) ps ss)
      else if p = s
      then match_segments params ps ss
      else None
    | p :: ps, [] ->
      ...
      if
        String.length p > 1
        && String.get p 0 = ':'
        && String.get p (String.length p - 1) = '?'
      then match_segments params ps []
      else None
  in
...
let match_path path manifest =
  ...
  List.find_map
    (fun route ->
       ...
       | Some params -> Some { route; params = List.rev params; search_params }
       | None -> None)
    manifest.routes
;;
...
let of_json json =
  ...
  | `Assoc fields ->
    let routes =
      ...
      | Some (`List routes_json) -> List.filter_map route_entry_of_json routes_json
      | _ -> []
    in
    let root_id =
      match List.assoc_opt "rootId" fields with
...
let from_discovered_routes routes =
  let rec collect_entries parent_id routes =
    List.concat_map
      (fun (route : File_router.discovered_route) ->
         let entry =
           { id = route.id
           ; path = route.path
           ; parent_id
