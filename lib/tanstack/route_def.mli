...
type param_type = PString | PInt | PUuid | PSlug
val param_type_to_string : param_type -> string
type param = { name : string; param_type : param_type; optional : bool; }
...
val create :
  ...
  ?meta:meta ->
  ?loader:('a, 'b) loader ->
  ?action:('a, 'c) action -> ?children:string list -> unit -> ('a, 'b, 'c) t
val with_loader :
  id:string ->
  path:string ->
  ?params:param list -> ?meta:meta -> ('a, 'b) loader -> ('a, 'b, 'c) t
val with_action :
  id:string ->
  path:string ->
  ?params:param list -> ?meta:meta -> ('a, 'b) action -> ('a, 'c, 'b) t
val with_loader_action :
  id:string ->
  path:string ->
  ?params:param list ->
  ?meta:meta ->
  loader:('a, 'b) loader -> action:('a, 'c) action -> unit -> ('a, 'b, 'c) t
val layout :
  id:string -> path:string -> ?meta:meta -> string list -> ('a, 'b, 'c) t
