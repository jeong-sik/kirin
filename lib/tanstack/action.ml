         ...
         | Pattern pattern ->
           (match value with
            | Some (`String s) ->
              ...
              if Str.string_match re s 0 then None else Some (name, "has invalid format")
            | _ -> None)
         | Email ->
           (match value with
            | Some (`String s) ->
            ...
            | _ -> None)
         | Custom (check, msg) -> if check value then None else Some (name, msg))
      rules
  in
  errors
;;
...
let validate rules ctx =
  let all_errors =
    List.concat_map (fun (name, field_rules) -> validate_field name field_rules ctx) rules
  in
...
let for_method expected_method action ctx =
  if ctx.method_ = expected_method
  then action ctx
  else ServerError (Printf.sprintf "Method %s not allowed" (method_to_string ctx.method_))
;;
...
let to_json serialize = function
  | Success data -> `Assoc [ "success", `Bool true; "data", serialize data ]
  | Redirect (url, status) -> `Assoc [ "redirect", `String url; "status", `Int status ]
  | ValidationError errors ->
    `Assoc
      [ "success", `Bool false
      ; "errors", `Assoc (List.map (fun (k, v) -> k, `String v) errors)
      ]
  | ServerError msg -> `Assoc [ "success", `Bool false; "error", `String msg ]
;;
