...
let parse_segment name =
  ...
  if len >= 5 && String.sub name 0 4 = "[..." && String.get name (len - 1) = ']'
  then
    (* [...slug] → catch-all *)
    CatchAll (String.sub name 4 (len - 5))
  else if len >= 4 && String.sub name 0 2 = "[[" && String.sub name (len - 2) 2 = "]]"
  then
    (* [[id]] → optional *)
    Optional (String.sub name 2 (len - 4))
  else if len >= 2 && String.get name 0 = '[' && String.get name (len - 1) = ']'
  then
    (* [id] → dynamic *)
...
let route_id_of_path segments =
  let parts =
    List.filter_map
      ...
      segments
  in
  if parts = [] then "root" else String.concat "_" parts
;;
...
let rec discover_routes ~base_path ~parent_segments dir =
  ...
  let children =
    List.filter_map
      (fun subdir ->
         ...
         let child_routes =
           discover_routes ~base_path ~parent_segments:child_segments child_dir
         in
  ...
  if has_page || has_layout || children <> []
  then (
    let route =
...
let discover root_dir =
  ...
  then discover_routes ~base_path:root_dir ~parent_segments:[] root_dir
  else []
;;
