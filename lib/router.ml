...
let parse_pattern pattern =
  ...
  List.map
    (fun part ->
       if String.length part > 0 && part.[0] = ':'
       then Param (String.sub part 1 (String.length part - 1))
       else if part = "*"
       then Wildcard
       else Static part)
    parts
...
let match_segments segments path_parts =
  let rec loop segs parts params =
    ...
    | Static s :: rest_segs, p :: rest_parts ->
      if s = p then loop rest_segs rest_parts params else None
    | Param name :: rest_segs, p :: rest_parts ->
      loop rest_segs rest_parts ((name, p) :: params)
    | Wildcard :: _, _ -> Some (List.rev params)
