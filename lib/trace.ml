...
let finish span = span.end_time <- Some (Unix.gettimeofday ())
let set_attr span ~key ~value = span.attributes <- (key, value) :: span.attributes
let set_error span msg = span.status <- `Error msg

let duration span =
  let end_t =
    match span.end_time with
    | Some t -> t
...
let is_hex_string s =
  ...
  try
    String.iter
      (fun c ->
         if not ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f')) then raise_notrace Exit)
      s;
    true
  with
  | Exit -> false
...
let parse_traceparent header =
  ...
  | [ _version; tid; pid; _flags ]
    when String.length tid = 32
         && String.length pid = 16
         && is_hex_string tid
         && is_hex_string pid -> Some (tid, pid)
  | _ -> None
;;

(** Format a traceparent header value. *)
let to_traceparent span = Printf.sprintf "00-%s-%s-01" span.trace_id span.span_id
...
let current_span req = Hmap.find span_key (Request.ctx req)

(** Create a child span under the current request span. *)
let child_span req ~name =
  match current_span req with
  | Some parent -> start ~name ~trace_id:parent.trace_id ~parent_id:parent.span_id ()
  | None -> start ~name ()
;;
...
module Json_exporter : EXPORTER = struct
  let export span =
    ...
    in
    let parent =
      match span.parent_id with
      | Some p -> Printf.sprintf {|"%s"|} p
      | None -> "null"
    in
    let attrs =
      match span.attributes with
      | [] -> "{}"
      | kvs ->
        let pairs = List.map (fun (k, v) -> Printf.sprintf {|"%s":"%s"|} k v) kvs in
        "{" ^ String.concat "," pairs ^ "}"
    in
    Printf.eprintf
      {|{"trace_id":"%s","span_id":"%s","parent_id":%s,"name":"%s","duration_ms":%.3f,"status":"%s","attributes":%s}|}
      span.trace_id
      span.span_id
      parent
      span.name
...
let middleware ?(exporter : (module EXPORTER) option) () : Middleware.t =
  ...
  let parent_ctx =
    match Request.header "traceparent" req with
  ...
  let span =
    ...
    | Some (tid, pid) -> start ~name:"request" ~trace_id:tid ~parent_id:pid ()
    | None -> start ~name:"request" ()
  in
  ...
  let resp =
    ...
    | exn ->
      ...
      finish span;
      (match exporter with
       | Some (module E : EXPORTER) -> E.export span
       | None -> ());
      raise exn
  in
  (* Finish and export *)
  ...
  if status_code >= 500 then set_error span (Printf.sprintf "HTTP %d" status_code);
  finish span;
  (match exporter with
   | Some (module E : EXPORTER) -> E.export span
   | None -> ());
