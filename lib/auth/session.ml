...
let generate_id () = bytes_to_hex (Secure_random.random_string 32)
...
let create_memory_store ?(max_age = 86400.0) () =
  ...
  let with_lock f = Eio.Mutex.use_rw ~protect:true mutex f in
  let get id =
    with_lock (fun () ->
      ...
      | Some entry ->
        ...
        (match entry.expires_at with
         ...
         | _ ->
           ...
           Some entry))
  in
  let set id entry = with_lock (fun () -> Hashtbl.replace sessions id entry) in
  let delete id = with_lock (fun () -> Hashtbl.remove sessions id) in
  let cleanup () =
    with_lock (fun () ->
      let now = Kirin.Time_compat.now () in
      ...
      Hashtbl.iter
        (fun id entry ->
           ...
           | _ ->
             ...
             if now -. entry.last_accessed > max_age then to_remove := id :: !to_remove)
        sessions;
...
let create store ?expires_in () =
  ...
  let entry =
    { data = Hashtbl.create 16; created_at = now; last_accessed = now; expires_at }
  in
  store.set id entry;
  id
;;
...
let get_all store id =
  match store.get id with
  | None -> []
  | Some entry -> Hashtbl.fold (fun k v acc -> (k, v) :: acc) entry.data []
;;
...
let clear_session_cookie ?(cookie_name = default_cookie_name) ?(path = "/") resp =
  let attrs =
    Kirin.Cookie.{ default_attributes with path = Some path; max_age = Some 0 }
  in
...
(** Get session ID from request cookie *)
let get_id ?(cookie_name = default_cookie_name) req = get_id_from_request ~cookie_name req
