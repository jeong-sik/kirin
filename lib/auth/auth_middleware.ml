...
let get_auth_info req = Hmap.find auth_key (Kirin.Request.ctx req)
...
let extract_bearer_token req =
  ...
  | None -> None
  | Some auth ->
    if String.length auth > 7 && String.sub auth 0 7 = "Bearer "
    then Some (String.sub auth 7 (String.length auth - 7))
    else None
;;
...
let jwt
      ...
      ?(on_error =
        fun _req msg ->
          Kirin.Response.json ~status:`Unauthorized (`Assoc [ "error", `String msg ]))
      handler
  =
  fun req ->
...
let jwt_with_claims ~secret ~extract_user_id ?on_error handler =
  let on_error =
    Option.value on_error ~default:(fun _req msg ->
      Kirin.Response.json ~status:`Unauthorized (`Assoc [ "error", `String msg ]))
  in
  fun req ->
    match extract_bearer_token req with
      ...
      (match Jwt.decode ~secret token with
       ...
       | Ok decoded ->
         (match extract_user_id decoded.Jwt.payload with
          ...
          | Some user_id ->
            let info =
              { user_id; claims = Some decoded.Jwt.payload; token_type = "jwt" }
            in
            let req = set_auth_info req info in
            handler req))
;;
...
let session
      ~store
      ?(user_key = "user_id")
      ?(on_error = fun _req -> Kirin.Response.redirect "/login")
      handler
  =
  fun req ->
...
let api_key
      ...
      ?(on_error =
        fun _req ->
          Kirin.Response.json
            ~status:`Unauthorized
            (`Assoc [ "error", `String "Invalid API key" ]))
      handler
  =
  fun req ->
  let key =
...
let require_role
      ...
      ?(on_error =
        fun _req ->
          Kirin.Response.json
            ~status:`Forbidden
            (`Assoc [ "error", `String "Insufficient permissions" ]))
      handler
  =
  fun req ->
  ...
  | Some claims ->
    let user_roles =
      ...
      | _ -> []
    in
    if List.exists (fun r -> List.mem r user_roles) required_roles
    then handler req
    else on_error req
;;
...
let any_of
      ...
      ?(on_error =
        fun _req ->
          Kirin.Response.json
            ~status:`Unauthorized
            (`Assoc [ "error", `String "Authentication required" ]))
      handler
  =
  fun req ->
