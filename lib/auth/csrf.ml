  ...
  | Ok decoded ->
    ...
    (match String.rindex_opt decoded '.' with
     ...
     | Some idx ->
       ...
       let provided_sig =
         String.sub decoded (idx + 1) (String.length decoded - idx - 1)
       in
       (* Verify signature *)
       let expected_sig = Digestif.SHA256.hmac_string ~key:secret data in
       let expected_sig_hex =
         string_to_hex (Digestif.SHA256.to_raw_string expected_sig)
       in
       if not (constant_time_compare provided_sig expected_sig_hex)
       then Error "Invalid signature"
         ...
         | [ token_session; expires_str; _nonce ] ->
           (* Verify session *)
           if not (constant_time_compare token_session session_id)
           then Error "Session mismatch"
           else (
             (* Check expiration *)
             try
               let expires = float_of_string expires_str in
               if Unix.gettimeofday () > expires then Error "Token expired" else Ok ()
             with
             | Failure _ -> Error "Invalid expiration")
...
let middleware
  ...
  (* Skip safe methods *)
  if List.mem method_str safe_methods
  then handler req
  else (
    (* Get session ID *)
