    ...
    create
      ...
      ~authorize_url:
        (Printf.sprintf
           ...
           tenant)
      ~token_url:
        (Printf.sprintf "https://login.microsoftonline.com/%s/oauth2/v2.0/token" tenant)
...
let authorization_url
  ...
  in
  let query =
    params |> List.map (fun (k, v) -> k ^ "=" ^ Uri.pct_encode v) |> String.concat "&"
  in
...
let parse_tokens json =
  ...
  ; token_type =
      json |> member "token_type" |> to_string_option |> Option.value ~default:"Bearer"
...
let parse_user_info (provider : provider) json =
  ...
  let open Yojson.Safe.Util in
  let id, email, user_name, picture =
    ...
    | "discord" ->
      ...
      , let discord_id = json |> member "id" |> to_string in
        ...
        Option.map
          (fun a ->
             Printf.sprintf "https://cdn.discordapp.com/avatars/%s/%s.png" discord_id a)
          avatar )
    | "microsoft" ->
      ( json |> member "id" |> to_string
...
let authorization_url_pkce
  ...
  let params =
    extra_params @ [ "code_challenge", challenge; "code_challenge_method", "S256" ]
  in
...
let login_handler provider ?(scope = []) () =
  ...
  in
  Kirin.Response.redirect url |> Kirin.Cookie.set ~attrs "oauth_state" state
;;
