...
let header_to_json header =
  `Assoc [ "alg", `String (algorithm_to_string header.alg); "typ", `String header.typ ]
;;
     ...
     | Ok header_str ->
       ...
       (match algorithm_of_string alg_str with
        ...
        | Some algorithm ->
          ...
          if not (Eqaf.equal signature_b64 expected_sig_b64)
...
let get_claim token key = Yojson.Safe.Util.(token.payload |> member key)
