...
val parallel :
  ('a -> 'b loader_result) ->
  ('a -> 'c loader_result) -> 'a -> ('b * 'c) loader_result
...
val require_auth :
  check_auth:('a -> bool) ->
  login_url:string -> ('a -> 'b loader_result) -> 'a -> 'b loader_result
val to_json_result : Yojson.Safe.t loader_result -> json_result
val json_result_to_yojson :
  json_result -> [> `Assoc of (string * Yojson.Safe.t) list ]
val run_loader :
  ('a -> Yojson.Safe.t loader_result) -> 'a -> Kirin.Response.t
val endpoint :
  (loader_context -> Yojson.Safe.t loader_result) ->
  Kirin.Request.t -> Kirin.Response.t
type 'a deferred = Resolved of 'a | Pending of (unit -> 'a loader_result)
