...
val parallel :
  ...
  ('a -> 'c Loader.loader_result) -> 'a -> ('b * 'c) Loader.loader_result
val optional :
  ('a -> 'b Loader.loader_result) -> 'a -> 'b option Loader.loader_result
val action_context :
  form_data:Action.form_data -> Kirin.Request.t -> Action.action_context
...
val validate :
  ...
  Action.action_context -> Action.form_data Action.action_result
...
val route :
  ?loader:(Loader.loader_context ->
           Yojson.Safe.t Loader.loader_result)
          option ->
  ?action:(Action.action_context ->
           Yojson.Safe.t Action.action_result)
          option ->
  ?error_boundary:bool ->
  ?index:bool -> ?children:Route.t list -> string -> Route.t
val index :
  ?loader:(Loader.loader_context ->
           Yojson.Safe.t Loader.loader_result)
          option ->
...
val layout :
  ?loader:(Loader.loader_context ->
           Yojson.Safe.t Loader.loader_result)
          option ->
  ?error_boundary:bool -> Route.t list -> Route.t
val with_loader :
...
val find_matches :
  Route.t list ->
  string list -> Route.match_result list -> Route.match_result list
...
val render_context :
  ...
  ?errors:(string * string) list -> unit -> Ssr.render_context
val load_data :
  Route.match_result list ->
  Loader.loader_context -> (string * Yojson.Safe.t) list
val render_document :
  config:Ssr.ssr_config ->
  context:Ssr.render_context -> body:string -> string
val handler :
  config:Ssr.ssr_config ->
  routes:Route.t list ->
...
val render_context_to_json :
  Ssr.render_context -> [> `Assoc of (string * Yojson.Safe.t) list ]
