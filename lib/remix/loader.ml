...
let context_of_request req =
  ...
  ; request_method = meth
  ; params = []
  ; (* Would be populated from route matching *)
...
let require_auth ~check_auth ~login_url loader ctx =
  if check_auth ctx then loader ctx else Redirect (login_url, 302)
;;
...
let to_json_result = function
  | Data json -> { data = Some json; redirect = None; error = None; status = 200 }
  | Redirect (url, status) ->
    { data = None; redirect = Some (url, status); error = None; status }
  | NotFound -> { data = None; redirect = None; error = Some "Not found"; status = 404 }
  | ServerError msg -> { data = None; redirect = None; error = Some msg; status = 500 }
;;
...
let run_loader loader ctx =
  match loader ctx with
  | Data json -> Kirin.Response.json json
  | Redirect (url, status) ->
    Kirin.Response.redirect ~status:(Http.Status.of_int status) url
  | NotFound -> Kirin.Response.not_found ()
  | ServerError msg ->
    Kirin.Response.json ~status:(Http.Status.of_int 500) (`Assoc [ "error", `String msg ])
;;
