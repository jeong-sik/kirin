...
let create
      ...
      path
  =
  { path; segments = parse_path path; loader; action; error_boundary; index; children }
;;

(** Create index route *)
let index ?loader ?action () = create ~index:true ?loader ?action ""

(** Create layout route (no path, just nesting) *)
let layout ?loader ?error_boundary children = create ~children ?loader ?error_boundary ""

(** Add loader to route *)
let with_loader loader route = { route with loader = Some loader }

(** Add action to route *)
let with_action action route = { route with action = Some action }

(** Add error boundary *)
let with_error_boundary route = { route with error_boundary = true }
...
let match_segment segment part =
  ...
  | Splat -> Some [ "*", part ]
  | Optional name -> if part = "" then Some [] else Some [ name, part ]
;;
...
let match_route route path_parts =
  let rec match_segments segments parts params =
    ...
    | [ Splat ], rest -> Some (params @ [ "*", String.concat "/" rest ])
    | segment :: rest_segments, part :: rest_parts ->
      (match match_segment segment part with
       | Some new_params -> match_segments rest_segments rest_parts (params @ new_params)
       | None -> None)
    | _segment :: _rest, [] ->
...
let rec find_match routes path_parts =
  ...
  |> List.find_map (fun route ->
    ...
    | Some params -> Some { route; params; pathname = route.path }
    | None ->
      (* Try children *)
  ...
  |> List.fold_left
       (fun found route ->
          ...
          | Some params ->
            ...
            let child_parts =
              if remaining < List.length path_parts
              then List.filteri (fun i _ -> i >= remaining) path_parts
              else []
            in
