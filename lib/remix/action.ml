...
(** Get form field *)
let field ctx name = List.assoc_opt name ctx.form_data
...
let field_values ctx name =
  ctx.form_data |> List.filter (fun (k, _) -> k = name) |> List.map snd
;;
...
let validate ~validators ctx =
  let errors =
    validators
    |> List.filter_map (fun (field_name, validator, message) ->
...
(** Is email *)
let is_email s = matches_regex "^[^@]+@[^@]+\\.[^@]+$" s

(** Is numeric *)
let is_numeric s = matches_regex "^[0-9]+$" s
...
let require_csrf ~get_session_token action ctx =
  if check_csrf ~get_session_token ctx
  then action ctx
  else ValidationError [ "_csrf", "Invalid CSRF token" ]
;;
...
let result_to_json to_json = function
  | Success x -> `Assoc [ "ok", `Bool true; "data", to_json x ]
  | Redirect (url, status) ->
    `Assoc
      [ "ok", `Bool true
  ...
  | ServerError msg -> `Assoc [ "ok", `Bool false; "error", `String msg ]
;;
...
let run_action ~to_json action ctx =
  ...
  | Success data -> Kirin.Response.json (to_json data)
  | Redirect (url, status) ->
