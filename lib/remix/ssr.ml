...
let load_data matches loader_ctx =
  matches
  |> List.filter_map (fun (m : Route.match_result) ->
...
let check_redirects matches loader_ctx =
  matches
  |> List.find_map (fun (m : Route.match_result) ->
    match m.route.loader with
    | None -> None
    | Some loader ->
      let ctx = Loader.with_params m.params loader_ctx in
    ...
    (match List.assoc_opt "links" pairs with
     | Some (`List links) ->
       ...
       |> List.filter_map (function
         | `Assoc l ->
           let rel =
             match List.assoc_opt "rel" l with
             | Some (`String r) -> r
             | _ -> "stylesheet"
           in
           let href =
             match List.assoc_opt "href" l with
             | Some (`String h) -> Some h
             | _ -> None
           in
           (match href with
            | Some h -> Some (Printf.sprintf {|<link rel="%s" href="%s">|} rel h)
            | None -> None)
         | _ -> None)
       |> String.concat "\n"
     | _ -> "")
...
let render_document ~config ~context ~body =
  ...
  let hydration =
    hydration_script ~loader_data:context.loader_data ~action_data:context.action_data
  in
...
let render_error_boundary ~error ~route =
  Printf.sprintf
    {|<div class="remix-error-boundary" data-route="%s">
  <h1>Error</h1>
  <p>%s</p>
</div>|}
    route
...
let render_catch_boundary ~status ~message =
  Printf.sprintf
    {|<div class="remix-catch-boundary">
  <h1>%d</h1>
  <p>%s</p>
</div>|}
    status
...
let handler ~config ~routes ~render_component =
  ...
  let path_parts = String.split_on_char '/' url |> List.filter (fun s -> s <> "") in
