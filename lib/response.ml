...
let text ?(status = `OK) body =
  make ~status (`String body) |> with_header "content-type" "text/plain; charset=utf-8"
;;

(** HTML response *)
let html ?(status = `OK) ?(doctype = false) body =
  let body = if doctype then "<!DOCTYPE html>\n" ^ body else body in
  make ~status (`String body) |> with_header "content-type" "text/html; charset=utf-8"
;;
...
let empty status = make ~status (`String "")

(** Redirect response *)
let redirect ?(status = `Found) location =
  make ~status (`String "") |> with_header "location" location
;;

(** Permanent redirect (301) *)
let redirect_permanent location = redirect ~status:`Moved_permanently location

(** Not found response *)
let not_found ?(body = "Not Found") () = text ~status:`Not_found body

(** Bad request response *)
let bad_request ?(body = "Bad Request") () = text ~status:`Bad_request body
...
let stream ?(status = `OK) ?(headers = []) s =
  make ~status (`Stream s) |> with_headers headers
;;
