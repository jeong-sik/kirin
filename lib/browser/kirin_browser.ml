...
(** Get body element *)
let body () : element = (Dom_html.document##.body :> element)

(** {1 Element Manipulation} *)

(** Get inner HTML *)
let get_html (el : element) = Js.to_string el##.innerHTML

(** Set inner HTML *)
let set_html (el : element) html = el##.innerHTML := Js.string html

(** Get text content *)
let get_text (el : element) = Js.Opt.case el##.textContent (fun () -> "") Js.to_string

(** Set text content *)
let set_text (el : element) text = el##.textContent := Js.some (Js.string text)

(** Get attribute *)
let get_attr (el : element) name =
  Js.Opt.to_option (el##getAttribute (Js.string name)) |> Option.map Js.to_string
;;
...
(** Add CSS class *)
let add_class (el : element) cls = el##.classList##add (Js.string cls)

(** Remove CSS class *)
let remove_class (el : element) cls = el##.classList##remove (Js.string cls)

(** Toggle CSS class *)
let toggle_class (el : element) cls = Js.to_bool (el##.classList##toggle (Js.string cls))

(** Has CSS class *)
let has_class (el : element) cls = Js.to_bool (el##.classList##contains (Js.string cls))

(** {1 Element Creation} *)

(** Create element *)
let create tag : element = (Dom_html.document##createElement (Js.string tag) :> element)
...
(** Click handler *)
let on_click (el : element) handler = on el "click" handler

(** Input handler *)
let on_input (el : element) handler = on el "input" handler
...
(** Get input value *)
let get_value (el : element) = Js.to_string (Js.Unsafe.coerce el)##.value

(** Set input value *)
let set_value (el : element) value = (Js.Unsafe.coerce el)##.value := Js.string value
...
let fetch ?(meth = GET) ?(headers = []) ?body url callback =
  ...
  List.iter (fun (k, v) -> Js.Unsafe.set headers_obj (Js.string k) (Js.string v)) headers;
  let init =
    Js.Unsafe.obj
...
let response_text resp callback =
  ...
  ignore
    (Js.Unsafe.meth_call
       ...
       "then"
       [| Js.Unsafe.inject
            (Js.wrap_callback (fun text_js -> callback (Js.to_string text_js)))
       |])
;;
...
let response_json resp callback =
  ...
  ignore
    (Js.Unsafe.meth_call
       ...
       "then"
       [| Js.Unsafe.inject
            (Js.wrap_callback (fun text_js ->
               let text = Js.to_string text_js in
               try callback (Some (Yojson.Safe.from_string text)) with
               | Yojson.Json_error _ -> callback None))
       |])
;;
...
let set_timeout ms f =
  let id =
    Dom_html.window##setTimeout (Js.wrap_callback f) (Js.float (float_of_int ms))
  in
...
let set_interval ms f =
  let id =
    Dom_html.window##setInterval (Js.wrap_callback f) (Js.float (float_of_int ms))
  in
...
let get_storage () = Js.Optdef.to_option Dom_html.window##.localStorage
...
let storage_set key value =
  match get_storage () with
  | None -> ()
  | Some storage -> storage##setItem (Js.string key) (Js.string value)
;;
...
let storage_remove key =
  match get_storage () with
  | None -> ()
  | Some storage -> storage##removeItem (Js.string key)
;;
...
let get_data (el : element) key = get_attr el (data_prefix ^ key)

(** Set kirin data attribute *)
let set_data (el : element) key value = set_attr el (data_prefix ^ key) value

(** Component registry *)
let components : (string * (element -> string -> unit)) list ref = ref []

(** Register component *)
let register_component name handler = components := (name, handler) :: !components
...
module Router = struct
  ...
  let route pattern handler =
    let segments = String.split_on_char '/' pattern |> List.filter (fun s -> s <> "") in
    { pattern; segments; handler }
  ;;

  (** Create router *)
  let create ~not_found routes = { routes; not_found }

  (** Match path against pattern segments *)
  let match_path pattern_segs path_segs =
    if List.length pattern_segs <> List.length path_segs
    then None
    else (
  ...
  let find_route router pathname =
    let path_segs = String.split_on_char '/' pathname |> List.filter (fun s -> s <> "") in
