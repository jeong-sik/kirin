...
let parse_entry json =
  ...
  let is_entry =
    json |> member "isEntry" |> to_bool_option |> Option.value ~default:false
  in
  let css = json |> member "css" |> to_list_safe |> List.filter_map to_string_option in
  let assets =
    json |> member "assets" |> to_list_safe |> List.filter_map to_string_option
  in
  let dynamic_imports =
    json |> member "dynamicImports" |> to_list_safe |> List.filter_map to_string_option
  in
  let imports =
    json |> member "imports" |> to_list_safe |> List.filter_map to_string_option
  in
...
let parse ?(base_url = "/") json =
  match json with
  | `Assoc entries ->
    let parsed = List.map (fun (key, value) -> key, parse_entry value) entries in
...
let resolve manifest key = List.assoc_opt key manifest.entries
...
let all_css manifest =
  List.concat_map
    (fun (_, entry) -> List.map (fun css -> manifest.base_url ^ css) entry.css)
    manifest.entries
;;
...
let entry_points manifest =
  List.filter_map
    (fun (key, entry) -> if entry.is_entry then Some key else None)
    manifest.entries
;;
...
let css_link_tag url = Printf.sprintf {|<link rel="stylesheet" href="%s">|} url

(** Generate all CSS link tags for entry *)
let css_tags manifest key =
  css_for manifest key |> List.map css_link_tag |> String.concat "\n"
;;
