...
let serialize_data data = Yojson.Safe.to_string data |> escape_json

(** Generate initial data script *)
let data_script data =
  match data with
  | Some d -> "<script>window.__INITIAL_DATA__=" ^ serialize_data d ^ ";</script>"
  | None -> ""
;;
...
let meta_tag (name, content) =
  if String.length name >= 3 && String.sub name 0 3 = "og:"
  then "<meta property=\"" ^ name ^ "\" content=\"" ^ content ^ "\">"
  else "<meta name=\"" ^ name ^ "\" content=\"" ^ content ^ "\">"
;;

(** Generate all meta tags *)
let meta_tags metas = List.map meta_tag metas |> String.concat "\n    "
...
let render options =
  ...
  let styles_str =
    List.map (fun s -> "<link rel=\"stylesheet\" href=\"" ^ s ^ "\">") options.styles
    |> String.concat "\n    "
  in
  let scripts_str =
    List.map
      (fun s -> "<script type=\"module\" src=\"" ^ s ^ "\"></script>")
      options.scripts
    |> String.concat "\n    "
  in
...
let options_to_json o =
  `Assoc
    ...
    ; ( "meta"
      , `List
          (List.map
             (fun (n, c) -> `Assoc [ "name", `String n; "content", `String c ])
             o.meta) )
