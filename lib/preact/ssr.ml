...
let create config =
  let workers =
    Array.init config.pool_size (fun _ ->
      { state = Idle; request_count = 0; last_error = None })
  in
  { config
  ; workers
  ; total_renders = 0
...
let render_with_signals engine ~url ~signals () =
  if not engine.config.signals_ssr
  then render engine ~url ()
  else render engine ~url ~signals ()
;;

(** Dehydrate signals for client *)
let dehydrate_signals signals =
  "<script>window.__PREACT_SIGNALS__=" ^ Yojson.Safe.to_string signals ^ ";</script>"
;;
...
let stats engine =
  ...
  let hit_rate =
    if total > 0 then float_of_int engine.cache_hits /. float_of_int total else 0.0
  in
