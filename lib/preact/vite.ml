...
(** Dev server URL *)
let dev_url config = "http://" ^ config.host ^ ":" ^ string_of_int config.port
...
let html config ~entry ~title =
  if is_dev ()
  then development_html config ~entry ~title
  else production_html config ~entry ~title
;;
