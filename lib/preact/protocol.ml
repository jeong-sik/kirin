...
let render_request ~url ?(props = `Null) ?(signals = `Null) () =
  let params = `Assoc [ "url", `String url; "props", props; "signals", signals ] in
  { id = next_id (); method_ = "render"; params }
;;

(** Create health check request *)
let health_request () = { id = next_id (); method_ = "health"; params = `Null }

(** Create shutdown request *)
let shutdown_request () = { id = next_id (); method_ = "shutdown"; params = `Null }
...
let encode_error_detail err =
  let base = [ "code", `Int err.code; "message", `String err.message ] in
  let with_data =
    match err.data with
    | Some d -> base @ [ "data", d ]
    | None -> base
...
let encode_response resp =
  let json =
    match resp with
    | RpcSuccess s ->
      `Assoc [ "jsonrpc", `String "2.0"; "id", `Int s.success_id; "result", s.result ]
    | RpcError e ->
      `Assoc
        [ "jsonrpc", `String "2.0"
        ; ( "id"
          , match e.failure_id with
            | Some i -> `Int i
            | None -> `Null )
...
let decode_response str =
  try
    ...
    | `Null, `Null ->
      let fail =
        { failure_id = id
        ; error = { code = parse_error; message = "Invalid response"; data = None }
        }
      in
      Ok (RpcError fail)
    | _, `Null ->
      (match id with
       ...
       | None ->
         let fail =
           { failure_id = None
           ; error = { code = invalid_request; message = "Missing id"; data = None }
           }
         in
         Ok (RpcError fail))
    | `Null, error_json ->
    ...
    | _, _ ->
      let fail =
        { failure_id = id
        ; error = { code = parse_error; message = "Ambiguous response"; data = None }
        }
      in
      Ok (RpcError fail)
  with
  | Yojson.Json_error msg ->
    let fail =
      { failure_id = None; error = { code = parse_error; message = msg; data = None } }
    in
    Ok (RpcError fail)
  | _ ->
    let fail =
      { failure_id = None
      ; error = { code = internal_error; message = "Unknown error"; data = None }
      }
    in
    Ok (RpcError fail)
;;
...
let request_to_json req =
  `Assoc [ "id", `Int req.id; "method", `String req.method_; "params", req.params ]
;;

(** Response to JSON *)
let response_to_json = function
  | RpcSuccess s ->
    `Assoc [ "type", `String "success"; "id", `Int s.success_id; "result", s.result ]
  | RpcError e ->
    `Assoc
      [ "type", `String "error"
      ; ( "id"
