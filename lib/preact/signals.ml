...
let signal ?(computed = false) ?(readonly = false) name initial_value =
  { name; initial_value; computed; readonly }
;;
...
let make_effect ?(cleanup = false) ~name dependencies =
  { effect_name = name; effect_dependencies = dependencies; effect_cleanup = cleanup }
;;
...
let signal_to_ts s =
  ...
  in
  if s.readonly
  then Printf.sprintf "const %s: ReadonlySignal<%s>;" s.name value_type
  else Printf.sprintf "const %s: Signal<%s>;" s.name value_type
;;
...
let batch_to_json b =
  let ops =
    List.map
      (fun (name, value) -> `Assoc [ "name", `String name; "value", value ])
      b.operations
  in
