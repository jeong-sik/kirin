...
(** Default config *)
let default_config =
  { enabled = true; strict_mode = true; legacy_context = false; hooks_enabled = true }
;;
...
let vite_aliases () =
  let alias_entries =
    List.map
      (fun a -> Printf.sprintf "      '%s': 'preact/compat'" a.from)
      [ react_alias; react_dom_alias ]
  in
  Printf.sprintf
    {|resolve: {
    alias: {
...
let find_compat lib = List.find_opt (fun c -> c.name = lib) library_compatibility
...
(** Alias to JSON *)
let alias_to_json a = `Assoc [ "from", `String a.from; "to", `String a.to_ ]
