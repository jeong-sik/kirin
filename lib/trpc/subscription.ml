...
let generate_id () = Printf.sprintf "sub_%d_%d" (Unix.getpid ()) (Random.int 1000000)
...
let encode_message ~id msg =
  ...
  | Data result ->
    `Assoc
      [ "id", `String id; "result", `Assoc [ "type", `String "data"; "data", result ] ]
  | Complete ->
    `Assoc [ "id", `String id; "result", `Assoc [ "type", `String "stopped" ] ]
  | Err { code; message } ->
    `Assoc
      [ "id", `String id
...
module Registry = struct
  ...
  let count t =
    Eio.Mutex.use_rw ~protect:true t.mutex (fun () -> List.length t.subscriptions)
  ;;

  let has t id =
    Eio.Mutex.use_rw ~protect:true t.mutex (fun () -> List.mem_assoc id t.subscriptions)
  ;;
end
...
(** Format SSE message *)
let sse_message msg = sse_event ~event:"message" ~data:(Yojson.Safe.to_string msg)
      ...
      (match method_ with
       | "subscription.start" ->
         ...
         let path =
           match params with
           | `Assoc p ->
             (match List.assoc_opt "path" p with
              | Some (`String s) -> s
              | _ -> "")
           | _ -> ""
         in
       ...
       | "subscription.stop" -> Some (Unsubscribe { id })
       | "ping" -> Some Ping
       | "pong" -> Some Pong
       | _ -> None)
    | _ -> None
  ...
  let encode = function
    | Subscribe { id; path; input } ->
      `Assoc
        ...
        ; "params", `Assoc [ "path", `String path; "input", input ]
        ]
    | Unsubscribe { id } ->
      `Assoc [ "id", `String id; "method", `String "subscription.stop" ]
