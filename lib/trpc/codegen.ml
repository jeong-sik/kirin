...
let rec generate_type = function
  ...
  | TObject fields ->
    let field_strs =
      List.map (fun (name, t) -> Printf.sprintf "  %s: %s;" name (generate_type t)) fields
    in
    Printf.sprintf "{\n%s\n}" (String.concat "\n" field_strs)
  | TOptional t -> Printf.sprintf "%s | undefined" (generate_type t)
  | TUnion types -> String.concat " | " (List.map generate_type types)
  | TCustom name -> name
;;
...
|}
  ...
  let parts = [ header ] in
  let parts =
    if input_types <> "" then parts @ [ "\n// Input Types\n"; input_types ] else parts
  in
  let parts =
    if output_types <> "" then parts @ [ "\n// Output Types\n"; output_types ] else parts
  in
...
let rec generate_json_schema = function
  ...
  | TArray t -> `Assoc [ "type", `String "array"; "items", generate_json_schema t ]
  | TObject fields ->
    let properties =
      `Assoc (List.map (fun (name, t) -> name, generate_json_schema t) fields)
    in
    let required = `List (List.map (fun (name, _) -> `String name) fields) in
    `Assoc [ "type", `String "object"; "properties", properties; "required", required ]
  | TOptional t ->
    `Assoc
      [ "oneOf", `List [ generate_json_schema t; `Assoc [ "type", `String "null" ] ] ]
  | TUnion types -> `Assoc [ "oneOf", `List (List.map generate_json_schema types) ]
  | TCustom name -> `Assoc [ "$ref", `String (Printf.sprintf "#/definitions/%s" name) ]
;;
...
let rec generate_zod = function
  ...
  | TObject fields ->
    let field_strs =
      List.map (fun (name, t) -> Printf.sprintf "  %s: %s," name (generate_zod t)) fields
    in
  ...
  let schemas =
    List.map
      (fun info ->
         let input_schema =
           Printf.sprintf
             "export const %sInputSchema = %s;"
             info.name
             (generate_zod info.input_type)
         in
         let output_schema =
           Printf.sprintf
             "export const %sOutputSchema = %s;"
             info.name
             (generate_zod info.output_type)
         in
         input_schema ^ "\n" ^ output_schema)
      infos
  in
