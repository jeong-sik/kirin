...
(** List serializer *)
let list_output (s : 'a serializer) : 'a list serializer =
  fun items -> `List (List.map s items)
;;
...
let default_meta = { description = None; deprecated = false; tags = [] }
...
let rec type_info_to_typescript = function
  ...
  | TObject fields ->
    let field_strs =
      List.map
        (fun (name, t) -> Printf.sprintf "%s: %s" name (type_info_to_typescript t))
        fields
    in
    Printf.sprintf "{ %s }" (String.concat "; " field_strs)
  | TOptional t -> Printf.sprintf "%s | undefined" (type_info_to_typescript t)
  | TUnion types -> String.concat " | " (List.map type_info_to_typescript types)
  | TCustom name -> name
;;
