...
let base_for_test ?(headers = []) ?(path = "/") () : base =
  let dummy_request : Request.t =
    ...
    let dummy_buf_read = Eio.Buf_read.of_string "" in
...
let create ~data req = { base = base_of_request req; data }

(** Map context data *)
let map f ctx = { base = ctx.base; data = f ctx.data }
...
let unit_factory : unit factory = fun req -> create ~data:() req
