...
let response_to_json resp =
  ...
  | Ok result -> `Assoc (base @ [ "result", result ])
  | Error err ->
    let error_obj = [ "code", `Int err.code; "message", `String err.message ] in
    let error_obj =
      match err.data with
...
let create_batch_request calls =
  `List
    (List.map
       (fun (id, path, input) ->
          `Assoc [ "id", `Int id; "path", `String path; "input", input ])
       calls)
;;
