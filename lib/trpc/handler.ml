...
let default_config = { prefix = "/trpc"; enable_batching = true; max_batch_size = 10 }
...
let parse_path ~prefix url =
  ...
  if
    String.length url_path > String.length prefix
  ...
  then (
    ...
    if String.length rest > 0 && String.get rest 0 = '/'
    then Some (String.sub rest 1 (String.length rest - 1))
    else if rest = ""
    then None
    else Some rest)
  else None
...
let success_response result =
  let json = `Assoc [ "result", `Assoc [ "data", result ] ] in
  Response.json json
;;
...
let error_response ?(code = -32603) message =
  let json =
    `Assoc [ "error", `Assoc [ "message", `String message; "code", `Int code ] ]
  in
  Response.json json |> Response.with_status `Internal_server_error
;;
...
let not_found_response path =
  ...
  in
  Response.json json |> Response.with_status `Not_found
;;
...
let handle_query router ctx_factory path req =
  ...
  | None -> error_response ~code:Batch.ErrorCode.parse_error "Invalid input JSON"
  | Some input ->
...
let handle_mutation router ctx_factory path body req =
  ...
  | Error msg -> error_response ~code:Batch.ErrorCode.parse_error msg
  | Ok input ->
    (match Trpc_router.execute_procedure path ctx input router with
     | Ok result -> success_response result
     | Error msg -> error_response msg)
...
let handle_batch ~config router ctx_factory body req =
  ...
  | Ok json ->
    ...
    if List.length calls > config.max_batch_size
    then
      error_response
        ~code:Batch.ErrorCode.invalid_request
  ...
  | Some path ->
    (match method_ with
     ...
     | `POST ->
       ...
       then handle_batch ~config router ctx_factory body req
       else handle_mutation router ctx_factory path body req
     | _ ->
...
let routes ?(config = default_config) ~ctx_factory router =
  ...
  [ get pattern handler; post pattern handler ]
;;
