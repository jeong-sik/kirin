...
let box_procedure (proc : ('ctx, 'input, 'output) t) ~input_type ~output_type
  : 'ctx boxed_procedure
  =
  { path = proc.path
...
let create ?(prefix = "") () : 'ctx t = { prefix; procedures = []; children = [] }

(** Get router prefix *)
let get_prefix router = router.prefix

(** Add a procedure to the router *)
let add_procedure proc router = { router with procedures = proc :: router.procedures }
  ...
  | None ->
    ...
    let check_child (prefix, child) =
      if
        ...
        && String.get path (String.length prefix) = '.'
      then (
        let sub_path =
          String.sub
            path
            (String.length prefix + 1)
            (String.length path - String.length prefix - 1)
        in
...
let rec all_procedures ?(prefix = "") router =
  let prefixed_path p = if prefix = "" then p else prefix ^ "." ^ p in
  let direct =
    List.map (fun p -> { p with path = prefixed_path p.path }) router.procedures
  in
  let nested =
    List.concat_map
      (fun (child_prefix, child) ->
...
let all_procedure_infos router = all_procedures router |> List.map (fun p -> p.info)
...
(** Get procedure paths *)
let paths router = all_procedures router |> List.map (fun p -> p.path)

(** Get queries *)
let queries router = all_procedures router |> List.filter (fun p -> p.proc_type = Query)
