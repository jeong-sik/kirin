...
(** Re-export all schema types and builders for backward compatibility. *)
...
(** Get JSON spec as Yojson value (for Kirin integration).
    Usage: [Router.get "/openapi.json" (fun _ -> Response.json (Openapi.spec_json spec))] *)
let spec_json spec = to_json spec

(** Get Swagger UI HTML (for Kirin integration).
