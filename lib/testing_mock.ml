...
let on ?(meth = None) ~path ~status ?(headers = []) ?(body = "") t =
  let endpoint =
    { meth; path; response = { Testing_response.status; headers; body }; call_count = 0 }
  in
...
let assert_called ~path t =
  if call_count ~path t = 0
  then Testing_assert.fail (Printf.sprintf "Expected endpoint '%s' to be called" path)
;;
...
let assert_called_times ~path ~times t =
  ...
  then
    Testing_assert.fail
      (Printf.sprintf
         "Expected endpoint '%s' to be called %d times but was called %d times"
         path
         times
         actual)
;;
...
let assert_no_unmatched t =
  if t.unmatched_requests <> []
  then
    Testing_assert.fail
      (Printf.sprintf "Found %d unmatched requests" (List.length t.unmatched_requests))
;;
