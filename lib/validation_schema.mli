type error = { path : string list; message : string; code : string; }
...
val string :
  ?min_length:int ->
  ?max_length:int -> ?pattern:string -> ?format:string -> unit -> schema
val int :
  ?minimum:int ->
  ?maximum:int ->
  ?exclusive_min:bool ->
  ?exclusive_max:bool -> ?multiple_of:int -> unit -> schema
val float :
  ?minimum:float ->
  ?maximum:float ->
  ?exclusive_min:bool ->
  ?exclusive_max:bool -> ?multiple_of:float -> unit -> schema
val bool : unit -> schema
val null : unit -> schema
val array :
  ?min_items:int -> ?max_items:int -> ?unique_items:bool -> schema -> schema
val object_ :
  ?required:string list ->
  ?additional:bool -> (string * schema) list -> schema
val field : 'a -> 'b -> 'a * 'b
