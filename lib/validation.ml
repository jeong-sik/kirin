...
(** Create error *)
let make_error ?(path = []) ~code message = { path; message; code }
...
let validate_string constraints path value =
  ...
  (match constraints.min_length with
   | Some min when String.length value < min ->
     ...
     := make_error
          ...
          (Printf.sprintf "String must be at least %d characters" min)
        :: !errors
   | _ -> ());
  (match constraints.max_length with
   | Some max when String.length value > max ->
     ...
     := make_error
          ...
          (Printf.sprintf "String must be at most %d characters" max)
        :: !errors
   | _ -> ());
  (match constraints.pattern with
   | Some pattern ->
     let re = Str.regexp pattern in
     if not (Str.string_match re value 0)
     then
       errors
       := make_error
            ~path
            ~code:"pattern_mismatch"
            (Printf.sprintf "String does not match pattern: %s" pattern)
          :: !errors
   | None -> ());
  (match constraints.format with
   ...
   | None -> ());
  if !errors = [] then Ok (`String value) else Error !errors
;;
   ...
   | Some min ->
     ...
     then
       ...
       := make_error
            ...
            (Printf.sprintf
               "Number must be %s %g"
               (if constraints.exclusive_min then "greater than" else "at least")
               min)
          :: !errors
   | None -> ());
   ...
   | Some max ->
     ...
     then
       ...
       := make_error
            ...
            (Printf.sprintf
               "Number must be %s %g"
               (if constraints.exclusive_max then "less than" else "at most")
               max)
          :: !errors
   | None -> ());
  (match constraints.multiple_of with
   | Some m when m <> 0. ->
     if mod_float value m <> 0.
     then
       errors
       := make_error
            ~path
            ~code:"not_multiple_of"
            (Printf.sprintf "Number must be a multiple of %g" m)
          :: !errors
   | _ -> ());
  !errors
;;
...
let rec validate_array item_schema constraints path items =
  ...
  (match constraints.min_items with
   | Some min when List.length items < min ->
     ...
     := make_error
          ...
          (Printf.sprintf "Array must have at least %d items" min)
        :: !errors
   | _ -> ());
  (match constraints.max_items with
   | Some max when List.length items > max ->
     ...
     := make_error
          ~path
          ~code:"array_too_long"
          (Printf.sprintf "Array must have at most %d items" max)
        :: !errors
   | _ -> ());
  ...
  let validated_items =
    List.mapi
      ...
      items
  in
  if !errors = [] then Ok (`List validated_items) else Error !errors
...
and validate_object obj_schema path json =
  ...
  | `Assoc fields ->
    ...
    List.iter
      (fun req ->
         if not (List.mem_assoc req fields)
         then
           errors
           := make_error
                ~path:(path @ [ req ])
                ~code:"required_field"
                (Printf.sprintf "Required field '%s' is missing" req)
              :: !errors)
      obj_schema.required;
    (* Validate each field *)
    List.iter
      (fun (name, value) ->
         ...
         | None ->
           ...
           then validated := (name, value) :: !validated
           else
             errors
             := make_error
                  ~path:field_path
                ...
                :: !errors)
      fields;
    if !errors = [] then Ok (`Assoc (List.rev !validated)) else Error !errors
  | _ -> Error [ make_error ~path ~code:"type_error" "Expected object" ]
...
and validate_schema schema path json =
  ...
  | Enum values, json ->
    if List.exists (fun v -> v = json) values
    then Ok json
    else
      Error
        [ make_error ~path ~code:"invalid_enum" "Value is not one of the allowed values" ]
  | Const expected, json ->
    if expected = json
    then Ok json
    else
      Error
        [ make_error ~path ~code:"const_mismatch" "Value does not match expected constant"
        ]
  | OneOf schemas, json ->
    let results = List.map (fun s -> validate_schema s path json) schemas in
    let valid = List.filter Result.is_ok results in
    if List.length valid = 1
    then List.hd valid
    else if List.length valid = 0
    then
      Error
        [ make_error
            ~path
...
let validate schema json = validate_schema schema [] json

(** Validate and return typed result *)
let validate_json schema json = validate schema json
...
let validate_string_json schema str =
  ...
  with
  | Yojson.Json_error _ -> Error [ make_error ~code:"parse_error" "Invalid JSON" ]
;;
    ...
    (match of_yojson validated_json with
     ...
     | Error msg ->
       Error
         [ make_error
             ~code:"mapping_error"
             (Printf.sprintf "Failed to map JSON to type: %s" msg)
         ])
;;
...
let validated_body of_yojson schema handler next req =
  ...
  | Error _ ->
    Response.json ~status:`Bad_request (`Assoc [ "error", `String "Invalid JSON" ])
  | Ok json ->
...
module Type = struct
  ...
  let string ?min ?max ?pattern ?format () =
    make
      (String { min_length = min; max_length = max; pattern; format })
      (fun j ->
         match j with
         | `String s -> Ok s
...
let validate_body schema body_string = validate_string_json schema body_string
       ...
       | Some value ->
         ...
         (match validate schema json with
          ...
          | Error errs ->
            errors
            := !errors
               @ List.map (fun e -> { e with path = [ "query"; name ] @ e.path }) errs))
    params;
