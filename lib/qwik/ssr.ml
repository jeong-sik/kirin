...
(** Create cache *)
let create_cache ~max_size ~default_ttl = { entries = []; max_size; default_ttl }
...
let cache_put cache key html ?container_state () =
  let entry =
    { html; container_state; created_at = Unix.gettimeofday (); ttl = cache.default_ttl }
  in
  ...
  (* Evict if over max *)
  if List.length cache.entries > cache.max_size
  then cache.entries <- List.rev cache.entries |> List.tl |> List.rev
;;
...
let create config =
  ...
  in
  { config; cache; stats = create_stats (); running = true; next_id = 1 }
;;
...
let prerender engine ~routes ~output_dir =
  List.iter
    (fun route ->
       ...
       | Error msg -> Kirin.Logger.error "Failed to prerender %s: %s" url msg)
    routes
;;
