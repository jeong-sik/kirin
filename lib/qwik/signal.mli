...
val serialize_signal :
  'a signal -> ('a -> Yojson.Safe.t) -> serialized_signal
val signal_to_json :
  'a signal ->
  ('a ->
   ([> `List of [> `String of signal_id ] list | `String of signal_id ] as 'b)) ->
  [> `Assoc of (string * 'b) list ]
val resource_to_json :
  'a resource ->
  ('a -> ([> `String of string ] as 'b)) ->
  [> `Assoc of
       (string * [> `Assoc of (string * 'b) list | `String of signal_id ])
       list ]
...
val register_resource :
  context -> 'a resource -> ('a -> Yojson.Safe.t) -> unit
val register_store : context -> store -> unit
val context_to_json :
  context ->
  [> `Assoc of
       (string * [> `Assoc of (signal_id * Yojson.Safe.t) list ]) list ]
