...
(** Create static route *)
let static path = { (empty path) with render_mode = Static }

(** Create SPA route *)
let spa path = { (empty path) with render_mode = SPA }
...
(** Add loader *)
let with_loader loader route = { route with loaders = loader :: route.loaders }

(** Add action *)
let with_action action route = { route with actions = action :: route.actions }
...
let with_on_request handler route = { route with on_request = Some handler }

(** Set onGet handler *)
let with_on_get handler route = { route with on_get = Some handler }

(** Set onPost handler *)
let with_on_post handler route = { route with on_post = Some handler }

(** Add children routes *)
let with_children children route = { route with children }
...
let extract_params path =
  ...
  List.filter_map
    (fun seg ->
       ...
       then
         if seg.[0] = '[' && seg.[String.length seg - 1] = ']'
         then (
           let inner = String.sub seg 1 (String.length seg - 2) in
           if String.length inner > 3 && String.sub inner 0 3 = "..."
           then
             (* [...catchAll] *)
             Some
               { param_name = String.sub inner 3 (String.length inner - 3)
               ; param_type = Slug
               }
           else if inner.[0] = '[' && inner.[String.length inner - 1] = ']'
           then (
             (* [[optional]] *)
             let name = String.sub inner 1 (String.length inner - 2) in
             Some { param_name = name; param_type = Optional String })
           else Some { param_name = inner; param_type = String })
         else None
       else None)
    segments
;;
...
let matches route request_path =
  ...
  let route_segs =
    String.split_on_char '/' route.path |> List.filter (fun s -> s <> "")
  in
  let path_segs =
    String.split_on_char '/' request_path |> List.filter (fun s -> s <> "")
  in
  let rec match_segs route_segs path_segs =
    ...
    | [], _ -> false
    | rs :: rrest, ps :: prest ->
      if String.length rs > 0 && rs.[0] = '['
      then
        (* Dynamic segment *)
        if String.length rs > 4 && String.sub rs 1 3 = "..."
        then
          (* Catch-all matches rest *)
          true
        else match_segs rrest prest
      else if rs = ps
      then match_segs rrest prest
      else false
    | _ :: _, [] ->
        ...
        | Some h ->
          `Assoc
            ...
            ; ( "links"
              , `List
                  (List.map
                     (fun l -> `Assoc [ "rel", `String l.rel; "href", `String l.href ])
                     h.links) )
            ]
        | None -> `Null )
...
(** Create layout route *)
let layout name children = { (empty "") with layout = Some name; children }

(** Create catch-all route *)
let catch_all () = ssr "[...catchAll]"

(** Create optional segment route *)
let optional segment = ssr (Printf.sprintf "[[%s]]" segment)
