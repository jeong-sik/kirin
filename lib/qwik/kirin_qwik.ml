...
let static_config ~dist_path () =
  { Handler.default_config with mode = Handler.Static; dist_path }
;;
...
(** Create SPA route *)
let spa_route path = Route_def.spa path
...
(** Create signal *)
let signal value = Signal.create value

(** Create computed signal *)
let computed fn = Signal.computed fn

(** Create resource signal *)
let resource track_fn = Signal.resource track_fn

(** {1 Container Helpers} *)

(** Create container *)
let create_container () = Container.create ()

(** Serialize container to script tag *)
let container_script container = Container.script_tag container

(** {1 Loader Helpers} *)

(** Create data loader *)
let loader ~name f = Loader.data ~name f

(** Execute loader *)
let run_loader loader ctx = Loader.execute loader ctx

(** {1 Action Helpers} *)

(** Create route action *)
let action ~name f = Action.route_action ~name f

(** Execute action *)
let run_action action ctx = Action.execute action ctx
...
let qwik_handler config = fun request -> Handler.catch_all_handler config request
...
let discover_routes ~root_dir () = File_router.discover_routes root_dir
