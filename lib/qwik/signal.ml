...
(** Create a signal *)
let create value = { id = gen_id (); value; subscribers = [] }

(** Create a computed signal *)
let computed compute = { id = gen_id (); compute; cached_value = None; dependencies = [] }

(** Create a resource signal *)
let resource track_fn = { id = gen_id (); state = ResourcePending; track_fn }
...
(** Set signal value *)
let set signal value = signal.value <- value

(** Update signal value with function *)
let update signal f = signal.value <- f signal.value
...
let invalidate comp = comp.cached_value <- None
...
(** Resolve resource *)
let resolve res value = res.state <- ResourceResolved value

(** Reject resource *)
let reject res error = res.state <- ResourceRejected error
...
(** Create a store *)
let create_store () = { id = gen_id (); props = []; nested_stores = [] }

(** Get store property *)
let get_prop store key = List.assoc_opt key store.props
...
(** Get nested store *)
let get_nested store key = List.assoc_opt key store.nested_stores

(** Set nested store *)
let set_nested store key nested =
  store.nested_stores
  <- (key, nested) :: List.filter (fun (k, _) -> k <> key) store.nested_stores
;;
...
let resource_to_json (res : 'a resource) value_to_json =
  let state_json =
    ...
    | ResourceResolved v ->
      `Assoc [ "state", `String "resolved"; "value", value_to_json v ]
    | ResourceRejected msg -> `Assoc [ "state", `String "rejected"; "error", `String msg ]
  in
  `Assoc [ "id", `String res.id; "resource", state_json ]
;;
...
(** Create context *)
let create_context () = { signals = []; resources = []; stores = [] }
...
let register_store ctx store = ctx.stores <- (store.id, store) :: ctx.stores
