...
(** Empty head *)
let empty = { title = None; metas = []; links = []; scripts = []; styles = [] }
...
(** Render style tag *)
let render_style css = Printf.sprintf "<style>%s</style>" css
...
let to_json head =
  `Assoc
    ...
    ; ( "links"
      , `List
          (List.map
             (fun l -> `Assoc [ "rel", `String l.link_rel; "href", `String l.link_href ])
             head.links) )
    ]
;;
