...
(** Create route action *)
let route_action ~name action = { name; scope = RouteAction; action }

(** Create global action *)
let global_action ~name action = { name; scope = GlobalAction; action }

(** {1 Action Execution} *)

(** Execute action *)
let execute action ctx = action.action ctx
...
let has_field form name = List.mem_assoc name form.fields
...
let email name form =
  ...
  | Some v -> if String.contains v '@' then Valid else Invalid [ name, "invalid email" ]
;;
...
let context_of_request ~url ~params ~headers ~cookies ~form () =
  { url; params; headers; cookie = cookies; form_data = form }
;;
...
let field_value_to_json = function
  ...
  | File { name; content_type; size } ->
    `Assoc
      [ "name", `String name; "contentType", `String content_type; "size", `Int size ]
;;
...
let result_to_json value_to_json = function
  | ActionOk v -> `Assoc [ "type", `String "ok"; "data", value_to_json v ]
  | ActionFail { status; errors } ->
    `Assoc
      [ "type", `String "fail"
      ; "status", `Int status
      ; "errors", `Assoc (List.map (fun (k, v) -> k, `String v) errors)
      ]
  | ActionRedirect url -> `Assoc [ "type", `String "redirect"; "url", `String url ]
;;

(** {1 QRL Integration} *)

(** Create action QRL *)
let to_qrl ~chunk ~symbol action = Qrl.create ~chunk ~symbol ~dev_name:action.name ()
...
let with_qrl ~chunk ~symbol action = { action; qrl = to_qrl ~chunk ~symbol action }
