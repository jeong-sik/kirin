...
let of_string ~chunk ~symbol = create ~chunk ~symbol ()
...
let with_refs ~chunk ~symbol refs = create ~chunk ~symbol ~capture:(CaptureRef refs) ()
...
let to_string qrl =
  ...
  | NoCapture -> base
  | Capture vars -> Printf.sprintf "%s[%s]" base (String.concat " " vars)
  | CaptureRef refs ->
  ...
  let parse_base s =
    ...
    | [ chunk; rest ] ->
      ...
      then (
        ...
        with
        | Failure _ -> Some { chunk; symbol; capture = Capture captures; dev_name = None })
      else Some { chunk; symbol = rest; capture = NoCapture; dev_name = None }
    | _ -> None
  in
  parse_base s
;;
...
let on_click qrl = Printf.sprintf {|on:click="%s"|} (to_string qrl)

(** Generate on:input QRL attribute *)
let on_input qrl = Printf.sprintf {|on:input="%s"|} (to_string qrl)

(** Generate on:submit QRL attribute *)
let on_submit qrl = Printf.sprintf {|on:submit="%s"|} (to_string qrl)

(** Generate custom event QRL attribute *)
let on_event ~event qrl = Printf.sprintf {|on:%s="%s"|} event (to_string qrl)

(** Generate window event QRL attribute *)
let on_window ~event qrl = Printf.sprintf {|on-window:%s="%s"|} event (to_string qrl)

(** Generate document event QRL attribute *)
let on_document ~event qrl = Printf.sprintf {|on-document:%s="%s"|} event (to_string qrl)
...
let prefetch_hint ?(strategy = PrefetchEvent) qrl =
  ...
  in
  Printf.sprintf
    {|<link rel="modulepreload" href="/%s" data-qwik-prefetch="%s" />|}
    qrl.chunk
    strategy_str
;;
...
let component ~chunk ~symbol ?(styles = []) ?(slot_projection = false) () =
  { qrl = create ~chunk ~symbol (); styles; slot_projection }
;;
...
let of_json json =
  ...
  let capture =
    match json |> member "capture" with
    | `Null -> NoCapture
    | `List items ->
      (try CaptureRef (List.map to_int items) with
       | Yojson.Safe.Util.Type_error (_, _) -> Capture (List.map to_string items))
    | _ -> NoCapture
  in
  { chunk; symbol; capture; dev_name }
;;
