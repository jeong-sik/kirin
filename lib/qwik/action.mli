...
type file_info = { name : string; content_type : string; size : int; }
type field_value =
    Text of string
...
type scope = RouteAction | GlobalAction
type 'a t = { name : string; scope : scope; action : context -> 'a result; }
...
type validation_result = Valid | Invalid of (string * string) list
...
val context_of_request :
  ...
  params:(string * string) list ->
  headers:(string * string) list ->
  cookies:(string * string) list -> form:form_data -> unit -> context
...
type 'a action_qrl = { action : 'a t; qrl : Qrl.t; }
