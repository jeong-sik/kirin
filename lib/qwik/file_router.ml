...
let parse_segment name =
  ...
  if len = 0
  then { raw = name; segment_type = Static ""; priority = 0 }
...
let detect_file_type filename =
  ...
  if String.lowercase_ascii name_no_ext = "index"
  then Index
  else if String.lowercase_ascii name_no_ext = "layout"
  then Layout
  else if String.length name_no_ext >= 6 && String.sub name_no_ext 0 6 = "plugin"
  then Plugin
  else if String.lowercase_ascii name_no_ext = "service"
  then Service
  else
...
let rec to_route_def discovered =
  ...
  let route =
    if discovered.has_layout then Route_def.with_layout "layout" route else route
  in
  let route =
    List.fold_left (fun r l -> Route_def.with_loader l r) route discovered.loaders
  in
  let route =
    List.fold_left (fun r a -> Route_def.with_action a r) route discovered.actions
  in
...
let extract_params ~pattern ~url =
  ...
  let rec extract acc pats urls =
    ...
    | p :: prest, u :: urest ->
      ...
      then extract acc prest urest
      else None
    | _ :: prest, [] ->
      (* Check if remaining patterns are optional *)
      if List.for_all (fun p -> String.length p > 1 && p.[0] = ':' && p.[1] = '?') prest
