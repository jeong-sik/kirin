...
let render_request
      ...
      ()
  =
  { url; base; headers; container_attrs; server_data }
;;
...
let encode_render ~id request =
  let params =
    `Assoc
      ...
      ; ( "containerAttrs"
        , `Assoc (List.map (fun (k, v) -> k, `String v) request.container_attrs) )
      ; ( "serverData"
        , match request.server_data with
          | Some d -> d
          | None -> `Null )
      ]
  in
...
let encode_prefetch ~id ~qrls =
  let params = `Assoc [ "qrls", `List (List.map (fun q -> `String q) qrls) ] in
  let rpc =
    `Assoc
      [ "jsonrpc", `String "2.0"
      ; "id", `Int id
...
let decode_response str =
    ...
    | `Null ->
      ...
      Success
        ...
        ; result =
            ...
            ; timing
            ; head = None
            ; (* Would parse from result if present *)
  ...
  | e -> Failure { id = 0; code = -32700; message = Printexc.to_string e }
;;
...
let encode_batch requests =
  let json =
    `List
      (List.map
         (fun (id, req) -> Yojson.Safe.from_string (encode_render ~id req))
         requests)
  in
