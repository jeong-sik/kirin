...
let serve_static config path =
  ...
  if Kirin.Fs_compat.file_exists file_path && not (Kirin.Fs_compat.is_directory file_path)
  then (
     ...
     | Error msg ->
       ...
       then (
         ...
         then (
           let content = Kirin.Fs_compat.load index_path in
           html_response content)
         else error_response ~status:500 ~message:msg)
       else error_response ~status:500 ~message:msg)
;;
...
let handle_static config request =
  ...
  if Kirin.Fs_compat.file_exists html_path
  then (
    let content = Kirin.Fs_compat.load html_path in
    html_response content)
  else handle_ssr config request (* Fallback to SSR *)
;;
...
let handle_spa config =
  ...
  then (
    let content = Kirin.Fs_compat.load index_path in
    html_response content)
  else error_response ~status:404 ~message:"index.html not found"
;;

(** Main request handler *)
let handle ~config ~request () =
  (* First check for static assets *)
  if is_static_asset request.path
  then (
...
(** Catch-all handler *)
let catch_all_handler config request = handle ~config ~request ()
...
let handle_loader ~config ~request ~loader_name () =
  ...
  let _ = config in
  let _ = request in
  json_response (`Assoc [ "loader", `String loader_name; "data", `Assoc [] ])
;;
...
let handle_action ~config ~request ~action_name () =
  ...
  let _ = config in
  let _ = request in
  json_response (`Assoc [ "action", `String action_name; "result", `Assoc [] ])
;;
...
let handle_q_data ~config ~request () =
  let _ = config in
  json_response
    (`Assoc [ "url", `String request.path; "loaders", `Assoc []; "actions", `Assoc [] ])
;;
