type capture = NoCapture | Capture of string list | CaptureRef of int list
type t = {
  chunk : string;
  symbol : string;
  capture : capture;
...
val create :
  chunk:string ->
  symbol:string -> ?capture:capture -> ?dev_name:string -> unit -> t
...
val component :
  chunk:string ->
  symbol:string ->
  ?styles:string list -> ?slot_projection:bool -> unit -> component_qrl
val to_json :
  t ->
  [> `Assoc of
       (string *
