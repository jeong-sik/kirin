...
(** Create container *)
let create () = { objects = []; qrls = []; next_ref = 0; paused = false }
...
let get_object container ref_ = List.assoc_opt ref_ container.objects

(** Add QRL to container *)
let add_qrl container name qrl = container.qrls <- (name, qrl) :: container.qrls

(** Get QRL by name *)
let get_qrl container name = List.assoc_opt name container.qrls

(** {1 Pause/Resume} *)

(** Pause container (for SSR) *)
let pause container = container.paused <- true

(** Is container paused? *)
let is_paused container = container.paused

(** Resume container *)
let resume container = container.paused <- false
          ...
          (match List.assoc_opt "$signal$" props with
           ...
           | _ ->
             (match List.assoc_opt "$resource$" props with
              | Some (`Int r) ->
                let state =
                  try props |> List.assoc "$state$" |> to_string with
                  | Not_found | Yojson.Safe.Util.Type_error (_, _) -> "pending"
                in
                SResource (r, state)
              | _ ->
                (match List.assoc_opt "$store$" props with
                 | Some (`Int r) -> SStore r
                 | _ -> SObject (List.map (fun (k, v) -> k, sobj_of_json v) props))))))
;;
...
let to_json container =
  ...
  in
  let qrls_json = container.qrls |> List.map (fun (name, qrl) -> name, Qrl.to_json qrl) in
  `Assoc
    [ "objs", `Assoc objects_json
    ; "qrls", `Assoc qrls_json
    ; "ctx", `Assoc [ "paused", `Bool container.paused; "refs", `Int container.next_ref ]
    ]
;;
...
let prefetch_script container =
  ...
  let links =
    List.map
      (fun chunk -> Printf.sprintf {|<link rel="modulepreload" href="/%s" />|} chunk)
      unique_chunks
  in
  String.concat "\n" links
;;
...
(** Get context value *)
let get_ctx store id = List.assoc_opt id store.contexts

(** Context store to JSON *)
let ctx_store_to_json store = `Assoc store.contexts
