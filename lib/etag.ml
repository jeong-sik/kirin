...
let parse s =
  ...
  if String.length s >= 3 && String.sub s 0 2 = "W/"
  then (
    (* Weak ETag: W/"xxx" *)
...
let any_match etags target = List.exists (fun etag -> matches etag target) etags
...
let middleware : (Request.t -> Response.t) -> Request.t -> Response.t =
  ...
  | Response.Stream _ | Response.Producer _ ->
    resp (* ETag not supported for streams yet *)
  | Response.String body ->
    (* Only add ETag for successful responses with body *)
    if Response.status_code resp = 200 && String.length body > 0
    then (
      ...
      | Some inm_value ->
        ...
        if any_match client_etags etag
        then
          (* Return 304 Not Modified *)
          Response.empty `Not_modified |> Response.with_header "etag" etag_str
        else
...
let check_if_match req current_etag =
  ...
  | Some value ->
    ...
    (* Same parsing *)
    if any_match client_etags current_etag then `Ok else `Precondition_failed (* 412 *)
;;
