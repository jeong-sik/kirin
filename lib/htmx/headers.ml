...
let push_url url resp = Response.with_header "HX-Push-Url" url resp

(** Replace current URL without history *)
let replace_url url resp = Response.with_header "HX-Replace-Url" url resp

(** Full page redirect *)
let redirect url resp = Response.with_header "HX-Redirect" url resp

(** Refresh the page *)
let refresh resp = Response.with_header "HX-Refresh" "true" resp

(** {1 Content Control Headers} *)

(** Override the target element *)
let retarget selector resp = Response.with_header "HX-Retarget" selector resp

(** Override the swap method *)
let reswap method_ resp = Response.with_header "HX-Reswap" method_ resp

(** Override the select filter *)
let reselect selector resp = Response.with_header "HX-Reselect" selector resp
...
let reswap_typed swap resp = Response.with_header "HX-Reswap" (swap_to_string swap) resp

(** Reswap with modifiers (e.g., "innerHTML swap:1s settle:500ms") *)
let reswap_with_modifiers
      swap
      ?(swap_delay : float option)
      ?(settle_delay : float option)
  ...
  let mods = [] in
  let mods =
    match swap_delay with
    | Some d -> Printf.sprintf "swap:%.0fms" (d *. 1000.) :: mods
    | None -> mods
  in
  let mods =
    match settle_delay with
...
let location
  ...
  let obj =
    match target with
    | Some t -> ("target", `String t) :: obj
    | None -> obj
  in
  let obj =
    match swap with
    | Some s -> ("swap", `String (swap_to_string s)) :: obj
    | None -> obj
  in
  let obj =
    match source with
    | Some s -> ("source", `String s) :: obj
    | None -> obj
  in
...
let location_simple path resp = Response.with_header "HX-Location" path resp
...
let current_url req = Request.header "HX-Current-URL" req

(** Get the triggering element's ID *)
let trigger_id req = Request.header "HX-Trigger" req

(** Get the triggering element's name *)
let trigger_name req = Request.header "HX-Trigger-Name" req

(** Get the target element's ID *)
let target_id req = Request.header "HX-Target" req

(** Get the prompt response *)
let prompt_response req = Request.header "HX-Prompt" req
