...
(** Debounce trigger *)
let trigger_debounce event ms = "hx-trigger", Printf.sprintf "%s delay:%dms" event ms

(** Throttle trigger *)
let trigger_throttle event ms = "hx-trigger", Printf.sprintf "%s throttle:%dms" event ms

(** Changed trigger (only when value changes) *)
let trigger_changed event = "hx-trigger", Printf.sprintf "%s changed" event
...
let input
  ...
  let id =
    ...
    | None -> name
  in
  let base_attrs = [ "type", input_type_to_string type_; "name", name; "id", id ] in
  let base_attrs =
    match value with
  ...
  let attr_str =
    all_attrs
    |> List.map (fun (k, v) -> if v = "" then k else Printf.sprintf "%s=\"%s\"" k v)
    |> String.concat " "
  in
...
let textarea
  ...
  let id =
    ...
    | None -> name
  in
  let base_attrs = [ "name", name; "id", id; "rows", string_of_int rows ] in
  let base_attrs =
    match cols with
...
let option ~value ?(selected = false) label =
  if selected
  then Printf.sprintf "<option value=\"%s\" selected>%s</option>" value label
  else Printf.sprintf "<option value=\"%s\">%s</option>" value label
;;
...
let form
  ...
  let attrs =
    match target with
    | Some t -> ("hx-target", t) :: attrs
    | None -> attrs
  in
  let attrs =
    match indicator with
    | Some i -> ("hx-indicator", i) :: attrs
    | None -> attrs
  in
  let attr_str =
    attrs |> List.map (fun (k, v) -> Printf.sprintf "%s=\"%s\"" k v) |> String.concat " "
  in
...
let field ~label ~name ?(error : string option) input_html =
  let error_html =
    match error with
    | Some e ->
      Printf.sprintf "<span id=\"%s-error\" class=\"field-error\">%s</span>" name e
    | None -> Printf.sprintf "<span id=\"%s-error\" class=\"field-error\"></span>" name
  in
  Printf.sprintf
    {|<div class="field" id="%s-wrapper">
  <label for="%s">%s</label>
  %s
  %s
</div>|}
...
let validate_on_blur ~url = trigger_with_mods "blur" [ "hx-post"; url ]
