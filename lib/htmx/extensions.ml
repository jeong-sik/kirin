...
let use extensions = "hx-ext", String.concat ", " extensions
...
module Ws = struct
  ...
  let setup url = [ use_one "ws"; "ws-connect", url ]
end
...
module Sse = struct
  ...
  (** Full SSE setup *)
  let setup url event = [ use_one "sse"; "sse-connect", url; "sse-swap", event ]
end
...
module ResponseTargets = struct
  ...
  let target_status code selector = Printf.sprintf "hx-target-%d" code, selector
  ...
  let error_handling ~error_container =
    [ enable; target_4xx error_container; target_5xx error_container ]
  ;;
end
...
module LoadingStates = struct
  ...
  let with_spinner ~class_ ~delay_ms = [ enable; add_class class_; delay delay_ms ]
end
...
module ClassTools = struct
  ...
  let classes_add cls delay = "classes", Printf.sprintf "add %s:%s" cls delay

  (** Remove class after delay *)
  let classes_remove cls delay = "classes", Printf.sprintf "remove %s:%s" cls delay

  (** Toggle class after delay *)
  let classes_toggle cls delay = "classes", Printf.sprintf "toggle %s:%s" cls delay

  (** Chain multiple class operations *)
  let classes operations =
    let ops =
      operations
      |> List.map (fun (op, cls, delay) -> Printf.sprintf "%s %s:%s" op cls delay)
    in
...
let combine attr_lists = List.flatten attr_lists
