...
let model_debounce name ms = Printf.sprintf "x-model.debounce.%dms" ms, name
...
let transition_named ~enter ~leave =
  [ "x-transition:enter", enter; "x-transition:leave", leave ]
;;
...
let dispatch_detail event detail = Printf.sprintf "$dispatch('%s', %s)" event detail

(** {1 Stores} *)

(** Define a global store (for script tag) *)
let define_store name data = Printf.sprintf "Alpine.store('%s', %s)" name data
...
let component ?(id : string option) ?(class_ : string option) data_obj children =
  ...
  let attrs =
    match id with
    | Some i -> Printf.sprintf "id=\"%s\"" i :: attrs
    | None -> attrs
  in
  let attrs =
    match class_ with
