...
let notification ~id ?(class_ = "notification") message =
  Printf.sprintf
    "<div id=\"%s\" class=\"%s\" hx-swap-oob=\"true\">%s</div>"
    id
    class_
    message
;;
...
let success_message ~id message = notification ~id ~class_:"notification success" message

(** Update an error message *)
let error_message ~id message = notification ~id ~class_:"notification error" message

(** Clear a container *)
let clear ~id = Printf.sprintf "<div id=\"%s\" hx-swap-oob=\"true\"></div>" id

(** Delete an element *)
let delete ~id = Printf.sprintf "<div id=\"%s\" hx-swap-oob=\"delete\"></div>" id

(** Update multiple elements at once *)
let multi elements = String.concat "\n" elements
...
let field_error ~field_id message =
  Printf.sprintf
    "<span id=\"%s-error\" class=\"field-error\" hx-swap-oob=\"true\">%s</span>"
    field_id
    message
;;
...
let clear_field_error ~field_id =
  Printf.sprintf
    "<span id=\"%s-error\" class=\"field-error\" hx-swap-oob=\"true\"></span>"
    field_id
;;
...
let field_with_state ~field_id ~state content =
  Printf.sprintf
    "<div id=\"%s-wrapper\" class=\"field-wrapper %s\" hx-swap-oob=\"true\">%s</div>"
    field_id
    state
    content
;;
...
let counter ~id count =
  Printf.sprintf
    "<span id=\"%s\" class=\"counter\" hx-swap-oob=\"true\">%d</span>"
    id
    count
;;

(** Update cart badge *)
let cart_badge count = counter ~id:"cart-count" count

(** Update notification badge *)
let notification_badge count = counter ~id:"notification-count" count
...
let response ~main_content oob_updates = main_content ^ "\n" ^ multi oob_updates

(** Build response with only OOB updates (no main content) *)
let response_oob_only oob_updates = multi oob_updates
