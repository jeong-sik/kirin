...
(** Add island *)
let with_island island route = { route with islands = island :: route.islands }

(** Add multiple islands *)
let with_islands islands route = { route with islands = islands @ route.islands }
...
let extract_params path =
  ...
  List.filter_map
    (fun seg ->
       if String.length seg >= 3 && seg.[0] = '[' && seg.[String.length seg - 1] = ']'
       then (
         let inner = String.sub seg 1 (String.length seg - 2) in
         if String.length inner >= 4 && String.sub inner 0 3 = "..."
         then
           Some { name = String.sub inner 3 (String.length inner - 3); param_type = Rest }
         else Some { name = inner; param_type = String })
       else None)
    segments
;;
...
let is_dynamic route = String.contains route.path '[' && String.contains route.path ']'
...
let matches route url =
  ...
  let rec match_segments routes urls params =
    ...
    | r :: _, [] ->
      ...
      if String.length r >= 5 && String.sub r 0 4 = "[..."
      then Some (List.rev params)
      else None
    | r :: rs, u :: us ->
      ...
      then match_segments rs us params
      else None
  in
