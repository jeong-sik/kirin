...
let parse_segment s =
  ...
  if len >= 6 && String.sub s 0 3 = "[[." && String.sub s (len - 2) 2 = "]]"
  then (
    (* [[...param]] optional catch-all *)
    let inner = String.sub s 3 (len - 5) in
    if String.length inner >= 3 && String.sub inner 0 3 = ".."
    then OptionalCatchAll (String.sub inner 2 (String.length inner - 2))
    else Static s)
  else if len >= 5 && String.sub s 0 1 = "[" && String.sub s (len - 1) 1 = "]"
  then (
  ...
  let parts =
    List.map
      (fun p ->
         ...
         if name = "index" then "" else segment_to_pattern (parse_segment name))
      parts
  in
...
let route_priority route =
  ...
  let static_count =
    List.length (List.filter (fun s -> String.length s > 0 && s.[0] <> '[') segments)
  in
  let dynamic_count =
    List.length
      (List.filter
         (fun s -> String.length s > 0 && s.[0] = '[' && not (String.contains s '.'))
         segments)
  in
...
let sort_routes routes =
  List.sort (fun a b -> compare (route_priority b) (route_priority a)) routes
;;
...
let is_api_route file_path =
  String.length file_path >= 4 && String.sub file_path 0 4 = "api/"
;;
...
let endpoint_from_file ~pages_dir file_path method_ =
  let route_path = file_to_route_path file_path in
  let _ = pages_dir in
  { path = route_path; method_; handler = file_path }
;;
