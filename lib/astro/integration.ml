...
let react =
  { config =
      ...
      ; client_script =
          "import { createElement } from 'react'; import { hydrateRoot } from \
           'react-dom/client';"
...
let detect_framework filename =
  if Filename.check_suffix filename ".vue"
  then Some "vue"
  else if Filename.check_suffix filename ".svelte"
  then Some "svelte"
  else if Filename.check_suffix filename ".jsx" || Filename.check_suffix filename ".tsx"
  then
  ...
  let matches_include =
    ...
    || List.exists
         (fun p ->
            ...
            String.length p > 2
            && String.sub p (String.length p - String.length ext) (String.length ext)
               = ext)
         integration.include_patterns
  in
  let matches_exclude =
    List.exists
      (fun p ->
         let ext = Filename.extension filename in
         String.length p > 2
         && String.sub p (String.length p - String.length ext) (String.length ext) = ext)
      integration.exclude_patterns
  in
...
let get_client_script integration = integration.config.client_script

(** Get server script for integration *)
let get_server_script integration = integration.config.server_script
...
let generate_config integrations =
  ...
  let imports =
    List.map
      (fun i -> Printf.sprintf "import %s from '%s';" i.config.name i.config.renderer)
      enabled
  in
  let items = List.map (fun i -> Printf.sprintf "%s()" i.config.name) enabled in
  Printf.sprintf
    {|// Auto-generated integrations config
%s
