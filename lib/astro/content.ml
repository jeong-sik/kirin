...
(** Create collection *)
let define_collection ~name ~schema () = { name; schema; entries = [] }

(** Add entry to collection *)
let add_entry collection entry = { collection with entries = entry :: collection.entries }
...
(** Get entry by id *)
let get_entry_by_id collection id = List.find_opt (fun e -> e.id = id) collection.entries

(** Filter entries *)
let filter_entries pred collection = List.filter pred collection.entries
  ...
  List.iter
    (fun (name, field) ->
       ...
       | NumberField { min = Some m; _ }, Some (`Float f) when f < m ->
         errors
         := { field = name; message = Printf.sprintf "value must be >= %f" m } :: !errors
       ...
       | EnumField { values; _ }, Some (`String v) when not (List.mem v values) ->
         errors
         := { field = name
            ; message = Printf.sprintf "must be one of: %s" (String.concat ", " values)
            }
            :: !errors
       | _ -> ())
    schema.fields;
