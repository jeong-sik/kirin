...
(** Create cache *)
let create_cache max_entries = { entries = []; max_entries }
...
let cache_put cache key html head ttl =
  ...
  (* Trim if over limit *)
  let entries =
    if List.length entries > cache.max_entries
    then List.filteri (fun i _ -> i < cache.max_entries) entries
    else entries
  in
...
let cache_hit_rate stats =
  ...
  if total = 0 then 0.0 else float_of_int stats.cache_hits /. float_of_int total
;;
...
let create config =
  let cache =
    if config.enable_cache then Some (create_cache config.max_cache_entries) else None
  in
  { config; cache; stats = create_stats (); running = true }
;;
...
(** Shutdown engine *)
let shutdown engine = engine.running <- false

(** {1 Rendering} *)

(** Generate cache key *)
let cache_key ~url = url

(** Render page *)
let render engine ~url ?(props = `Assoc []) ?(islands = []) () =
  let _ = props, islands in
  (* Silence unused warnings *)
  if not engine.running
  then Error "Engine is not running"
  else (
...
<body>
  ...
  %s
             ...
             url
             (Island.hydration_script ())
         in
         let elapsed = (Unix.gettimeofday () -. start) *. 1000.0 in
         engine.stats.avg_render_ms
         <- ((engine.stats.avg_render_ms *. float_of_int (engine.stats.total_renders - 1))
             +. elapsed)
...
let generate_all_static engine routes output_dir =
  List.filter_map
    (fun route ->
       if route.Route_def.prerender = Route_def.Static
       then (
         let output_path =
           Filename.concat
             output_dir
             (if route.Route_def.path = "/"
              then "index.html"
              else route.Route_def.path ^ ".html")
         in
