...
let rec schema_field_to_ts = function
  ...
  | Content.EnumField { values; _ } ->
    ...
    values_str
  | Content.ArrayField { items; _ } -> schema_field_to_ts items ^ "[]"
...
let route_params_type route =
  ...
  then "Record<string, never>"
    ...
    let fields =
      List.map (fun p -> Printf.sprintf "  %s: string;" p) params |> String.concat "\n"
    in
    Printf.sprintf "{\n%s\n}" fields)
;;
...
let route_to_ts route =
  ...
  Printf.sprintf
    ...
    path
    params
    (if route.Route_def.prerender = Route_def.Static then "true" else "false")
;;
...
let generate_types_file ~routes ~collections =
  let collection_types =
    List.map
      (fun c -> schema_to_ts c.Content.schema ^ "\n\n" ^ collection_entry_type c)
      collections
    |> String.concat "\n\n"
  in
...
|}
    ...
    (if collections = []
     then "never"
       ...
       List.map (fun c -> Printf.sprintf "\"%s\"" c.Content.name) collections
       |> String.concat " | ")
;;
  ...
  let route_entries =
    List.map
      (fun route ->
         let path = route.Route_def.path in
         let prerender =
           match route.Route_def.prerender with
...
let rec schema_field_to_zod = function
  ...
  | Content.ReferenceField _ -> "z.object({ collection: z.string(), id: z.string() })"
  | Content.ImageField _ ->
