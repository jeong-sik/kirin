...
(** Empty head *)
let empty =
  { title = None; meta = []; links = []; scripts = []; base = None; canonical = None }
;;
...
(** Description meta *)
let description content = Name { name = "description"; content }

(** Keywords meta *)
let keywords content = Name { name = "keywords"; content }

(** Author meta *)
let author content = Name { name = "author"; content }

(** Viewport meta *)
let viewport content = Name { name = "viewport"; content }

(** Robots meta *)
let robots content = Name { name = "robots"; content }
...
(** OG title *)
let og_title content = Property { property = "og:title"; content }

(** OG description *)
let og_description content = Property { property = "og:description"; content }

(** OG image *)
let og_image content = Property { property = "og:image"; content }

(** OG URL *)
let og_url content = Property { property = "og:url"; content }

(** OG type *)
let og_type content = Property { property = "og:type"; content }

(** OG site name *)
let og_site_name content = Property { property = "og:site_name"; content }

(** {1 Twitter Cards} *)

(** Twitter card *)
let twitter_card content = Name { name = "twitter:card"; content }

(** Twitter title *)
let twitter_title content = Name { name = "twitter:title"; content }

(** Twitter description *)
let twitter_description content = Name { name = "twitter:description"; content }

(** Twitter image *)
let twitter_image content = Name { name = "twitter:image"; content }

(** Twitter site *)
let twitter_site content = Name { name = "twitter:site"; content }
...
let seo ~title ?description:desc ?og_image_url ?twitter_card_type ?canonical_url () =
  ...
  let head =
    ...
    | None -> head
  in
  let head = head |> with_meta (og_title title) |> with_meta (twitter_title title) in
  let head =
    match og_image_url with
    | Some url -> head |> with_meta (og_image url) |> with_meta (twitter_image url)
    | None -> head
  in
  let head =
    match twitter_card_type with
...
let render_meta = function
  ...
  | Charset encoding -> Printf.sprintf {|<meta charset="%s">|} encoding
;;
...
let render_script script =
  ...
  | Some content ->
    Printf.sprintf "<script %s>%s</script>" (String.concat " " attrs) content
