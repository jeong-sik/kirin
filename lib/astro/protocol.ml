...
let render_island_request ~island_id ~component ?(props = `Assoc []) () =
  ...
  ; params =
      `Assoc
        [ "islandId", `String island_id; "component", `String component; "props", props ]
  }
;;
...
let content_request ~collection ~slug () =
  { id = gen_id ()
  ; method_ = "getContent"
  ; params = `Assoc [ "collection", `String collection; "slug", `String slug ]
  }
;;

(** Create health check request *)
let health_request () = { id = gen_id (); method_ = "health"; params = `Assoc [] }
...
let decode_response s =
  ...
  | e -> Error { id = 0; code = parse_error; message = Printexc.to_string e; data = None }
;;

(** {1 Response Construction} *)

(** Create success response *)
let success_response ~id result = Success { id; result }

(** Create error response *)
let error_response ~id ~code ~message ?data () = Error { id; code; message; data }
...
let encode_response = function
  | Success { id; result } ->
    let json = `Assoc [ "jsonrpc", `String "2.0"; "id", `Int id; "result", result ] in
    Yojson.Safe.to_string json ^ "\n"
  | Error { id; code; message; data } ->
    let error_obj =
    ...
    in
    let json =
      `Assoc [ "jsonrpc", `String "2.0"; "id", `Int id; "error", `Assoc error_obj ]
    in
    Yojson.Safe.to_string json ^ "\n"
;;
...
let parse_render_result json =
  ...
  let islands =
    json |> member "islands" |> to_assoc |> List.map (fun (k, v) -> k, to_string v)
  in
  let scripts = json |> member "scripts" |> to_list |> List.map to_string in
