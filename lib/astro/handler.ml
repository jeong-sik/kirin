...
let serve_static config path =
  ...
  let try_paths =
    [ Filename.concat config.dist_path path; Filename.concat config.public_path path ]
  in
  let rec try_serve = function
    | [] -> None
    | file_path :: rest ->
      if
        Kirin.Fs_compat.file_exists file_path
        && not (Kirin.Fs_compat.is_directory file_path)
      then (
...
let handle_static config request =
  let path =
    if request.path = "/"
    then "/index.html"
    else if Filename.extension request.path = ""
    then request.path ^ ".html"
    else request.path
  in
  ...
  else error_response ~status:404 ~message:"Page not found"
;;
...
let handle_server config request =
  ...
  | Some engine ->
    ...
    (match Ssr.render engine ~url:request.path ~islands () with
     | Ok html ->
       let html =
         if config.dev_mode then Ssr.inject_dev_scripts html config.dev_port else html
       in
       html_response html
...
let handle_hybrid config request =
  ...
  | _ ->
    ...
    if static_result.status = 404 then handle_server config request else static_result
;;
...
let handle_api ~config ~request ~endpoint () =
  let _ = config in
  json_response
    (`Assoc [ "endpoint", `String endpoint; "method", `String request.method_ ])
;;
