...
let create () =
  { checks = Hashtbl.create 16; start_time = Unix.gettimeofday (); manual_ready = true }
;;

(** Register a named health check *)
let register t name check = Hashtbl.replace t.checks name check

(** Set ready state manually *)
let set_ready t ready = t.manual_ready <- ready

(** Check if ready (including manual flag) *)
let is_ready t = t.manual_ready
(** {1 Check Logic} *)

(** Run all checks and aggregate status *)
let check t =
  let results =
    Hashtbl.fold (fun name check acc -> (name, check ()) :: acc) t.checks []
  in
  let overall =
    List.fold_left
      (fun acc (_, status) ->
...
let default_headers =
  Http.Header.of_list [ "Content-Type", "application/json"; "Cache-Control", "no-cache" ]
;;
...
let live_handler _t =
  fun _req ->
  Response.make ~status:`OK ~headers:default_headers (`String "{\"status\":\"alive\"}")
;;
...
let ready_handler t =
  ...
  match status with
  | Healthy | Degraded _ ->
    Response.make ~status:`OK ~headers:default_headers (`String "{\"status\":\"ready\"}")
  | Unhealthy _ ->
    Response.make
