...
val sveltekit_shell :
  ...
  ?ssr_html:string ->
  ?initial_data:Data.initial_data ->
  ?nonce:string -> ?body_class:string -> unit -> string
val error_page :
  title:string -> status:int -> message:string -> unit -> string
val streaming_placeholder : id:string -> fallback:string -> string
val streaming_replacement : id:string -> string -> string
val island :
  component_name:string -> props_json:Yojson.Safe.t -> string -> string
