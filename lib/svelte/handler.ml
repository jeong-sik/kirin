...
let html_response ~options:_ result =
  let status =
    match result.Protocol.status with
    | 200 -> `OK
  ...
  | Some url -> Kirin.Response.redirect ~status url
  | None ->
...
let fallback_response ~options:_ =
  let html = Hydrate.spa ~title:"Loading..." ~entry_script:"/src/entry-client.ts" () in
...
let page_handler ~options req =
  ...
  | Some engine ->
    ...
    let props =
      match Kirin.Request.json_body req with
      | Ok json -> json
      | Error _ -> `Assoc []
    in
    (match Ssr.render engine ~url ~props () with
     | Ok result -> html_response ~options result
     | Error _ ->
...
let load_handler ~loader req =
  let url = Kirin.Request.path req in
  let params = [] in
  (* TODO: Extract from route context *)
  let ctx = Loader.create_context ~url ~params ~route_id:url ~cookies:[] ~headers:[] () in
  match loader ctx with
  | Loader.LoadData data ->
    Kirin.Response.json (`Assoc [ "type", `String "data"; "data", data ])
  ...
  | Loader.LoadError (status, message) ->
    ...
    Kirin.Response.json
      ~status:status_code
      (`Assoc
          [ "type", `String "error"; "status", `Int status; "message", `String message ])
  | Loader.LoadNotFound ->
    Kirin.Response.json ~status:`Not_found (`Assoc [ "type", `String "notFound" ])
;;
...
let action_handler ~actions req =
  ...
  | None ->
    Kirin.Response.json
      ~status:`Not_found
      (`Assoc [ "type", `String "error"; "message", `String "Action not found" ])
  | Some a ->
    let ctx = Action.create_context ~url ~route_id:url ~headers:[] ~cookies:[] () in
    (match a.Action.handler ctx form_data with
     | Action.ActionSuccess data ->
       Kirin.Response.json (`Assoc [ "type", `String "success"; "data", data ])
     | Action.ActionFail (status, data) ->
       ...
       Kirin.Response.json
         ~status:status_code
         (`Assoc [ "type", `String "fail"; "status", `Int status; "data", data ])
...
let health_handler ~options _req =
  match options.ssr_engine with
  | None -> Kirin.Response.json (`Assoc [ "ok", `Bool true; "ssr", `Bool false ])
  | Some engine ->
...
let routes ~options =
  ...
  (* Base routes *)
  let base = [ Kirin.get "/_health" health ] in
  (* Dev mode routes *)
  let dev_routes =
    if options.dev_mode
    then
...
let load_routes ~loaders =
  List.map
    (fun (path, loader) -> Kirin.get (path ^ "/__data.json") (load_handler ~loader))
    loaders
;;

(** Create action routes *)
let action_routes ~actions_map =
  List.map (fun (path, actions) -> Kirin.post path (action_handler ~actions)) actions_map
;;
