...
let default_options =
  { strategy = Hover; preload_type = Data; behaviors = []; reload = false }
;;

(** No preloading *)
let no_preload = { default_options with strategy = Off }

(** Eager preload *)
let eager_preload = { default_options with strategy = Eager }
...
let link ~href ?options text =
  ...
  if attrs = ""
  then Printf.sprintf {|<a href="%s">%s</a>|} href (Kirin.html_escape text)
  else Printf.sprintf {|<a href="%s" %s>%s</a>|} href attrs (Kirin.html_escape text)
;;

(** Generate link with tap preload *)
let link_tap ~href text = link ~href ~options:{ default_options with strategy = Tap } text

(** Generate link with no preload *)
let link_no_preload ~href text = link ~href ~options:no_preload text
...
let modulepreload_hint href = Printf.sprintf {|<link rel="modulepreload" href="%s">|} href
...
let prefetch_hint href = Printf.sprintf {|<link rel="prefetch" href="%s">|} href

(** Generate prerender hint *)
let prerender_hint href = Printf.sprintf {|<link rel="prerender" href="%s">|} href
...
let route_hints ~js ~css ?(prefetch = []) () =
  { js_modules = js; css_files = css; prefetch }
;;
...
let options_to_json options =
  `Assoc
    ...
    ; ( "preloadType"
      , `String
          (match options.preload_type with
           | Code -> "code"
           | Data -> "data"
