...
let generate_load_type (entry : Manifest.route_entry) =
  ...
  in
  Printf.sprintf
    {|export type Load_%s = (event: LoadEvent<Route_%s['params']>) => MaybePromise<Record<string, unknown>>;|}
    route_name
    route_name
;;
...
let generate_load_types manifest =
  let loads =
    List.filter (fun (e : Manifest.route_entry) -> e.page) manifest.Manifest.routes
  in
...
let generate_router_file manifest =
  ...
  in
  Printf.sprintf
    {|// Auto-generated by Kirin Svelte SSR
...
let write_to_file ~path content = Kirin.Fs_compat.save path content
...
let generate_layout_type (layout : Manifest.layout_entry) =
  ...
  in
  Printf.sprintf
    {|export interface LayoutData_%s {
