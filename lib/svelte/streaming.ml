...
(** Create new stream *)
let create () =
  { chunks = []; completed = false; error = None; mutex = Eio.Mutex.create () }
;;

(** Add chunk to stream *)
let add_chunk stream chunk =
  Eio.Mutex.use_rw ~protect:true stream.mutex (fun () ->
    if not stream.completed then stream.chunks <- stream.chunks @ [ chunk ])
;;
...
(** Render all chunks *)
let render_chunks stream = List.map render_chunk stream.chunks |> String.concat ""
...
let hydration_marker ~id ~priority =
  Printf.sprintf
    {|<script>__svelte_hydrate('%s','%s')</script>|}
    id
...
let hydration_boundary ~id ~priority ~fallback content =
  Printf.sprintf
    {|<div data-svelte-hydrate="%s" data-priority="%s">%s</div>%s|}
    id
    (priority_string priority)
...
let placeholder ~id ~fallback =
  Printf.sprintf {|<template data-svelte-placeholder="%s">%s</template>|} id fallback
;;
...
let suspense ~id ~fallback =
  Printf.sprintf
    {|<div data-svelte-suspense="%s" style="display:contents">%s</div>|}
    id
    fallback
;;
...
let to_sse stream =
  List.map
    (fun chunk ->
       ...
       | Script content ->
         sse_event ~event:"script" ~data:(Data.serialize (`String content))
...
let error_boundary ~fallback content =
  Printf.sprintf
    {|<div data-svelte-error-boundary>
