...
let is_healthy worker = worker.status = Ready || worker.status = Busy
...
let age worker =
  if worker.last_request = 0.0 then 0.0 else Unix.gettimeofday () -. worker.last_request
;;
...
let render worker ~url ~props ?(route_id = None) ?(cookies = []) ?(headers = []) () =
  if not (is_healthy worker)
  then Error "Worker not healthy"
  else (
    mark_busy worker;
    ...
    let request = Protocol.{ url; props; route_id; cookies; headers } in
...
let health_check worker =
  if worker.status = Crashed then false else if needs_restart worker then false else true
;;
