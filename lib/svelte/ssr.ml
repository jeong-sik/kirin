...
let create_cache ~max_size ~ttl =
  { entries = Hashtbl.create max_size; max_size; ttl; mutex = Eio.Mutex.create () }
;;
...
let cache_get cache key =
  Eio.Mutex.use_rw ~protect:true cache.mutex (fun () ->
    ...
    | Some entry when entry.expires_at > Unix.gettimeofday () -> Some entry.response
    | Some _ ->
...
let cache_put cache key response =
  Eio.Mutex.use_rw ~protect:true cache.mutex (fun () ->
    ...
    if Hashtbl.length cache.entries >= cache.max_size
    then (
      let to_remove = ref [] in
      let count = ref 0 in
      Hashtbl.iter
        (fun k _ ->
           if !count < cache.max_size / 2
           then (
    ...
    let entry =
      { response; expires_at = Unix.gettimeofday () +. float_of_int cache.ttl }
    in
...
let create config =
  let workers =
    Array.init config.num_workers (fun _ ->
      Worker.create
...
let render engine ~url ?(props = `Assoc []) ?route_id ?(use_cache = true) () =
  ...
  let cached = if use_cache then cache_get engine.cache key else None in
  match cached with
  | Some response ->
...
let maintain_workers engine =
  Array.iter
    (fun worker ->
       if Worker.needs_restart worker
       then (
...
let ready_workers engine =
  Array.fold_left
    (fun acc worker -> if Worker.is_healthy worker then acc + 1 else acc)
    0
    engine.workers
;;
...
let get_stats (engine : t) =
  ...
  let hit_rate = if total = 0 then 0.0 else float_of_int hits /. float_of_int total in
  { total_renders = total
  ; cache_hits = hits
