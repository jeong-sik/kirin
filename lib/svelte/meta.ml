...
let render_tag = function
  | Title t -> Printf.sprintf "<title>%s</title>" (Kirin.html_escape t)
  | Description d ->
    Printf.sprintf {|<meta name="description" content="%s">|} (Kirin.html_escape d)
  | Charset c -> Printf.sprintf {|<meta charset="%s">|} c
  | Viewport v -> Printf.sprintf {|<meta name="viewport" content="%s">|} v
  | OgTitle t ->
  ...
  | OgImage url -> Printf.sprintf {|<meta property="og:image" content="%s">|} url
  | OgUrl url -> Printf.sprintf {|<meta property="og:url" content="%s">|} url
  | OgType t -> Printf.sprintf {|<meta property="og:type" content="%s">|} t
  | OgSiteName name ->
    Printf.sprintf
      {|<meta property="og:site_name" content="%s">|}
      (Kirin.html_escape name)
  | TwitterCard c -> Printf.sprintf {|<meta name="twitter:card" content="%s">|} c
  | TwitterTitle t ->
  ...
  | TwitterDescription d ->
    Printf.sprintf
      ...
      (Kirin.html_escape d)
  | TwitterImage url -> Printf.sprintf {|<meta name="twitter:image" content="%s">|} url
  | TwitterSite handle ->
    Printf.sprintf {|<meta name="twitter:site" content="%s">|} handle
  | Canonical url -> Printf.sprintf {|<link rel="canonical" href="%s">|} url
  | Robots r -> Printf.sprintf {|<meta name="robots" content="%s">|} r
  | ThemeColor color -> Printf.sprintf {|<meta name="theme-color" content="%s">|} color
  | Custom (name, content) ->
...
let render_tags tags = List.map render_tag tags |> String.concat "\n"
...
(** Add title *)
let title t b =
  b.tags <- Title t :: b.tags;
  b
;;

(** Add description *)
let description d b =
  b.tags <- Description d :: b.tags;
  b
;;

(** Add charset *)
let charset c b =
  b.tags <- Charset c :: b.tags;
  b
;;

(** Add viewport *)
let viewport v b =
  b.tags <- Viewport v :: b.tags;
  b
;;

(** Add theme color *)
let theme_color color b =
  b.tags <- ThemeColor color :: b.tags;
  b
;;
...
let og ~title ?description ?image ?url ?site_name ?(og_type = "website") () b =
  ...
  (match description with
   | Some d -> b.tags <- OgDescription d :: b.tags
   | None -> ());
  (match image with
   | Some i -> b.tags <- OgImage i :: b.tags
   | None -> ());
  (match url with
   | Some u -> b.tags <- OgUrl u :: b.tags
   | None -> ());
  (match site_name with
   | Some s -> b.tags <- OgSiteName s :: b.tags
   | None -> ());
...
let twitter ~card ~title ?description ?image ?site () b =
  ...
  (match description with
   | Some d -> b.tags <- TwitterDescription d :: b.tags
   | None -> ());
  (match image with
   | Some i -> b.tags <- TwitterImage i :: b.tags
   | None -> ());
  (match site with
   | Some s -> b.tags <- TwitterSite s :: b.tags
   | None -> ());
  b
;;
...
(** Add canonical URL *)
let canonical url b =
  b.tags <- Canonical url :: b.tags;
  b
;;

(** Add robots directive *)
let robots r b =
  b.tags <- Robots r :: b.tags;
  b
;;

(** Add custom meta tag *)
let custom ~name ~content b =
  b.tags <- Custom (name, content) :: b.tags;
  b
;;

(** Add property meta tag *)
let property ~prop ~content b =
  b.tags <- Property (prop, content) :: b.tags;
  b
;;
...
let defaults
      ?(charset_value = "utf-8")
      ?(viewport_value = "width=device-width, initial-scale=1")
      ()
  =
  let b = create () in
  ...
  | Some d ->
    ...
    let b =
      twitter
        ...
        b
    in
    (match canonical_url with
     | Some u -> canonical u b
     | None -> b)
  | None ->
...
let svelte_head_from_builder b = svelte_head (List.rev b.tags)
...
let tag_to_json = function
  ...
  | TwitterDescription d ->
    `Assoc [ "type", `String "twitter:description"; "value", `String d ]
  ...
  | Custom (name, content) ->
    `Assoc [ "type", `String "custom"; "name", `String name; "value", `String content ]
  | Property (prop, content) ->
    `Assoc
      [ "type", `String "property"; "property", `String prop; "value", `String content ]
  | HttpEquiv (equiv, content) ->
    `Assoc
      [ "type", `String "http-equiv"; "equiv", `String equiv; "value", `String content ]
;;

(** Builder to JSON *)
let to_json b = `List (List.rev_map tag_to_json b.tags)
