...
val route_data :
  route_id:string -> ?uses:string list -> Yojson.Safe.t -> route_data
type page_data = {
  url : string;
...
val page_data :
  ...
  ?status:int ->
  ?error:string ->
  ?data:Yojson.Safe.t -> ?form:Yojson.Safe.t -> unit -> page_data
val page_data_to_json :
  page_data -> [> `Assoc of (string * Yojson.Safe.t) list ]
type initial_data = {
  type_ : string;
...
val create_initial_data :
  ?nodes:Yojson.Safe.t list -> ?page_data:page_data -> unit -> initial_data
...
type deferred_chunk = { chunk_id : string; data : Yojson.Safe.t; }
val deferred_script : deferred_chunk -> string
type action_data = { success : bool; action_data : Yojson.Safe.t; }
val action_data : ?success:bool -> Yojson.Safe.t -> action_data
val action_data_to_json :
  action_data -> [> `Assoc of (string * Yojson.Safe.t) list ]
type snapshot = { scroll_y : int; snapshot_data : Yojson.Safe.t option; }
val snapshot_to_json :
  snapshot -> [> `Assoc of (string * Yojson.Safe.t) list ]
