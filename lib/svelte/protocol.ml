...
let next_id () = Atomic.fetch_and_add request_id 1 + 1
...
let decode_render_response json =
  ...
  then (
    ...
    let status =
      result |> member "status" |> to_int_option |> Option.value ~default:200
    in
...
let encode_health_request () = encode_request "health" (`Assoc [])
