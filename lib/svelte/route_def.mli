type param_type =
  ...
  | Rest
  | Matcher of string
type param = { name : string; param_type : param_type; required : bool; }
type load_result =
...
type layout = { layout_id : string; loader : loader option; reset : bool; }
type page = {
...
val page :
  ?prerender:bool option ->
  ?ssr:bool ->
  ?csr:bool -> ?loader:loader -> ?actions:action list -> string -> t
...
val server_loader :
  ?depends:string list ->
  (url:string -> params:(string * string) list -> load_result) -> loader
val universal_loader :
  ?depends:string list ->
  (url:string -> params:(string * string) list -> load_result) -> loader
val default_action :
  (url:string -> form_data:Yojson.Safe.t -> action_result) -> action
val named_action :
  string ->
  (url:string -> form_data:Yojson.Safe.t -> action_result) -> action
...
val param_type_to_json :
  param_type -> ([> `Assoc of (string * 'a) list | `String of string ] as 'a)
val to_json :
  t ->
  [> `Assoc of
       (string *
