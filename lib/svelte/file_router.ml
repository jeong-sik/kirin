...
let parse_segment raw =
  ...
  if len = 0
  then { raw; segment_type = Static ""; in_url = true }
  ...
  else if len >= 5 && raw.[0] = '[' && raw.[1] = '.' && raw.[2] = '.' && raw.[3] = '.'
  then (
    (* Rest param: [...rest] *)
  ...
  then (
    ...
    | None -> { raw; segment_type = Param inner; in_url = true })
  else { raw; segment_type = Static raw; in_url = true }
;;
...
let discover_route dir_path files =
  ...
  if file_types = []
  then None
  else Some { dir_path; segments; pattern; regex; files = file_types; group }
;;
...
let extract_params route url =
  ...
  then (
    ...
    let rec extract idx acc = function
      ...
      | seg :: rest ->
        let value =
          try Str.matched_group (idx + 1) url with
...
let segment_to_json seg =
  let type_json =
    ...
    | Matcher (name, m) ->
      `Assoc [ "matcher", `Assoc [ "name", `String name; "type", `String m ] ]
    | Group name -> `Assoc [ "group", `String name ]
  in
  `Assoc [ "raw", `String seg.raw; "type", type_json; "inUrl", `Bool seg.in_url ]
;;
