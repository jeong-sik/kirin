(** Svelte Hydration
    ...
    Server-side HTML shell for SvelteKit hydration. *)
...
let style_tag href = Printf.sprintf {|<link rel="stylesheet" href="%s">|} href

(** Render HTML shell *)
let render ?(ssr_html = "") options =
  let nonce = options.nonce in
  let scripts_html =
    String.concat "\n    " (List.map (script_tag ?nonce) options.scripts)
  in
  let styles_html = String.concat "\n    " (List.map style_tag options.styles) in
  let data_scripts = Data.all_scripts ~nonce options.initial_data in
  let body_attrs =
    let base = if options.body_attrs = "" then "" else " " ^ options.body_attrs in
    match options.body_class with
    | Some c -> Printf.sprintf {| class="%s"%s|} c base
    | None -> base
  in
  Printf.sprintf
    {|<!DOCTYPE html>
  ...
  let scripts, styles =
    ...
    | _ ->
      ...
      (match List.assoc_opt entry manifest with
       ...
       | None -> [ entry ], [])
  in
  let options = { default_options with title; scripts; styles } in
  render options
;;

(** {1 SvelteKit App Shell} *)

(** SvelteKit-style app shell *)
let sveltekit_shell
      ~title
      ~entry_script
      ~css_files
  ...
  let options =
    ...
    ; styles = css_files
    ; initial_data = Option.value ~default:Data.empty_initial_data initial_data
    ; preload_hints = Option.value ~default:"" preload_hints
    ; nonce
    ; body_class
    }
  in
...
(** Generate error page *)
let error_page ~title ~status ~message () =
  Printf.sprintf
    {|<!DOCTYPE html>
<html>
<head>
...
let streaming_placeholder ~id ~fallback =
  Printf.sprintf {|<template id="svelte-stream-%s">%s</template>|} id fallback
;;
...
let streaming_replacement ~id content =
  Printf.sprintf
    {|<script>
(function(){
  var t=document.getElementById("svelte-stream-%s");
  if(t){var r=document.createRange();r.setStartAfter(t);r.insertNode(r.createContextualFragment(%s));t.remove();}
})();
</script>|}
    id
    (Printf.sprintf "'%s'" (Data.serialize (`String content)))
;;

(** {1 Partial Hydration} *)

(** Generate island wrapper for partial hydration *)
let island ~component_name ~props_json content =
  Printf.sprintf
    {|<svelte-island data-component="%s" data-props='%s'>%s</svelte-island>|}
    component_name
    (Data.serialize props_json)
    content
;;

(** Generate lazy island *)
let lazy_island ~component_path ~props_json =
  Printf.sprintf
    {|<svelte-island data-import="%s" data-props='%s'></svelte-island>|}
    component_path
    (Data.serialize props_json)
;;
