...
let sveltekit_shell ~title ~entry_script ~css_files ?preload_hints ?ssr_html
    ?initial_data ?nonce ?body_class () =
  let options = {
    default_options with
    title;
    scripts = [entry_script];
...
let island ~component_name ~props_json content =
  Printf.sprintf {|<svelte-island data-component="%s" data-props='%s'>%s</svelte-island>|}
    component_name
...
let lazy_island ~component_path ~props_json =
  Printf.sprintf {|<svelte-island data-import="%s" data-props='%s'></svelte-island>|}
    component_path
    (Data.serialize props_json)
