...
let page ?(prerender = None) ?(ssr = true) ?(csr = true) ?loader ?(actions = []) path =
  let page = { loader; actions; prerender; ssr; csr } in
  { path
  ; pattern = path
  ; params = []
  ; page = Some page
...
let layout ?(reset = false) ?loader path =
  let layout = { layout_id = path; loader; reset } in
  { path
  ; pattern = path
  ; params = []
  ; page = None
...
(** Set route group *)
let in_group group route = { route with group = Some group }

(** {1 Load Function Helpers} *)

(** Create server-side loader *)
let server_loader ?(depends = []) handler = { load_server = true; depends; handler }

(** Create universal loader *)
let universal_loader ?(depends = []) handler = { load_server = false; depends; handler }

(** {1 Action Helpers} *)

(** Create default action *)
let default_action handler = { action_name = None; handler }

(** Create named action *)
let named_action name handler = { action_name = Some name; handler }
...
let to_json route =
  ...
  in
  let with_group =
    match route.group with
    | Some g -> ("group", `String g) :: base
    | None -> base
  in
  let with_page =
    match route.page with
    | Some p ->
      let page_json =
        `Assoc
