...
(** Empty manifest *)
let empty = { routes = []; layouts = []; matchers = []; hooks = [] }

(** Add route to manifest *)
let add_route entry manifest = { manifest with routes = entry :: manifest.routes }

(** Add layout to manifest *)
let add_layout entry manifest = { manifest with layouts = entry :: manifest.layouts }

(** Add matcher to manifest *)
let add_matcher name manifest = { manifest with matchers = name :: manifest.matchers }

(** Add hook to manifest *)
let add_hook name manifest = { manifest with hooks = name :: manifest.hooks }
...
let route
      ...
      ()
  =
  { id; pattern; params; page; layout; error; endpoint }
;;

(** Create layout entry *)
let layout ~id ?parent ?(reset = false) () = { layout_id = id; parent; reset }
...
let from_discovered_routes discovered =
  let routes =
    List.filter_map
      (fun (r : File_router.discovered_route) ->
         let has_page =
           List.exists
             (fun f ->
                match f with
         ...
         let has_endpoint =
           List.exists
             (fun f ->
                match f with
                | File_router.Server -> true
                | _ -> false)
             r.files
         in
         if (not has_page) && not has_endpoint
         then None
         else (
           let params =
             List.filter_map
               (fun (seg : File_router.segment) ->
      ...
      (fun (r : File_router.discovered_route) ->
         let has_layout =
           List.exists
             (fun f ->
                match f with
                | File_router.Layout | File_router.LayoutServer | File_router.LayoutTs ->
                  true
                | _ -> false)
             r.files
         in
         ...
         then None
           ...
           Some
             ...
             ; parent = None
             ; (* Would need hierarchy resolution *)
               reset = false
             })
      discovered
  in
...
let route_to_json entry =
  let base =
    ...
    ; ( "params"
      , `List
          (List.map
             (fun (name, typ) -> `Assoc [ "name", `String name; "type", `String typ ])
             entry.params) )
...
let layout_to_json entry =
  let base = [ "id", `String entry.layout_id; "reset", `Bool entry.reset ] in
  let with_parent =
