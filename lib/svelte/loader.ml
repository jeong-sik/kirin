...
(** Cache for duration *)
let cache_for seconds = { default_cache with max_age = Some seconds }

(** CDN cache for duration *)
let cdn_cache_for seconds = { default_cache with s_maxage = Some seconds }

(** No caching *)
let no_cache = { default_cache with no_store = true }

(** Cache control to header value *)
let cache_header cache =
  let parts = [] in
  let parts =
    match cache.max_age with
    | Some s -> Printf.sprintf "max-age=%d" s :: parts
    | None -> parts
  in
  let parts =
    match cache.s_maxage with
    | Some s -> Printf.sprintf "s-maxage=%d" s :: parts
    | None -> parts
  in
  let parts =
    match cache.stale_while_revalidate with
...
let streaming initial_data updates =
  ...
  List.iter (fun (key, data) -> results := Update (key, data) :: !results) updates;
...
let output_to_json = function
  | LoadData data -> `Assoc [ "type", `String "data"; "data", data ]
  ...
  | LoadNotFound -> `Assoc [ "type", `String "notFound" ]
;;
