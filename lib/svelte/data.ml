...
let script_tag ?(nonce = None) json =
  ...
  match nonce with
  | Some n -> Printf.sprintf {|<script nonce="%s">%s=%s</script>|} n data_var data
  | None -> Printf.sprintf {|<script>%s=%s</script>|} data_var data
;;
...
(** Create route data *)
let route_data ~route_id ?(uses = []) data = { route_id; data; uses }
...
let page_data ~url ~params ?route_id ?(status = 200) ?error ?(data = `Assoc []) ?form () =
  { url; params; route_id; status; error; data; form }
;;
...
(** Empty initial data *)
let empty_initial_data = { type_ = "data"; nodes = []; page_data = None }

(** Create initial data *)
let create_initial_data ?(nodes = []) ?page_data () = { type_ = "data"; nodes; page_data }
...
let initial_data_to_json id =
  let base = [ "type", `String id.type_; "nodes", `List id.nodes ] in
...
(** Create action data *)
let action_data ?(success = true) data = { success; action_data = data }

(** Action data to JSON *)
let action_data_to_json ad =
  `Assoc [ "success", `Bool ad.success; "data", ad.action_data ]
;;
