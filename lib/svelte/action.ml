...
let min_length len form name =
  ...
  | Some v ->
    ...
    then Valid v
      ...
      Invalid
        [ validation_error name (Printf.sprintf "Must be at least %d characters" len) ]
;;
    ...
    Valid
      (List.filter_map
         (function
           ...
           | _ -> None)
         validations)
  else Invalid (List.concat errors)
;;

(** Validation errors to JSON *)
let errors_to_json errors = `Assoc (List.map (fun e -> e.field, `String e.message) errors)

(** Fail with validation errors *)
let fail_validation errors = ActionFail (400, `Assoc [ "errors", errors_to_json errors ])
...
let default handler = { name = None; handler }

(** Create named action *)
let named name handler = { name = Some name; handler }
...
let create_context ~url ?(headers = []) ?(cookies = []) ?(locals = `Assoc []) ~route_id ()
  =
  { url; request_headers = headers; cookies; locals; route_id }
;;

(** Get cookie from context *)
let cookie ctx name = List.assoc_opt name ctx.cookies

(** Get header from context *)
let header ctx name =
  let name_lower = String.lowercase_ascii name in
  List.find_map
    (fun (k, v) -> if String.lowercase_ascii k = name_lower then Some v else None)
    ctx.request_headers
;;
...
let output_to_json = function
  | ActionSuccess data -> `Assoc [ "type", `String "success"; "data", data ]
  | ActionFail (status, data) ->
    `Assoc [ "type", `String "fail"; "status", `Int status; "data", data ]
  | ActionRedirect (status, location) ->
    `Assoc
      [ "type", `String "redirect"; "status", `Int status; "location", `String location ]
...
let form_to_json form =
  let rec value_to_json = function
    | Text s -> `String s
    | File f ->
      `Assoc [ "filename", `String f.filename; "contentType", `String f.content_type ]
