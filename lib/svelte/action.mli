...
type file_data = { filename : string; content_type : string; data : string; }
type field_value =
    Text of string
  | File of file_data
...
type 'a validation_result = Valid of 'a | Invalid of validation_error list
val validation_error : string -> string -> validation_error
val required :
  (string * field_value) list -> string -> string validation_result
val email : (string * field_value) list -> string -> string validation_result
val min_length :
  int -> (string * field_value) list -> string -> string validation_result
val combine_validations :
  'a validation_result list -> 'a list validation_result
val errors_to_json :
...
type named_action = { name : string option; handler : action_handler; }
...
val create_context :
  ...
  ?cookies:(string * string) list ->
  ?locals:Yojson.Safe.t -> route_id:string -> unit -> action_context
val cookie : action_context -> string -> string option
val header : action_context -> string -> string option
val output_to_json :
  action_output -> [> `Assoc of (string * Yojson.Safe.t) list ]
val form_to_json :
  ('a * field_value) list ->
  [> `Assoc of
       ('a *
