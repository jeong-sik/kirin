...
type hydration_priority = Immediate | Visible | Idle | Interaction
...
val hydration_boundary :
  id:string ->
  priority:hydration_priority -> fallback:string option -> string -> string
...
val streaming_response :
  stream_state -> title:string -> entry_script:string -> string
val sse_headers : (string * string) list
val error_boundary : fallback:string -> string -> string
val error_recovery_script : string
val chunk_to_json :
  chunk -> [> `Assoc of (string * [> `String of string ]) list ]
val stream_to_json :
  stream_state ->
  [> `Assoc of
       (string *
