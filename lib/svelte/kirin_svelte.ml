...
let handler ~bundle ?num_workers ?cache_ttl () =
  ...
  let options =
    Handler.{ default_options with ssr_engine = Some engine; dev_mode = false }
  in
...
let dev_handler ?(vite_port = 5173) () =
  let options =
    Handler.{ default_options with ssr_engine = None; dev_mode = true; vite_port }
  in
  Handler.routes ~options
;;
...
let load_manifest path = Manifest.load_file path
...
let scan_routes _dir =
  ...
  let discovered =
    List.filter_map (fun (path, fs) -> File_router.discover_route path fs) files
  in
...
let page ~path ~loader = Route_def.page path ~loader:(Route_def.server_loader loader)
...
let page_with_actions ~path ~loader ~actions =
  let actions =
    List.map
      (fun (name, handler) ->
         if name = "default"
         then Route_def.default_action handler
         else Route_def.named_action name handler)
      actions
  in
  Route_def.page path ~loader:(Route_def.server_loader loader) ~actions
;;
...
let meta ~title ?description ?image ?canonical () =
  Meta.page ~title ?description ?og_image:image ?canonical_url:canonical ()
;;
