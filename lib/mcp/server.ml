...
let add_tool_sync t ~name ~description ~schema ?annotations ?icon ~handler () =
  add_tool t ~name ~description ~schema ?annotations ?icon ~handler:(Sync handler) ()
;;

(** List all tools *)
let list_tools t = List.map (fun rt -> rt.tool) t.tools
...
let call_tool t ~ctx ~name ~arguments =
  ...
  | None -> Error (Printf.sprintf "Tool not found: %s" name)
;;
...
let add_resource t ~uri ~name ?description ?mime_type ?icon ~handler () =
  ...
  let resource = { Protocol.uri; name; description; mime_type; icon } in
  t.resources <- { resource; handler } :: t.resources
;;

(** List all resources *)
let list_resources t = List.map (fun rr -> rr.resource) t.resources
...
let read_resource t ~uri =
  ...
  | Some rr ->
    ...
    Ok { Protocol.uri; mime_type = Some mime_type; text = Some text; blob = None }
  | None -> Error (Printf.sprintf "Resource not found: %s" uri)
;;
...
let add_prompt t ~name ?description ?arguments ?icon ?handler () =
  ...
  let prompt = { Protocol.name; description; arguments; icon } in
  t.prompts <- { prompt; handler } :: t.prompts
;;

(** List all prompts *)
let list_prompts t = List.map (fun rp -> rp.prompt) t.prompts
    ...
    (match req.params with
     | Some params ->
       ...
       (match Session.handle_initialize t.session init_params with
        | Ok result ->
          Jsonrpc.success_response ~id:req.id (Protocol.initialize_result_to_json result)
        | Error msg ->
     ...
     | None ->
       Jsonrpc.error_response
         ...
         (Jsonrpc.make_error
            ...
            ()))
  | m when m = ping -> Jsonrpc.success_response ~id:req.id `Null
  | m when m = tools_list ->
       ...
       (match get_prompt t ~name ~arguments with
        | Ok messages ->
          Jsonrpc.success_response
            ~id:req.id
            (`Assoc
                [ "messages", `List (List.map Protocol.prompt_message_to_json messages) ])
        | Error msg ->
          Jsonrpc.error_response
            ~id:req.id
            (Jsonrpc.make_error ~code:Jsonrpc.Invalid_params ~message:msg ()))
    ...
    (match req.params with
     | Some params ->
       ...
       (match Tasks.get_task t.task_registry ~id with
        | Some task ->
          Jsonrpc.success_response ~id:req.id (`Assoc [ "task", Tasks.task_to_json task ])
        | None ->
          Jsonrpc.error_response
            ~id:req.id
            (Jsonrpc.make_error
     ...
     | None ->
       Jsonrpc.error_response
         ~id:req.id
         (Jsonrpc.make_error ~code:Jsonrpc.Invalid_params ~message:"Missing task ID" ()))
  | m when m = tasks_list ->
    let tasks_json =
      `List (List.map Tasks.task_to_json (Tasks.list_tasks t.task_registry))
    in
    ...
    (match req.params with
     | Some params ->
       ...
       (match Tasks.cancel_task t.task_registry ~id with
        | Ok () -> Jsonrpc.success_response ~id:req.id (`Assoc [])
        | Error msg ->
          Jsonrpc.error_response
            ~id:req.id
            (Jsonrpc.make_error ~code:Jsonrpc.Invalid_params ~message:msg ()))
...
let handle_message t ~ctx (msg : Jsonrpc.message) : Jsonrpc.message option =
  match msg with
  | Jsonrpc.Request req -> Some (Jsonrpc.Response (handle_request t ~ctx req))
...
let run t ~ctx transport =
  let rec loop () =
    ...
    (match handle_message t ~ctx msg with
     ...
     | None -> ());
    if Session.is_ready t.session || Session.state t.session = Session.Initializing
    then loop ()
  in
  loop ()
;;
