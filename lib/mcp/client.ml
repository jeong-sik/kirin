...
let create transport =
  { transport; server_capabilities = None; server_info = None; request_id = 0 }
;;
  ...
  | Some err ->
    raise
      (Client_error
         (Printf.sprintf
            "RPC error %d: %s"
            (Jsonrpc.error_code_to_int err.code)
            err.message))
...
let initialize t ?(client_name = "kirin-client") ?(client_version = "1.0.0") () =
  let params =
    `Assoc
      ...
      ; ( "capabilities"
        , Protocol.client_capabilities_to_json
            { Protocol.roots = None; sampling = None; experimental = None } )
      ; ( "clientInfo"
        , Protocol.implementation_info_to_json
            { Protocol.name = client_name; version = client_version; description = None }
        )
      ]
  in
  ...
  | Some result ->
    ...
    <- Some
         ...
         ; description =
             (match result |> member "serverInfo" |> member "description" with
              | `String s -> Some s
              | _ -> None)
         };
    ...
    <- Some
         { Protocol.tools =
             (match caps_json |> member "tools" with
              | `Null -> None
              | _ -> Some { Protocol.list_changed = None })
         ; resources =
             (match caps_json |> member "resources" with
              | `Null -> None
              | _ -> Some { Protocol.subscribe = None; list_changed = None })
         ; prompts =
             (match caps_json |> member "prompts" with
              | `Null -> None
              | _ -> Some { Protocol.list_changed = None })
         ; logging =
             (match caps_json |> member "logging" with
              | `Null -> None
              | _ -> Some true)
         };
    (* Send initialized notification *)
    send_notification t ~method_:Protocol.Method.initialized ();
    (* For HTTP transport, propagate session ID from protocol version negotiation *)
    (match t.transport with
     | Transport.Streamable_http _ ->
       ...
       if Transport.session_id t.transport = None
       then (
         Random.self_init ();
         let id =
           Printf.sprintf
             "client-%08x%08x%08x%08x"
             (Random.bits ())
             (Random.bits ())
             (Random.bits ())
             (Random.bits ())
     ...
     | _ -> ());
    t.server_capabilities
  | None -> raise (Client_error "No result from initialize")
;;
...
let call_tool t ~name ?(arguments = `Null) () =
  ...
  | None -> raise (Client_error "No result from tool call")
;;
  ...
  | Some result ->
    ...
    (match contents with
     | first :: _ ->
       ...
       ; mime_type =
           (match first |> member "mimeType" with
            | `String s -> Some s
            | _ -> None)
       ; text =
           (match first |> member "text" with
            ...
            | _ -> None)
       ; blob =
           (match first |> member "blob" with
            | `String s -> Some s
            | _ -> None)
       }
     | [] -> raise (Client_error "Empty resource contents"))
  | None -> raise (Client_error "No result from resource read")
;;
             ...
             Some
               (List.map
                  (fun arg ->
                     ...
                     ; description =
                         (match arg |> member "description" with
                          | `String s -> Some s
                          | _ -> None)
                     ; required =
