...
let check_tool_permission config ~tool_name =
  if List.mem tool_name config.denied_tools
  then Denied (Printf.sprintf "tool %S is in the deny list" tool_name)
  else (
    match config.allowed_tools with
    | Some allowed ->
      if List.mem tool_name allowed
      then Allowed
      else (
...
let record_event log event =
  Eio.Mutex.use_rw ~protect:true log.mutex (fun () ->
    if Queue.length log.events >= log.max_events then ignore (Queue.pop log.events);
...
let audit_log_to_json log =
  Eio.Mutex.use_ro log.mutex (fun () ->
    let events = Queue.to_seq log.events |> Seq.map audit_event_to_json |> List.of_seq in
    `Assoc
...
let config_of_json json =
  ...
  in
  let denied_tools = json |> member "denied_tools" |> to_list |> List.map to_string in
  { level
  ; allowed_tools
  ; denied_tools
