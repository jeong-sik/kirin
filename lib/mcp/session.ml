...
let create ~server_name ~server_version () =
  ...
  ; server_info =
      { Protocol.name = server_name; version = server_version; description = None }
  ; server_capabilities =
      { Protocol.tools = None; resources = None; prompts = None; logging = None }
  ; request_id = 0
  ; session_id = None
  }
;;
...
let enable_tools t =
  ...
  <- { t.server_capabilities with tools = Some { Protocol.list_changed = None } }
;;

(** Enable resources capability *)
let enable_resources t =
  t.server_capabilities
  <- { t.server_capabilities with
...
let enable_prompts t =
  t.server_capabilities
  <- { t.server_capabilities with prompts = Some { Protocol.list_changed = None } }
;;
...
let handle_initialized t =
  ...
  | Initializing ->
    t.state <- Ready;
    Ok ()
  | _ -> Error "Unexpected initialized notification"
;;
...
(** Begin shutdown *)
let begin_shutdown t = t.state <- Closing

(** Complete shutdown *)
let complete_shutdown t = t.state <- Closed
