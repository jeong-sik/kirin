...
let create ?(level = Warning) ?handler () = { current_level = level; handler }

(* {1 Level Management} *)

let set_level t level = t.current_level <- level
let current_level t = t.current_level
let should_log t level = level_to_int level >= level_to_int t.current_level

(* {1 Log Emission} *)

let log t ~level ?logger data =
  if should_log t level
  then (
...
let log_message_to_json msg =
  let fields = [ "level", `String (log_level_to_string msg.level); "data", msg.data ] in
  let fields =
    match msg.logger with
