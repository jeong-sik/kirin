...
val send_request :
  t ->
  method_:string -> ?params:Yojson.Safe.t -> unit -> Yojson.Safe.t option
val send_notification :
  t -> method_:string -> ?params:Yojson.Safe.t -> unit -> unit
val initialize :
  t ->
  ?client_name:string ->
  ?client_version:string ->
  unit -> Protocol.server_capabilities option
val list_tools : t -> Protocol.tool list
val call_tool :
  t ->
  name:string ->
  ?arguments:Yojson.Safe.t -> unit -> Protocol.tool_result
...
val server_capabilities :
  t -> Protocol.server_capabilities option
