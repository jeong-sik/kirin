...
let error_to_json err =
  let base =
    [ "code", `Int (error_code_to_int err.code); "message", `String err.message ]
  in
  let with_data =
    match err.data with
    | Some d -> base @ [ "data", d ]
...
let encode_request (req : request) =
  let base =
    [ "jsonrpc", `String "2.0"; "id", id_to_json req.id; "method", `String req.method_ ]
  in
  let with_params =
...
let encode_response (resp : response) =
  let base = [ "jsonrpc", `String "2.0"; "id", id_to_json resp.id ] in
  let with_result =
    match resp.result, resp.error with
    | Some r, None -> base @ [ "result", r ]
    | None, Some e -> base @ [ "error", error_to_json e ]
    | Some _, Some e ->
      base @ [ "error", error_to_json e ] (* Error takes precedence per JSON-RPC 2.0 *)
...
let encode_notification (notif : notification) =
  let base = [ "jsonrpc", `String "2.0"; "method", `String notif.method_ ] in
  let with_params =
...
let make_request ~id ~method_ ?params () = { id; method_; params }
let make_response ~id ?result ?error () = { id; result; error }
let make_notification ~method_ ?params () = { method_; params }
let make_error ~code ~message ?data () = { code; message; data }
let success_response ~id result = { id; result = Some result; error = None }
let error_response ~id err = { id; result = None; error = Some err }
