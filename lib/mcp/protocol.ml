...
type icon = { uri : string (** data: URI or https: URI *) }
...
type tool_annotations =
  ...
  ; destructive_hint : bool option
    (** Hint: tool may have destructive side-effects (default true) *)
  ; idempotent_hint : bool option
    (** Hint: calling repeatedly has same effect (default false) *)
  ; open_world_hint : bool option
    (** Hint: tool interacts with external entities (default true) *)
  }
...
let implementation_info_to_json (info : implementation_info) =
  let base = [ "name", `String info.name; "version", `String info.version ] in
  let fields =
...
let icon_to_json (i : icon) = `Assoc [ "uri", `String i.uri ]
...
let tool_annotations_to_json (a : tool_annotations) =
  ...
  in
  let fields =
    match a.read_only_hint with
    | Some b -> ("readOnlyHint", `Bool b) :: fields
    | None -> fields
  in
  let fields =
    match a.destructive_hint with
    | Some b -> ("destructiveHint", `Bool b) :: fields
    | None -> fields
  in
  let fields =
    match a.idempotent_hint with
    | Some b -> ("idempotentHint", `Bool b) :: fields
    | None -> fields
  in
  let fields =
    match a.open_world_hint with
...
let tool_to_json (t : tool) =
  let base = [ "name", `String t.name; "inputSchema", t.input_schema ] in
  let fields =
...
let content_to_json = function
  ...
  | Image { data; mime_type } ->
    `Assoc
      [ "type", `String "image"; "data", `String data; "mimeType", `String mime_type ]
...
let resource_to_json (res : resource) =
  let base = [ "uri", `String res.uri; "name", `String res.name ] in
  let fields =
...
let prompt_message_to_json (pm : prompt_message) =
  let role_str =
    ...
    | `assistant -> "assistant"
  in
  `Assoc [ "role", `String role_str; "content", content_to_json pm.content ]
;;
...
let tool_annotations_of_json json =
  ...
  { title =
      (match json |> member "title" with
       ...
       | _ -> None)
  ; read_only_hint =
      (match json |> member "readOnlyHint" with
       | `Bool b -> Some b
       | _ -> None)
  ; destructive_hint =
      (match json |> member "destructiveHint" with
       | `Bool b -> Some b
       | _ -> None)
  ; idempotent_hint =
      (match json |> member "idempotentHint" with
       | `Bool b -> Some b
       | _ -> None)
  ; open_world_hint =
      (match json |> member "openWorldHint" with
       | `Bool b -> Some b
       | _ -> None)
  }
;;
...
let roots_capability_of_json json =
  let open Yojson.Safe.Util in
  { list_changed =
      (match json |> member "listChanged" with
       | `Bool b -> Some b
       | _ -> None)
  }
;;
...
let client_capabilities_of_json json =
  let open Yojson.Safe.Util in
  { roots =
      (match json |> member "roots" with
       | `Null -> None
       | j -> Some (roots_capability_of_json j))
  ; sampling =
      (match json |> member "sampling" with
       | `Null -> None
       | _ -> Some ())
  ; experimental =
      (match json |> member "experimental" with
       | `Null -> None
       | j -> Some j)
  }
;;
...
let tool_call_of_json json =
  ...
  { name = json |> member "name" |> to_string
  ; arguments =
      (match json |> member "arguments" with
       | `Null -> None
       | args -> Some args)
  ; _meta =
      (match json |> member "_meta" with
...
let tool_of_json json =
  ...
  ; annotations =
      (match json |> member "annotations" with
       | `Null -> None
       | j -> Some (tool_annotations_of_json j))
  ; icon =
      (match json |> member "icon" with
       | `Null -> None
       | j -> Some (icon_of_json j))
  }
;;
