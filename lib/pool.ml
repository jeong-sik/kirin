...
let create
  ...
  =
  let config =
    { min_size; max_size; idle_timeout; max_wait_time; health_check_interval }
  in
  let stats =
    { total_connections = 0
...
let create_pooled pool =
  ...
  let time = now () in
  { conn; created_at = time; last_used = time; use_count = 0 }
;;
...
let is_valid pool pooled =
  ...
  | None -> true
  | Some validate ->
    (try validate pooled.conn with
     | Eio.Io _ | Failure _ -> false)
;;
...
(** Update stats *)
let update_stats pool f = pool.stats <- f pool.stats
...
let acquire pool =
    ...
    with_lock pool.mutex (fun () ->
      ...
      | pooled :: rest when is_valid pool pooled && not (is_expired pool pooled) ->
        ...
        update_stats pool (fun s ->
          { s with total_acquisitions = s.total_acquisitions + 1 });
        Some pooled
      | pooled :: rest ->
        ...
        update_stats pool (fun s ->
          { s with total_connections = s.total_connections - 1 });
        None
  ...
  let rec wait_for_connection () =
    ...
    if elapsed > pool.config.max_wait_time
    then (
...
let release pool pooled =
  with_lock pool.mutex (fun () ->
    ...
    if total < pool.config.max_size && is_valid pool pooled
    then pool.connections <- pooled :: pool.connections
    else (
...
let use pool f =
  ...
  Fun.protect ~finally:(fun () -> release pool pooled) (fun () -> f pooled.conn)
;;

(** Use a connection with the pooled wrapper (includes metadata) *)
let use_pooled pool f =
  let pooled = acquire pool in
  Fun.protect ~finally:(fun () -> release pool pooled) (fun () -> f pooled)
;;
...
let cleanup_idle pool =
  with_lock pool.mutex (fun () ->
    let keep, remove =
      List.partition (fun p -> not (is_expired pool p)) pool.connections
    in
...
let init pool = ignore (ensure_minimum pool)
...
let idle_count pool = Eio.Mutex.use_ro pool.mutex (fun () -> List.length pool.connections)

(** Get number of active connections *)
let active_count pool = Eio.Mutex.use_ro pool.mutex (fun () -> pool.in_use)
