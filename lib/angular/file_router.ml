...
let parse_segment s =
  ...
  then { raw = s; segment_type = Wildcard; param_name = Some "wildcard" }
  else if len > 1 && s.[0] = ':'
  then (
...
let detect_file_type name =
  if Filename.check_suffix name ".component.ts"
  then Some Component
  else if Filename.check_suffix name ".module.ts"
  then Some Module
  else if Filename.check_suffix name ".routes.ts"
  then Some Routes
  else if name = "app.routes.server.ts"
  then Some ServerRoutes
  else if Filename.check_suffix name ".guard.ts"
  then Some Guard
  else if Filename.check_suffix name ".resolver.ts"
  then Some Resolver
  else if Filename.check_suffix name ".service.ts"
  then Some Service
  else if Filename.check_suffix name ".interceptor.ts"
  then Some Interceptor
  else None
;;
  ...
  let route_parts =
    List.filter_map
      (fun part ->
         if part = "" || part = "app"
         then None
         else if String.length part > 0 && part.[0] = '['
         then (
           (* [id] -> :id *)
           let len = String.length part in
           if len > 2 && part.[len - 1] = ']'
           then Some (":" ^ String.sub part 1 (len - 2))
           else Some part)
         else Some part)
      parts
  in
...
let discover_route ~base_dir file_path =
  ...
  let file_type =
    match detect_file_type (Filename.basename file_path) with
    | Some ft -> ft
    | None -> Component
  in
...
let discover_routes app_dir = discover_routes_aux ~base_dir:app_dir ~prefix:"" ""
...
let discovered_to_json route =
  `Assoc
    ...
    ; ( "componentName"
      , match route.component_name with
        | Some n -> `String n
        | None -> `Null )
