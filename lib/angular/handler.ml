  ...
  || (String.length route.Route_def.path > 0
      ...
      && String.sub route.Route_def.path 0 (String.length route.Route_def.path - 1)
         = String.sub
             path
             0
             (min (String.length path) (String.length route.Route_def.path - 1)))
;;

(** Find matching route *)
let find_route routes path = List.find_opt (fun r -> route_matches r path) routes
  ...
  | None ->
    ...
    then (
      ...
      then (
        ...
        error_response
          ~status:500
          ~message:"SSR engine not configured and index.html not found")
