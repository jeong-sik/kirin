...
(** Create cache *)
let create_cache size = { entries = []; max_size = size }
...
let cache_set cache key html ?(ttl = 60.0) () =
  ...
  then cache.entries <- List.filteri (fun i _ -> i < cache.max_size) cache.entries
;;
