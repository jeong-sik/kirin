...
(** Empty head *)
let empty = { title = None; base_href = None; metas = []; links = []; scripts = [] }

(** Set title *)
let with_title title head = { head with title = Some title }

(** Set base href *)
let with_base href head = { head with base_href = Some href }
...
(** Add charset *)
let with_charset charset head = { head with metas = Charset charset :: head.metas }
...
let render_meta = function
  ...
  | Charset charset -> Printf.sprintf {|<meta charset="%s">|} charset
;;

(** Render link tag *)
let render_link link =
  let attrs =
    [ Printf.sprintf {|rel="%s"|} link.rel; Printf.sprintf {|href="%s"|} link.href ]
    @ (match link.hreflang with
       | Some h -> [ Printf.sprintf {|hreflang="%s"|} h ]
       | None -> [])
    @ (match link.type_ with
       | Some t -> [ Printf.sprintf {|type="%s"|} t ]
       | None -> [])
    @ (match link.sizes with
...
let render_script script =
  let attrs =
    ...
    @ (match script.src with
       | Some s -> [ Printf.sprintf {|src="%s"|} s ]
       | None -> [])
    @ (match script.type_ with
       | Some t -> [ Printf.sprintf {|type="%s"|} t ]
       | None -> [])
