...
let generate_server_routes (routes : Route_def.t list) =
  let route_configs =
    List.map
      (fun route ->
         let render_mode =
           match route.Route_def.render_mode with
  ...
  in
  Printf.sprintf
    {|// Auto-generated by Kirin Angular Codegen
...
let generate_component_type ~name ~inputs ~outputs =
  let input_fields =
    List.map
      (fun (prop_name, prop_type, required) ->
...
let generate_guard_type ~name =
  Printf.sprintf
    {|export const %sGuard: CanActivateFn = (
...
let generate_resolver_type ~name ~return_type =
  Printf.sprintf
    {|export const %sResolver: ResolveFn<%s> = (
  route: ActivatedRouteSnapshot,
  state: RouterStateSnapshot
) => {
...
let generate_type_guard ~name ~fields =
  let checks =
    List.map
      (fun (field, type_) -> Printf.sprintf "typeof obj.%s === '%s'" field type_)
      fields
  in
  Printf.sprintf
    {|export function is%s(obj: unknown): obj is %s {
  return (
    typeof obj === 'object' &&
    obj !== null &&
    %s
