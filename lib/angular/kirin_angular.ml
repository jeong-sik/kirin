...
let ssr_config ~dist_path ?(workers = 4) ?(timeout_s = 5.0) () =
  ...
  { Handler.default_config with mode = Handler.SSR; dist_path; ssr_engine = Some ssr }
;;
...
let prerender_config ~dist_path () =
  { Handler.default_config with mode = Handler.Prerender; dist_path }
;;

(** {1 Route Building} *)

(** Create SSR route *)
let ssr_route path component = Route_def.ssr path component

(** Create CSR route *)
let csr_route path component = Route_def.csr path component

(** Create SSG route *)
let ssg_route path component = Route_def.ssg path component

(** Create redirect route *)
let redirect ~from ~to_ () = Route_def.redirect ~from ~to_ ()

(** Create lazy-loaded route *)
let lazy_route path load_children = Route_def.lazy_route path load_children

(** {1 Transfer State} *)

(** Create transfer state *)
let create_transfer_state () = Transfer_state.create ()
...
let transfer_state_script state = Transfer_state.script_tag state
...
let skip_hydration tag content = Hydration.with_skip_hydration tag content
...
let head ~title () = Meta.empty |> Meta.with_title title |> Meta.with_charset "UTF-8"
...
let angular_handler config = fun request -> Handler.catch_all_handler config request
...
(** Render with SSR *)
let render_ssr engine ~url () = Ssr.render engine ~url ()
...
let prerender engine ~routes ~output_dir = Ssr.prerender engine ~routes ~output_dir

(** {1 Codegen} *)

(** Generate TypeScript types for routes *)
let generate_types ~routes = Codegen.generate_types_file ~routes

(** Generate server routes config *)
let generate_server_routes ~routes = Codegen.generate_server_routes routes

(** {1 File Router} *)

(** Discover routes from app directory *)
let discover_routes ~app_dir () = File_router.discover_routes app_dir
