...
let health_request ?(include_memory = true) () = { include_memory }

(** Create prerender request *)
let prerender_request ~routes ~output_dir = { routes; output_dir }
...
let encode_health ~id req =
  let params = `Assoc [ "includeMemory", `Bool req.include_memory ] in
...
let decode_render_result json =
  ...
  ; transfer_state =
      (match json |> member "transferState" with
       | `Null -> None
       | ts -> Some ts)
