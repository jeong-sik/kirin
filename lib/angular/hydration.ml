...
let incremental_options =
  { default_options with mode = Incremental; trigger = OnViewport }
;;
...
let boundary_end ~id () = Printf.sprintf {|<!--/nghb:%s-->|} id
...
let create_context ?(options = default_options) () = { components = []; options }
...
(** Get component state *)
let get_component_state ctx id = List.assoc_opt id ctx.components

(** Mark component as hydrated *)
let mark_hydrated ctx id = register_component ctx ~id Hydrated

(** Mark component as skipped *)
let mark_skipped ctx id = register_component ctx ~id Skipped
...
let mismatch_to_string = function
  ...
  | MissingNode tag -> Printf.sprintf "Missing node: <%s>" tag
  | ExtraNode tag -> Printf.sprintf "Extra node: <%s>" tag
;;
...
let context_to_json ctx =
  ...
  `Assoc
    [ ( "components"
      , `Assoc
          (List.map
             (fun (id, state) -> id, `String (state_to_string state))
             ctx.components) )
