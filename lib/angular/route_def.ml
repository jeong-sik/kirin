...
let route path component = { (empty path) with component = Some component }

(** Create redirect route *)
let redirect ~from ~to_ ?(path_match = "full") () =
  { (empty from) with redirect_to = Some to_; path_match = Some path_match }
;;

(** Create lazy-loaded route *)
let lazy_route path load_children =
  { (empty path) with component = Some load_children; lazy_load = true }
;;

(** {1 Modifiers} *)

(** Set render mode *)
let with_render_mode mode route = { route with render_mode = mode }
...
(** Add route data *)
let with_data key value route = { route with data = (key, value) :: route.data }

(** Set title *)
let with_title title route = { route with title = Some title }
...
(** Create SSR route *)
let ssr path component = route path component |> with_render_mode Server

(** Create SSG route *)
let ssg path component = route path component |> with_render_mode Prerender

(** Create CSR route *)
let csr path component = route path component |> with_render_mode Client
...
let rec to_json route =
  ...
  in
  let with_component =
    match route.component with
    | Some c -> ("component", `String c) :: base
    | None -> base
  in
  let with_redirect =
    match route.redirect_to with
    | Some r -> ("redirectTo", `String r) :: with_component
    | None -> with_component
  in
  let with_path_match =
    match route.path_match with
  ...
  in
  let with_data =
    if route.data = [] then with_title else ("data", `Assoc route.data) :: with_title
  in
  let with_children =
    if route.children = []
    then with_data
