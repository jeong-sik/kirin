...
let create ?(options = default_cache_options) () = { entries = []; options }
...
let set state key value =
  let entry = { key; value; timestamp = Unix.gettimeofday (); ttl = None } in
  state.entries <- entry :: List.filter (fun e -> e.key <> key) state.entries
;;

(** Set value with TTL *)
let set_with_ttl state key value ttl =
  let entry = { key; value; timestamp = Unix.gettimeofday (); ttl = Some ttl } in
  state.entries <- entry :: List.filter (fun e -> e.key <> key) state.entries
;;
...
(** Check if key exists *)
let has_key state key = Option.is_some (get state key)

(** Remove value *)
let remove state key = state.entries <- List.filter (fun e -> e.key <> key) state.entries

(** Clear all *)
let clear state = state.entries <- []

(** Get all keys *)
let keys state = List.map (fun e -> e.key) state.entries
...
let cache_response state ~method_ ~url ?(params = []) response =
  if
    method_ = "GET"
    || method_ = "HEAD"
    || (method_ = "POST" && state.options.include_post_requests)
  then (
    let key = make_http_key ~method_ ~url ~params () in
    let should_cache =
      match state.options.filter with
      | None -> true
      | Some f -> f url
    in
...
let to_json state = `Assoc [ "entries", `List (List.map entry_to_json state.entries) ]
