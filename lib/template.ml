...
let context pairs = `Assoc (List.map (fun (k, v) -> k, `String v) pairs)

(** Create context with various value types *)
let context_of pairs = `Assoc pairs
...
let find_closing tag content start =
  ...
  let rec find depth pos =
    if pos >= content_len
    then None
    else if pos + close_len <= content_len && String.sub content pos close_len = close_tag
    then if depth = 0 then Some pos else find (depth - 1) (pos + close_len)
    else if
      pos + String.length open_tag <= content_len
      && String.sub content pos (String.length open_tag) = open_tag
    then find (depth + 1) (pos + 1)
    else find depth (pos + 1)
  in
  find 0 start
;;
...
let rec parse template =
  ...
  while !pos < len do
    ...
    | Some i when i + 1 < len && template.[i + 1] = '{' ->
      (* Found {{ *)
      if i > !pos then nodes := Text (String.sub template !pos (i - !pos)) :: !nodes;
      let tag_start = i + 2 in
      (* Check for {{{ (raw) *)
      if tag_start < len && template.[tag_start] = '{'
      then (
        match String.index_from_opt template (tag_start + 1) '}' with
        | Some j when j + 2 < len && template.[j + 1] = '}' && template.[j + 2] = '}' ->
          let var_name =
            String.trim (String.sub template (tag_start + 1) (j - tag_start - 1))
          in
        ...
        (* Check for {{! (comment) *)
        tag_start < len && template.[tag_start] = '!'
      then (
           ...
           | Some close_pos ->
             ...
             (match section_type with
              | `If | `Section ->
                ...
                let rec find_else i =
                  if i + else_len > String.length inner
                  then None
                  else if String.sub inner i else_len = else_tag
                  then Some i
                  else find_else (i + 1)
                in
                (match find_else 0 with
                 | Some ep ->
                   ...
                   let else_part =
                     String.sub inner (ep + else_len) (String.length inner - ep - else_len)
                   in
                   nodes := If (key, parse then_part, Some (parse else_part)) :: !nodes
                 | None -> nodes := If (key, parse inner, None) :: !nodes)
              | `Unless -> nodes := Unless (key, parse inner) :: !nodes
              | `Each -> nodes := Each (key, parse inner) :: !nodes);
...
let rec render_nodes ?(partials = no_partials) ctx nodes =
  ...
  List.iter (fun node -> Buffer.add_string buf (render_node ~partials ctx node)) nodes;
  Buffer.contents buf
...
and render_node ?(partials = no_partials) ctx = function
  ...
  | If (key, then_nodes, else_nodes) ->
    ...
    if Option.map is_truthy value = Some true
    then render_nodes ~partials ctx then_nodes
    else (
  ...
  | Unless (key, nodes) ->
    let value = lookup ctx key in
    if Option.map is_truthy value <> Some true
    then render_nodes ~partials ctx nodes
    else ""
  | Each (key, nodes) ->
    (match lookup ctx key with
     | Some (`List items) ->
    ...
    (fun acc (key, value) ->
       ...
       let rec replace s =
         ...
         | Some i ->
           ...
           if i + plen <= String.length s && String.sub s i plen = pattern
           then (
