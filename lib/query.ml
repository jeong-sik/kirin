...
let select_distinct columns = { (empty Select) with columns; distinct = true }
...
let set column value t = { t with sets = t.sets @ [ column, value ] }

(** Add multiple SET clauses *)
let set_many pairs t = { t with sets = t.sets @ pairs }
...
(** Add HAVING *)
let having condition params t = { t with having = Some (condition, params) }
...
let order_by_many orders t = { t with order_by_clauses = t.order_by_clauses @ orders }
...
let build_where_conditions clauses =
  ...
  then [], []
    ...
    let is_or cond = String.length cond > 3 && String.sub cond 0 3 = "OR " in
...
let build_select t =
  ...
  let parts =
    if t.group_by = []
    then parts
    else parts @ [ "GROUP BY " ^ String.concat ", " t.group_by ]
  in
  (* HAVING *)
  let parts, having_params =
    match t.having with
    | None -> parts, []
  ...
  (* ORDER BY *)
  let parts =
    if t.order_by_clauses = []
    then parts
    else (
      let orders =
        List.map
          (fun (col, ord) -> Printf.sprintf "%s %s" col (order_to_sql ord))
          t.order_by_clauses
      in
  ...
  (* LIMIT *)
  let parts =
    match t.limit_val with
    | None -> parts
    | Some n -> parts @ [ Printf.sprintf "LIMIT %d" n ]
  in
  (* OFFSET *)
  let parts =
    match t.offset_val with
    | None -> parts
    | Some n -> parts @ [ Printf.sprintf "OFFSET %d" n ]
  in
  { sql = String.concat " " parts; params = where_params @ having_params }
;;
...
let build_insert t =
  ...
  let values_str =
    if List.length t.values_list <= 1
    then Printf.sprintf "(%s)" placeholders
    else (
      let rows = List.map (fun _ -> Printf.sprintf "(%s)" placeholders) t.values_list in
      String.concat ", " rows)
  in
  let sql = Printf.sprintf "INSERT INTO %s (%s) VALUES %s" table cols values_str in
  (* RETURNING *)
  let sql =
    if t.returning = [] then sql else sql ^ " RETURNING " ^ String.concat ", " t.returning
  in
...
let build_update t =
  let table = Option.value t.table ~default:"" in
  let set_clauses = List.map (fun (col, _) -> Printf.sprintf "%s = ?" col) t.sets in
  ...
  let parts =
    if t.returning = []
    then parts
    else parts @ [ "RETURNING " ^ String.concat ", " t.returning ]
  in
  { sql = String.concat " " parts; params = set_params @ where_params }
;;
...
let build_delete t =
  ...
  in
  { sql = String.concat " " parts; params = where_params }
;;
...
let where_null column t = where (Printf.sprintf "%s IS NULL" column) [] t

(** WHERE column IS NOT NULL *)
let where_not_null column t = where (Printf.sprintf "%s IS NOT NULL" column) [] t
...
(** Pagination helper *)
let paginate ~page ~per_page t = t |> limit per_page |> offset ((page - 1) * per_page)
