...
let compose (m1 : t) (m2 : t) : t = fun handler -> m1 (m2 handler)

(** Compose a list of middlewares (left to right application) *)
let pipeline middlewares = List.fold_right compose middlewares id
...
let catch (error_handler : exn -> Request.t -> Response.t) : t =
  fun handler req ->
...
let cors ?(config = default_cors_config) () : t =
  fun handler req ->
    ...
    | Some ov ->
      let resp =
        ...
        |> Response.with_header
             ...
             (String.concat ", " config.methods)
        |> Response.with_header
             "Access-Control-Allow-Headers"
             (String.concat ", " config.headers)
      in
      (* When reflecting a specific origin (not "*"), add Vary: Origin
         to prevent cache poisoning. *)
      let resp = if ov <> "*" then Response.with_header "Vary" "Origin" resp else resp in
      ...
      (match config.max_age with
       | Some age ->
         Response.with_header "Access-Control-Max-Age" (string_of_int age) resp
       | None -> resp)
  in
  if is_preflight
  then Response.empty `No_content |> add_cors_headers
  else handler req |> add_cors_headers
;;
...
let timing : t =
  ...
  Response.with_header
    "X-Response-Time"
    (Printf.sprintf "%.3fms" (elapsed *. 1000.0))
    resp
;;
