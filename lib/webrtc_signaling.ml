...
let encode_message msg =
  let json =
    ...
    | SdpOffer { from_peer; sdp } ->
      `Assoc [ "type", `String "offer"; "from", `String from_peer; "sdp", `String sdp ]
    | SdpAnswer { from_peer; sdp } ->
      `Assoc [ "type", `String "answer"; "from", `String from_peer; "sdp", `String sdp ]
      ...
      `Assoc
        ...
        ; ( "candidate"
          , `Assoc
              ...
              ; ( "sdpMid"
                , match candidate.Webrtc_config.sdp_mid with
                  | Some s -> `String s
                  | None -> `Null )
              ; ( "sdpMLineIndex"
                , match candidate.Webrtc_config.sdp_mline_index with
                  | Some i -> `Int i
                  | None -> `Null )
              ] )
        ]
    | Join { peer_id; room } ->
      `Assoc [ "type", `String "join"; "peerId", `String peer_id; "room", `String room ]
    | Leave { peer_id } -> `Assoc [ "type", `String "leave"; "peerId", `String peer_id ]
    | Error { message } -> `Assoc [ "type", `String "error"; "message", `String message ]
  in
...
let decode_message json_str =
  try
    ...
    | "leave" ->
      let peer_id = json |> member "peerId" |> to_string in
      Ok (Leave { peer_id })
    | t -> Result.Error (Printf.sprintf "Unknown message type: %s" t)
  with
  | Yojson.Json_error e -> Result.Error (Printf.sprintf "JSON parse error: %s" e)
  | Yojson.Safe.Util.Type_error (msg, _) ->
    Result.Error (Printf.sprintf "Decode error: %s" msg)
;;
...
let create_server () = { rooms = Hashtbl.create 16; mutex = Eio.Mutex.create () }
...
let get_or_create_room server room_id =
  Eio.Mutex.use_rw ~protect:true server.mutex (fun () ->
    ...
    | Some r -> r
    | None ->
      let r = { id = room_id; peers = Hashtbl.create 8; mutex = Eio.Mutex.create () } in
