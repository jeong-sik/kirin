...
let serialize_resource serialize_data = function
  ...
  | Refreshing data ->
    `Assoc [ "state", `String "refreshing"; "data", serialize_data data ]
;;
...
let store_data_tag ~store_id value =
  Printf.sprintf
    {|<script>window.__SOLID_STORES__=window.__SOLID_STORES__||{};window.__SOLID_STORES__["%s"]=%s;</script>|}
    store_id
    (serialize value)
;;
...
let signal_data_tag ~signal_id value =
  Printf.sprintf
    {|<script>window.__SOLID_SIGNALS__=window.__SOLID_SIGNALS__||{};window.__SOLID_SIGNALS__["%s"]=%s;</script>|}
    signal_id
    (serialize value)
;;
...
let hydration_start ~component_id = Printf.sprintf {|<!--$%s-->|} component_id

(** Generate hydration end marker *)
let hydration_end ~component_id = Printf.sprintf {|<!--/%s-->|} component_id
...
let empty_initial_data =
  { route_id = None; route_data = None; stores = []; signals = []; resources = [] }
;;
...
let all_scripts data =
  ...
  (* Route data *)
  let scripts =
    match data.route_id, data.route_data with
    | Some route_id, Some route_data -> route_data_tag ~route_id route_data :: scripts
    | _ -> scripts
  in
  (* Stores *)
  let scripts =
    List.fold_left
      (fun acc (store_id, value) -> store_data_tag ~store_id value :: acc)
      scripts
      data.stores
  in
  (* Signals *)
  let scripts =
    List.fold_left
      (fun acc (signal_id, value) -> signal_data_tag ~signal_id value :: acc)
      scripts
      data.signals
  in
  (* Resources *)
  let scripts =
    List.fold_left
      (fun acc (resource_id, value) ->
         Printf.sprintf
           {|<script>window.__SOLID_RESOURCES__=window.__SOLID_RESOURCES__||{};window.__SOLID_RESOURCES__["%s"]=%s;</script>|}
           resource_id
           (serialize value)
         :: acc)
      scripts
