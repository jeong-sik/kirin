...
let start engine =
  Array.iter
    (fun worker ->
       ...
       | Ok () -> ()
       | Error msg -> Kirin.Logger.error "Failed to start SSR worker: %s" msg)
    engine.pool
;;

(** Stop all workers *)
let shutdown engine = Array.iter Worker.stop engine.pool
...
let cache_key ~url ~props = Printf.sprintf "%s|%s" url (Yojson.Safe.to_string props)
     ...
     | Some worker ->
       ...
       (match Worker.render worker ~url ~props () with
        | Ok response ->
          ...
          if Hashtbl.length engine.cache < engine.cache_max_size
          then Hashtbl.replace engine.cache key response;
          Ok response
        | Error msg ->
          ignore (Atomic.fetch_and_add engine.errors 1);
          Error msg))
...
(** Clear render cache *)
let clear_cache engine = Hashtbl.clear engine.cache

(** Get cache size *)
let cache_size engine = Hashtbl.length engine.cache
...
let stats engine =
  let workers_ready =
    Array.fold_left
      (fun acc w -> if Worker.is_ready w then acc + 1 else acc)
      0
      engine.pool
  in
  ...
  let cache_hit_rate =
    if total > 0 then float_of_int (total - errs) /. float_of_int total else 0.0
  in
  { total_renders = total
  ; errors = errs
...
let recover_unhealthy engine =
  Array.iter
    (fun worker -> if Worker.needs_restart worker then ignore (Worker.restart worker))
    engine.pool
;;
