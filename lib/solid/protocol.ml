...
let encode_render_request req =
  let id = next_id () in
  let params = `Assoc [ "url", `String req.url; "props", req.props ] in
  let json =
    `Assoc
      [ "jsonrpc", `String "2.0"
      ; "id", `Int id
...
let encode_stream_request ~url ~props =
  let id = next_id () in
  let params = `Assoc [ "url", `String url; "props", props ] in
  let json =
    `Assoc
      [ "jsonrpc", `String "2.0"
      ; "id", `Int id
       ...
       | _ ->
         (match List.assoc_opt "result" fields with
          | Some (`Assoc result_fields) ->
            let html =
              match List.assoc_opt "html" result_fields with
              | Some (`String h) -> h
              | _ -> ""
            in
            let head =
              match List.assoc_opt "head" result_fields with
              | Some (`String h) -> h
              | _ -> ""
            in
            let status =
              match List.assoc_opt "status" result_fields with
              | Some (`Int s) -> s
              | _ -> 200
            in
            let redirect =
              match List.assoc_opt "redirect" result_fields with
              | Some (`String r) -> Some r
              | _ -> None
            in
            let resources =
              match List.assoc_opt "resources" result_fields with
              | Some (`Assoc res) -> List.map (fun (k, v) -> k, v) res
              | _ -> []
            in
...
let decode_stream_chunk json_str =
  ...
  with
  | Yojson.Json_error _ -> Error "Chunk parse error"
;;
...
let decode_health_response json_str =
  try
    ...
    | `Assoc fields ->
      (match List.assoc_opt "result" fields with
       | Some (`Assoc result) ->
         let ok =
           match List.assoc_opt "ok" result with
           | Some (`Bool b) -> b
           | _ -> false
         in
         let memory_mb =
           match List.assoc_opt "memoryMB" result with
           | Some (`Int m) -> m
           | _ -> 0
         in
         let uptime_s =
           match List.assoc_opt "uptimeSeconds" result with
           | Some (`Int u) -> u
           | _ -> 0
         in
         let renders =
           match List.assoc_opt "renders" result with
           | Some (`Int r) -> r
           | _ -> 0
         in
  ...
  with
  | Yojson.Json_error _ | Yojson.Safe.Util.Type_error (_, _) -> Error "Parse error"
;;
