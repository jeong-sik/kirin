...
val route
  ...
  -> ?data:string
  -> ?preload:bool
  -> ?children:Router.route list
  -> unit
  -> Router.route

val index_route : component:string -> ?data:string -> unit -> Router.route
val catch_all_route : component:string -> ?data:string -> unit -> Router.route
...
val stream_sse : engine:Ssr.t -> url:string -> ?props:Yojson.Safe.t -> unit -> string
val progressive_marker : component_id:string -> priority:Streaming.priority -> string
...
val data_routes
  :  loaders:
       (string * (url:string -> params:'a list -> Yojson.Safe.t Router.loader_result))
         list
  -> Kirin.Router.route list

val stats : Ssr.t -> Ssr.stats
val clear_cache : Ssr.t -> unit
val health_check : Ssr.t -> (Worker.state * Protocol.health_status option) array
