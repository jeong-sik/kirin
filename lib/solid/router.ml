...
let route ~path ~component ?data ?(preload = false) ?(children = []) () =
  { path; component; children; data; preload }
;;

(** Create index route *)
let index ~component ?data () = route ~path:"" ~component ?data ()

(** Create catch-all route *)
let catch_all ~component ?data () = route ~path:"*" ~component ?data ()

(** Create layout route (no path, just children) *)
let layout ~component ?(children = []) () =
  { path = ""; component; children; data = None; preload = false }
;;

(** {1 Route Matching} *)

(** Parse path into segments *)
let parse_path path = String.split_on_char '/' path |> List.filter (fun s -> s <> "")

(** Check if segment is dynamic *)
let is_dynamic_segment seg = String.length seg > 0 && String.get seg 0 = ':'

(** Extract param name from dynamic segment *)
let param_name seg =
  if is_dynamic_segment seg then String.sub seg 1 (String.length seg - 1) else seg
;;
...
let match_segment pattern segment =
  if pattern = "*"
  then Some [ "*", segment ]
  else if is_dynamic_segment pattern
  then Some [ param_name pattern, segment ]
...
let match_path pattern path =
  ...
  let rec match_segs acc patterns paths =
    ...
    | _, [] ->
      ...
      if
        List.for_all
          (fun p -> String.length p > 1 && String.get p (String.length p - 1) = '?')
          patterns
...
let rec find_route path routes =
  List.find_map
    (fun route ->
       ...
       | Some params -> Some { route; params; search = [] }
       | None ->
         (* Check children *)
...
let rec route_to_config route =
  let base =
    Printf.sprintf
      {|{
  path: "%s",
  component: lazy(() => import("%s"))|}
      route.path
      route.component
  in
  let data =
    match route.data with
    | Some d ->
      Printf.sprintf
        {|,
  data: () => import("%s").then(m => m.default)|}
        d
    | None -> ""
  in
  let children =
    ...
    let routes =
      Array.fold_left
        (fun acc entry ->
           ...
           then (
             ...
             let route_path =
               ...
               ^ String.sub
                   ...
                   (String.length full_path - String.length base_path - 1)
             in
             (* Check for index component *)
             let index_file = Filename.concat full_path "index.tsx" in
             if Sys.file_exists index_file
             then (
               let r =
                 route ~path:route_path ~component:index_file ~children:child_routes ()
               in
               r :: acc)
             else if child_routes <> []
             then (
           ...
           else if Filename.check_suffix entry ".tsx" && entry <> "index.tsx"
           then (
...
let serialize_loader_result serialize = function
  | Data d -> `Assoc [ "type", `String "data"; "data", serialize d ]
  | Redirect (url, status) ->
    `Assoc [ "type", `String "redirect"; "url", `String url; "status", `Int status ]
  | NotFound -> `Assoc [ "type", `String "notFound" ]
  | ServerError msg -> `Assoc [ "type", `String "error"; "message", `String msg ]
;;
