...
module PubSub = struct
  ...
  let create () =
    { mutex = Eio.Mutex.create (); cond = Eio.Condition.create (); version = 0L }
  ;;
...
module Make (Db : DB) = struct
  ...
  let handle_pull client_id from_version = Db.process_pull client_id from_version

  (** Handle Poke (Wait for changes) *)
  let handle_poke _client_id current_version = PubSub.wait pubsub current_version
end
