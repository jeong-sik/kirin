...
let map ?(domains = recommended_domains ()) f items =
  if domains <= 1 || List.length items <= 1
  then List.map f items
  else (
    ...
    let spawned =
      Array.init
        (min domains ((n + chunk_size - 1) / chunk_size))
        (fun i -> Domain.spawn (fun () -> process_chunk (i * chunk_size)))
    in
...
let filter ?(domains = recommended_domains ()) pred items =
  ...
  List.filter_map
    (fun (item, keep) -> if keep then Some item else None)
...
let reduce ?(domains = recommended_domains ()) combine init items =
  if domains <= 1 || List.length items <= 1
  then List.fold_left combine init items
  else (
    let items_array = Array.of_list items in
    let n = Array.length items_array in
    let chunk_size = max 1 ((n + domains - 1) / domains) in
    ...
    in
    let spawned =
      Array.init num_chunks (fun i -> Domain.spawn (fun () -> process_chunk i))
    in
    Array.iter Domain.join spawned;
...
module Pool = struct
  ...
  let create ?(size = recommended_domains ()) () = { size; active = Atomic.make true }

  (** Map using domain pool *)
  let map pool f items =
    if not (Atomic.get pool.active)
    then failwith "Domain pool is shut down"
    else map ~domains:pool.size f items
  ;;
  ...
  let iter pool f items =
    if not (Atomic.get pool.active)
    then failwith "Domain pool is shut down"
    else iter ~domains:pool.size f items
  ;;
  ...
  let reduce pool combine init items =
    if not (Atomic.get pool.active)
    then failwith "Domain pool is shut down"
    else reduce ~domains:pool.size combine init items
  ;;
  ...
  (** Shutdown the pool *)
  let shutdown pool = Atomic.set pool.active false
end
...
let with_domains n f = map ~domains:n f
...
let chunk_list n items =
  let rec aux acc current count = function
    ...
    | x :: xs ->
      if count >= n
      then aux (List.rev current :: acc) [ x ] 1 xs
