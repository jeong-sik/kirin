...
let parse_accept_encoding header_value =
  ...
  if List.mem "gzip" encodings
  then Some Gzip
  else if List.mem "deflate" encodings
  then Some Deflate
  else None
;;
...
let skip_compression_for content_type =
  ...
  let ends_with s suffix =
    ...
    and sufl = String.length suffix in
    sl >= sufl && String.sub s (sl - sufl) sufl = suffix
  in
  ends_with ct "/zip"
  || ends_with ct "/gzip"
  || ends_with ct "/x-gzip"
  || ends_with ct "/x-compress"
;;
...
let compress_gzip input =
  ...
  then input
    ...
    let encoder =
      Gz.Def.encoder (`String input) (`Buffer output_buf) ~mtime:0l Gz.Unix ~q ~w ~level:6
    in
...
let compress_deflate input =
  ...
  then input
    ...
    let q = De.Queue.create 4096 in
    let w = De.Lz77.make_window ~bits:15 in
    let encoder = Zl.Def.encoder (`String input) (`Buffer output_buf) ~q ~w ~level:6 in
    let rec loop encoder =
...
let middleware ?(min_size = min_compress_size)
  : (Request.t -> Response.t) -> Request.t -> Response.t
  =
  fun handler req ->
  let resp = handler req in
  match Response.body resp with
  | Response.Stream _ | Response.Producer _ ->
    resp (* Streaming compression not yet supported *)
  | Response.String body ->
    ...
    then resp
      ...
      let content_type =
        Response.header "content-type" resp |> Option.value ~default:"text/plain"
      in
      if skip_compression_for content_type
      then resp
      else (
