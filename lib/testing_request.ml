...
let empty = { meth = `GET; path = "/"; headers = []; body = ""; query = [] }
...
(** Add header *)
let with_header name value req = { req with headers = (name, value) :: req.headers }
...
(** Add query parameter *)
let with_query name value req = { req with query = (name, value) :: req.query }
...
let get ?headers ?query path =
  ...
  let req =
    match headers with
    | None -> req
    | Some h -> with_headers h req
  in
  match query with
  | None -> req
  | Some q -> with_queries q req
;;
...
let with_bearer_token token req = with_header "Authorization" ("Bearer " ^ token) req
...
(** Set Accept header *)
let with_accept content_type req = with_header "Accept" content_type req
