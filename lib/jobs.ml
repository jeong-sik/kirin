...
let default_config =
  { workers = 4; max_queue_size = 10000; default_max_retries = 3; retry_delay = 1.0 }
;;
  ...
  while t.running do
    let job_opt =
      Eio.Mutex.use_rw ~protect:true t.mutex (fun () ->
        ...
        | Some _ as r -> r
        | None ->
          if t.running then Eio.Condition.await t.job_available t.mutex;
          if t.running then take_job t else None)
    in
    match job_opt with
...
let submit ?priority ?max_retries t task =
  ...
  Eio.Mutex.use_rw ~protect:true t.mutex (fun () ->
    if List.length t.jobs >= t.config.max_queue_size then failwith "Job queue full";
    let id =
      let n = t.next_id in
      t.next_id <- n + 1;
...
let wait t job_id =
  let rec loop () =
    let st = Eio.Mutex.use_ro t.mutex (fun () -> Hashtbl.find_opt t.results job_id) in
    match st with
...
let is_running t = Eio.Mutex.use_ro t.mutex (fun () -> t.running)

(** {1 Statistics} *)

let stats t =
  Eio.Mutex.use_ro t.mutex (fun () -> { t.stats with queue_size = List.length t.jobs })
;;

let pending_count t = Eio.Mutex.use_ro t.mutex (fun () -> List.length t.jobs)
let running_count t = Eio.Mutex.use_ro t.mutex (fun () -> t.stats.currently_running)
