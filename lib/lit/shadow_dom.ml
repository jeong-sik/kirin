...
let with_named_slots slots content =
  let slot_defs =
    List.map
      (fun (name, fallback) -> { slot_name = Some name; slot_fallback = fallback })
      slots
  in
...
(** Create default slot *)
let default_slot ?fallback () = { slot_name = None; slot_fallback = fallback }

(** Create named slot *)
let named_slot name ?fallback () = { slot_name = Some name; slot_fallback = fallback }

(** Render slot element *)
let render_slot slot =
  let name_attr =
    match slot.slot_name with
    | Some name -> Printf.sprintf " name=\"%s\"" name
    | None -> ""
  in
  let fallback_content =
    match slot.slot_fallback with
...
let to_declarative_shadow_dom shadow =
  ...
  in
  Printf.sprintf
    {|<template shadowrootmode="%s"%s>%s%s</template>|}
    mode_str
    focus_attr
    styles_html
    shadow.content
...
let render_component ~tag_name ~attributes ?(light_dom = "") shadow =
  ...
  Printf.sprintf
    ...
    tag_name
    attrs_str
    (to_declarative_shadow_dom shadow)
    light_dom
    tag_name
;;
...
(** Create CSS module *)
let css_module ~id content = { css_id = id; css_content = content }
...
(** Create part *)
let part ~name element = { part_name = name; part_element = element }

(** Render element with part attribute *)
let render_with_part p = Printf.sprintf "<%s part=\"%s\">" p.part_element p.part_name
...
let exportparts_attr parts = Printf.sprintf "exportparts=\"%s\"" (exportparts parts)
...
let to_json shadow =
  `Assoc
    ...
    ; ( "slotAssignment"
      , `String
          (match shadow.slot_assignment with
           | `Named -> "named"
           | `Manual -> "manual") )
