...
(** Create slot *)
let slot ?name ?fallback () = { parts = [ Slot { name; fallback } ] }
...
(** Create loop *)
let repeat ~items ~item ?index body = { parts = [ Loop { items; item; index; body } ] }

(** Create directive *)
let directive directive_name args = { parts = [ Directive { directive_name; args } ] }

(** Concatenate templates *)
let concat templates = { parts = List.concat_map (fun t -> t.parts) templates }
...
let class_map classes =
  let args =
    classes
    |> List.map (fun (cls, expr) -> Printf.sprintf "'%s': %s" cls expr)
    |> String.concat ", "
  in
...
let style_map styles =
  let args =
    styles
    |> List.map (fun (prop, value) -> Printf.sprintf "'%s': %s" prop value)
    |> String.concat ", "
  in
...
(** ifDefined directive *)
let if_defined expr = directive "ifDefined" [ expr ]

(** live directive *)
let live expr = directive "live" [ expr ]

(** ref directive *)
let ref_directive name = directive "ref" [ name ]

(** until directive (async) *)
let until promise fallback = directive "until" [ promise; fallback ]
...
let to_html_literal template = "html`" ^ render_to_lit template ^ "`"
...
let rec render_static_expression ~data = function
  ...
  | Conditional { condition; then_; else_ } ->
    ...
    let is_truthy =
      match List.assoc_opt condition data with
      | Some (`Bool true) -> true
    ...
    in
    if is_truthy
    then render_static ~data then_
    else (
      match else_ with
      | Some e -> render_static ~data e
      | None -> "")
  | Loop { items; item; body; _ } ->
    (match List.assoc_opt items data with
     | Some (`List arr) ->
       arr
       |> List.map (fun item_data ->
         let item_obj =
           match item_data with
           | `Assoc pairs -> pairs
           | _ -> [ item, item_data ]
         in
...
and to_json template =
  `Assoc [ "parts", `List (List.map expression_to_json template.parts) ]
;;
