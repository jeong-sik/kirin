...
let create config =
  let workers =
    Array.init config.pool_size (fun _ ->
      { state = Idle; request_count = Atomic.make 0; last_error = None })
  in
  { config
  ; workers
...
let render_internal engine ~tag_name ?(props = `Null) ?(slots = []) () =
  ...
  in
  let shadow_dom =
    if engine.config.use_declarative_shadow_dom
    then
      Printf.sprintf
  ...
  in
  let html =
    Printf.sprintf "<%s%s>%s%s</%s>" tag_name props_attrs shadow_dom slot_content tag_name
  in
  Ok
    { html
    ; components = [ tag_name ]
    ; has_shadow_dom = engine.config.use_declarative_shadow_dom
    }
;;
...
let render engine ~tag_name ?props ?slots () =
  ...
  | Ok result ->
    let html =
      if engine.config.include_polyfill
      then Shadow_dom.with_polyfill result.html
      else result.html
    in
    Ok html
  | Error msg ->
...
let render_page engine ~components =
  let results =
    components
    |> List.map (fun (tag_name, props, slots) ->
      match render_internal engine ~tag_name ~props ~slots () with
      | Ok r -> r.html
  ...
  if engine.config.include_polyfill then html ^ "\n" ^ Shadow_dom.polyfill_check else html
;;
...
let render_stream engine ~tag_name ~on_chunk =
  ...
  (* Shadow DOM template *)
  if engine.config.use_declarative_shadow_dom
  then on_chunk (Component {|<template shadowrootmode="open"><slot></slot></template>|});
  (* Close tag *)
  on_chunk (Shell (Printf.sprintf "</%s>" tag_name));
  (* Polyfill if needed *)
  if engine.config.include_polyfill then on_chunk (Component Shadow_dom.polyfill_check);
  on_chunk Complete
;;
...
let stats engine =
  let active =
    Array.fold_left
      (fun acc w -> if w.state <> Dead then acc + 1 else acc)
      0
      engine.workers
  in
  ...
  ; stat_avg_time_ms = 0.0
  ; (* Would track actual times *)
...
(** Shutdown engine *)
let shutdown engine = Array.iter (fun w -> w.state <- Dead) engine.workers

(** Check if engine is healthy *)
let is_healthy engine = Array.exists (fun w -> w.state <> Dead) engine.workers
