...
let property_decorator prop =
  ...
  let options =
    ...
    | Some attr when attr <> prop.prop_name -> [ Printf.sprintf "attribute: '%s'" attr ]
    | _ -> []
  in
...
let to_typescript elem =
  let props_code =
    elem.properties
    |> List.map (fun prop ->
      let decorator = if prop.prop_state then "@state()" else property_decorator prop in
      let default =
        match prop.prop_default with
...
|}
    ...
    (elem.properties
     |> List.filter_map (fun p ->
       ...
       | Some json ->
         Some
           (Printf.sprintf "    this.%s = %s;" p.prop_name (Yojson.Safe.to_string json))
       | None -> None)
     |> String.concat "\n")
    elem.template
    elem.tag_name
