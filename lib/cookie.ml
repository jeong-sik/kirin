...
let parse_cookies header_value =
  ...
  |> List.filter_map (fun pair ->
    ...
    | Some idx ->
      ...
      let value =
        String.sub pair (idx + 1) (String.length pair - idx - 1) |> String.trim
      in
...
let set_secret key =
  ...
  then failwith "Cookie secret key must be at least 32 characters"
  else secret_key := key
;;
...
let sign value =
  if !secret_key = ""
  then failwith "Cookie secret key not set. Call Kirin.Cookie.set_secret first."
  else (
...
let verify signed_value =
  try
    ...
    | Some idx ->
      ...
      let signature =
        String.sub signed_value (idx + 1) (String.length signed_value - idx - 1)
      in
      let expected_sig = sign value in
      if Eqaf.equal signature expected_sig then Some value else None
    | None -> None
  with
