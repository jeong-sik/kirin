...
let upgrade_response req =
  ...
  | Some client_key ->
    ...
    let resp =
      Response.make ~status:`Switching_protocols (`String "")
    ...
    let resp =
      match Request.header "sec-websocket-protocol" req with
      | Some protocols ->
        (* Take first requested protocol as accepted *)
        let first_proto = String.split_on_char ',' protocols |> List.hd |> String.trim in
...
let encode_frame frame =
  ...
  if payload_len < 126
  then Buffer.add_char buf (Char.chr payload_len)
  ...
  then Error "Frame too short"
    ...
    | Some opcode ->
      let header_len, payload_len =
        ...
        then 2, payload_len_indicator
        else if payload_len_indicator = 126
        then
          if len < 4
          then 4, -1 (* Need more data *)
          else (
      ...
      in
      if payload_len < 0
      then Error "Incomplete frame header"
      else (
        let mask_len = if masked then 4 else 0 in
        let total_len = header_len + mask_len + payload_len in
        if len < total_len
        then Error (Printf.sprintf "Incomplete frame: need %d, have %d" total_len len)
        else (
          let payload =
...
(** Create a text frame *)
let text_frame ?(fin = true) payload = { fin; opcode = Text; payload }

(** Create a binary frame *)
let binary_frame ?(fin = true) payload = { fin; opcode = Binary; payload }

(** Create a ping frame *)
let ping_frame ?(payload = "") () = { fin = true; opcode = Ping; payload }

(** Create a pong frame (in response to ping) *)
let pong_frame ~payload = { fin = true; opcode = Pong; payload }
...
let close_frame ?(code = Normal) ?(reason = "") () =
  let payload =
    ...
    let code_bytes =
      String.init 2 (fun i ->
        if i = 0
        then Char.chr ((code_int lsr 8) land 0xFF)
...
let middleware ~path ~handler : (Request.t -> Response.t) -> Request.t -> Response.t =
  ...
  if req_path = path && is_upgrade_request req
  then (
    ...
    | Ok resp ->
      ...
      handler.on_open ();
      resp
    | Error msg -> Response.bad_request ~body:msg ())
  else next_handler req
;;
