...
let create_context
      ...
      ()
  =
  { url; method_; params; query; headers; cookies; body; route_name; is_server }
;;
...
(** Get query param *)
let query ctx name = List.assoc_opt name ctx.query

(** Get header *)
let header ctx name = List.assoc_opt (String.lowercase_ascii name) ctx.headers
