...
let render ~options ~ssr_html ~payload ~entry_script =
  let nonce_attr =
    ...
    | None -> ""
  in
  let preload = if options.preload_hints = "" then "" else options.preload_hints ^ "\n" in
  let styles =
    options.styles
    |> List.map (fun href ->
...
let nuxt3_shell ~title ~app_html ~payload ~entry_script =
  render ~options:{ default_options with title } ~ssr_html:app_html ~payload ~entry_script
;;
...
let island_wrapper ~id ~component ~priority ~html =
  Printf.sprintf
    {|<div data-island="%s" data-component="%s" data-priority="%s">%s</div>|}
    id
    component
...
let teleport_target ~id = Printf.sprintf {|<div id="%s"></div>|} id
