...
let page path =
  let name =
    String.map
      (fun c -> if c = '/' then '-' else if c = '[' || c = ']' then '_' else c)
      path
  in
  { path
...
(** Add useFetch loader *)
let with_fetch url route = { route with loader = Some (UseFetch url) }

(** Add useAsyncData loader *)
let with_async_data key route = { route with loader = Some (UseAsyncData key) }

(** Add server loader *)
let with_server_loader route = { route with loader = Some ServerLoader }

(** Configure fetch options *)
let with_fetch_options opts route = { route with fetch_options = opts }
...
(** Set page meta *)
let with_meta meta route = { route with meta }

(** Set layout *)
let with_layout name route = { route with meta = { route.meta with layout = Some name } }

(** Set title *)
let with_title title route = { route with meta = { route.meta with title = Some title } }
...
let rec to_json (route : t) =
  ...
  let with_middleware =
    ...
    then with_children
      ...
      ( "middleware"
      , `List (List.map (fun (m : middleware) -> `String m.name) route.middleware) )
      :: with_children
  in
