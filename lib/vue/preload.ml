...
(** No preload/prefetch *)
let no_preload =
  { default_options with preload = Never; prefetch = Disabled; no_prefetch = true }
;;
...
let preload_css href =
  { hint_type = Preload; href; as_ = Some "style"; crossorigin = false; media = None }
;;

(** Create prefetch hint *)
let prefetch href =
  { hint_type = Prefetch; href; as_ = None; crossorigin = false; media = None }
;;

(** Create preconnect hint *)
let preconnect href =
  { hint_type = Preconnect; href; as_ = None; crossorigin = true; media = None }
;;
...
let link_attrs ~href ?(options = default_options) () =
  ...
  let with_prefetch =
    if options.no_prefetch then ("data-no-prefetch", "true") :: base else base
  in
  let with_replace =
    if options.replace then ("data-replace", "true") :: with_prefetch else with_prefetch
  in
  let with_target =
  ...
  in
  let with_external =
    if options.external_
    then ("rel", if options.no_rel then "" else "noopener noreferrer") :: with_target
    else with_target
  in
  with_external
;;
...
let link ~href ?(options = default_options) text =
  ...
  if options.no_prefetch
  then
    Printf.sprintf
      {|<NuxtLink to="%s" class="%s" no-prefetch>%s</NuxtLink>|}
      href
      class_attr
      text
  else Printf.sprintf {|<NuxtLink to="%s" class="%s">%s</NuxtLink>|} href class_attr text
;;
...
let render_hint hint =
  ...
  let media_attr =
    match hint.media with
    | Some m -> Printf.sprintf " media=\"%s\"" m
    | None -> ""
  in
  Printf.sprintf
    {|<link rel="%s" href="%s"%s%s%s />|}
    rel
    hint.href
    as_attr
    crossorigin
...
let render_hints hints = hints |> List.map render_hint |> String.concat "\n"
...
let all_hints manifest = manifest.Manifest.routes |> List.concat_map hints_for_route
...
let default_payload_options = { enabled = true; limit = 3; delay = 50; threshold = 0.0 }
