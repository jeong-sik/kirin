...
let create_context
      ...
      ()
  =
  { url; method_; params; query; headers; body; raw_body }
;;
...
(** Read JSON body *)
let read_body ctx = ctx.body

(** Read raw body *)
let read_raw_body ctx = ctx.raw_body
...
(** Create server route *)
let route path handler = { path; method_ = None; handler; middleware = [] }

(** Create GET route *)
let get path handler = { path; method_ = Some GET; handler; middleware = [] }

(** Create POST route *)
let post path handler = { path; method_ = Some POST; handler; middleware = [] }

(** Create PUT route *)
let put path handler = { path; method_ = Some PUT; handler; middleware = [] }

(** Create DELETE route *)
let delete path handler = { path; method_ = Some DELETE; handler; middleware = [] }

(** Add middleware to route *)
let with_middleware mw route = { route with middleware = mw :: route.middleware }
...
let validation_error ~field ~message ?(code = "invalid") () = { field; message; code }
...
let email form name =
  ...
  | Some _ ->
    Error (validation_error ~field:name ~message:"Invalid email" ~code:"email" ())
...
let min_length len form name =
  ...
  | Some _ ->
    Error
      (validation_error
         ~field:name
         ~message:(Printf.sprintf "Minimum %d characters" len)
         ~code:"minLength"
         ())
  | None -> Error (validation_error ~field:name ~message:"Required" ~code:"required" ())
;;
...
let result_to_json = function
  | ActionData data -> `Assoc [ "type", `String "data"; "data", data ]
  | ActionRedirect (status, url) ->
    `Assoc [ "type", `String "redirect"; "status", `Int status; "location", `String url ]
  | ActionError (status, msg) ->
    `Assoc [ "type", `String "error"; "status", `Int status; "message", `String msg ]
  | ActionEmpty -> `Assoc [ "type", `String "empty" ]
  | ActionStream id -> `Assoc [ "type", `String "stream"; "id", `String id ]
;;
