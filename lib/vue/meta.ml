...
(** Set title template *)
let with_title_template template head = { head with title_template = Some template }

(** Add meta tag *)
let with_meta ~name ~content head =
  let meta =
    { name = Some name; property = None; content; http_equiv = None; charset = None }
  in
...
let with_property ~property ~content head =
  let meta =
    { name = None; property = Some property; content; http_equiv = None; charset = None }
  in
  { head with metas = meta :: head.metas }
;;
...
let with_link ~rel ~href ?type_ ?sizes head =
  let link =
    { rel; href; hreflang = None; type_; sizes; media = None; crossorigin = None }
  in
...
let with_script ~src ?(async = false) ?(defer = false) head =
  let script =
    { src = Some src; content = None; type_ = None; async; defer; crossorigin = None }
  in
...
(** Add style *)
let with_style css head = { head with styles = css :: head.styles }
...
let render_meta m =
  ...
  | Some n, _, _ -> Printf.sprintf {|<meta name="%s" content="%s" />|} n m.content
  | _, Some p, _ -> Printf.sprintf {|<meta property="%s" content="%s" />|} p m.content
  | _, _, Some c -> Printf.sprintf {|<meta charset="%s" />|} c
  | _ -> ""
;;
...
let render_link (l : link) =
  ...
  let sizes_attr =
    ...
    | None -> ""
  in
  Printf.sprintf {|<link rel="%s" href="%s"%s%s />|} l.rel l.href type_attr sizes_attr
;;
...
let render_script s =
  ...
  | Some src, _ ->
    Printf.sprintf {|<script src="%s"%s%s></script>|} src async_attr defer_attr
  | None, Some content -> Printf.sprintf {|<script>%s</script>|} content
  | None, None -> ""
;;
...
let render (head : head) =
  let title =
    match head.title, head.title_template with
    | Some t, Some template ->
      Printf.sprintf
        "<title>%s</title>\n"
        (Str.global_replace (Str.regexp "%s") t template)
    | Some t, None -> Printf.sprintf "<title>%s</title>\n" t
    | None, _ -> ""
  in
  ...
  let styles =
    head.styles
    |> List.map (fun css -> Printf.sprintf "<style>%s</style>" css)
    |> String.concat "\n"
  in
...
let seo_to_head seo =
  ...
  empty
  |> (fun h ->
  match seo.title with
  | Some t -> with_title t h
  | None -> h)
  ...
  `Assoc
    [ ( "title"
      , match head.title with
        ...
        | None -> `Null )
    ; ( "titleTemplate"
      , match head.title_template with
        | Some t -> `String t
        | None -> `Null )
