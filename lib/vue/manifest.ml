...
(** Empty manifest *)
let empty =
  { routes = []; layouts = []; server_routes = []; plugins = []; middleware = [] }
;;
...
let add_plugin entry manifest = { manifest with plugins = entry :: manifest.plugins }
...
let create_layout ~name ~component ?(is_default = false) () =
  { layout_id = "layout-" ^ name; name; component; is_default }
;;

(** Create server route entry *)
let create_server_route ~path ~handler ?method_ () =
  { route_id = "server-" ^ path; path; method_; handler }
;;
...
(** Get default layout *)
let default_layout manifest = List.find_opt (fun l -> l.is_default) manifest.layouts
...
let route_to_json entry =
  ...
  let with_params =
    ...
    then with_name
      ...
      ("params", `Assoc (List.map (fun (k, v) -> k, `String v) entry.params)) :: with_name
  in
  let with_layout =
  ...
  let with_middleware =
    ...
    then with_layout
      ...
      ("middleware", `List (List.map (fun m -> `String m) entry.middleware))
      :: with_layout
  in
  let with_redirect =
    match entry.redirect with
...
let plugin_to_json entry =
  `Assoc
    [ "name", `String entry.name; "src", `String entry.src; "mode", `String entry.mode ]
;;
...
let from_json json =
  ...
  let routes =
    ...
    |> List.map (fun r ->
      ...
      ; is_layout =
          r |> member "isLayout" |> to_bool_option |> Option.value ~default:false
      })
  in
  { empty with routes }
;;
