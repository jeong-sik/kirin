...
let dev_config ?(port = 3000) () =
  { Handler.default_config with dev_mode = true; dev_port = port; mode = Handler.SPA }
;;

(** Create production SPA configuration *)
let spa_config ~manifest_path ~entry_point () =
  { Handler.default_config with mode = Handler.SPA; manifest_path; entry_point }
;;

(** Create hydration configuration *)
let hydration_config ~manifest_path ~entry_point () =
  { Handler.default_config with mode = Handler.Hydration; manifest_path; entry_point }
;;

(** Create full SSR configuration *)
let ssr_config ~manifest_path ~entry_point ?(workers = 4) ?(timeout_s = 5.0) () =
  let ssr = { Ssr.default_config with workers; timeout_s } in
...
let page ~path ?layout ?middleware () =
  ...
  let route =
    match layout with
    | Some l -> Route_def.with_layout l route
    | None -> route
  in
  let route =
...
let dynamic ~path ~param_name ?(param_type = Route_def.String) () =
  Route_def.page path |> Route_def.with_param param_name param_type
;;

(** Create a catch-all route *)
let catch_all ~path () = Route_def.page path |> Route_def.with_param "slug" Route_def.Slug
...
let async_data ~key ?(server = true) ?(lazy_ = false) () =
  { (Loader.default_async_options key) with server; lazy_ }
;;

(** {1 Meta Tags} *)

(** Create SEO meta configuration *)
let seo ~title ?description ?image ?url () = Meta.seo ~title ?description ?image ?url ()
...
let payload ~route_path () = Data.empty_payload ~route_path

(** Add data to payload *)
let with_data key value payload = Data.with_data key value payload

(** Add state to payload *)
let with_state key value payload = Data.with_state key value payload

(** {1 Handler Building} *)

(** Create catch-all Vue handler *)
let vue_handler config = Handler.catch_all_handler config
...
let vite_dev_url ?(port = 3000) path = Printf.sprintf "http://localhost:%d%s" port path

(** {1 Codegen Helpers} *)

(** Generate TypeScript types for routes *)
let generate_types ~routes ?api_routes () =
  Codegen.generate_types_file ~routes ~api_routes:(Option.value ~default:[] api_routes)
;;

(** Generate single route type *)
let generate_route_type route = Codegen.generate_route_type route

(** {1 Preloading} *)

(** Create modulepreload hint *)
let modulepreload href = Preload.modulepreload href

(** Create prefetch hint *)
let prefetch href = Preload.prefetch href

(** Render hints to HTML *)
let render_hints hints = Preload.render_hints hints

(** {1 Error Pages} *)

(** Generate 404 page *)
let not_found_page () = Hydrate.error_page ~status:404 ~message:"Page not found"
...
let island ~id ~component ?props ?priority () =
  Data.island ~id ~component ?props ?priority ()
;;
...
let streaming_context ?config () = Streaming.create_context ?config ()

(** Start streaming response *)
let start_streaming ctx = Streaming.start ctx
...
(** Finish streaming *)
let finish_streaming ctx = Streaming.finish ctx
...
let manifest_from_routes routes = Manifest.from_discovered_routes routes
