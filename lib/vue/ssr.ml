...
let store_cache t url result =
  if cache_ttl t.config > 0 then Hashtbl.replace t.cache url (result, Unix.gettimeofday ())
;;
...
let render t ~url ?(headers = []) ?(payload = None) () =
  if not t.running
  then Error "SSR engine is not running"
  else (
    (* Check cache first *)
    match check_cache t url with
    | Some cached ->
      Eio.Mutex.use_rw ~protect:true t.stats_mutex (fun () ->
        t.stats
        <- { t.stats with
...
<body>
  ...
  <script type="module" src="/_nuxt/entry.js"></script>
      ...
      Eio.Mutex.use_rw ~protect:true t.stats_mutex (fun () ->
        ...
        let total_time =
          (t.stats.avg_render_time_ms *. Float.of_int t.stats.total_renders)
          +. render_time_ms
        in
        t.stats
        <- { t.stats with
...
let default_route_options =
  { ssr = true; cache = NoCache; prerender = false; headers = [] }
;;
...
let with_route_options opts t ~url =
  ...
  then
    ...
    Ok
      { html =
          (match t.config.fallback_html with
           | Some fb -> fb
           | None ->
             {|<!DOCTYPE html>
<html>
...
<body>
  ...
  else render t ~url ~headers:opts.headers ()
;;
