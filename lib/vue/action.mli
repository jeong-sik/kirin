...
val required : (string * form_value) list -> string -> (string, validation_error) result
val email : (string * form_value) list -> string -> (string, validation_error) result

val min_length
  :  int
  -> (string * form_value) list
  -> string
  -> (string, validation_error) result

val set_header : 'a -> 'b -> 'c -> 'c
val set_status : int -> action_result -> action_result
val result_to_json : action_result -> [> `Assoc of (string * Yojson.Safe.t) list ]
...
val route_to_json
  :  server_route
  -> [> `Assoc of
          (string * [> `List of [> `String of string ] list | `Null | `String of string ])
            list
     ]
