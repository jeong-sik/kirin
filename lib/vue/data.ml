...
let serialize json = Yojson.Safe.to_string json |> escape_for_html
...
(** Add data to payload *)
let with_data key value payload = { payload with data = (key, value) :: payload.data }

(** Add state to payload *)
let with_state key value payload = { payload with state = (key, value) :: payload.state }

(** Set config *)
let with_config config payload = { payload with config = Some config }

(** Set error *)
let with_error error payload = { payload with error = Some error }
...
let payload_script ?(nonce = None) payload =
  ...
  | None -> Printf.sprintf {|<script>window.%s=%s</script>|} payload_var data
;;
...
(** Create async entry *)
let async_entry ~key data = { key; data; pending = false; error = None }

(** Create pending async entry *)
let async_pending ~key = { key; data = `Null; pending = true; error = None }

(** Create error async entry *)
let async_error ~key msg = { key; data = `Null; pending = false; error = Some msg }

(** Async entry to JSON *)
let async_to_json entry =
  let base =
    [ "key", `String entry.key; "data", entry.data; "pending", `Bool entry.pending ]
  in
  let with_error =
...
let island ~id ~component ?(props = `Assoc []) ?(priority = "eager") () =
  { island_id = id; component; props; priority }
;;
