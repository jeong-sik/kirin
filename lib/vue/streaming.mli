type state = Pending | Streaming | Complete | Error of string
type chunk_type = Shell | Suspense | Island | Script | Flush | End
type chunk = {
...
val streaming_headers :
  ?content_type:string -> unit -> (string * string) list
type context = {
  config : config;
  mutable state : state;
...
val context_to_json :
  ...
  [> `Assoc of
       ...
       list ]
val config_to_json :
  config -> [> `Assoc of (string * [> `Bool of bool | `Int of int ]) list ]
