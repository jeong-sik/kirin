...
let parse_segment s =
  ...
  if len = 0
  then { raw = s; segment_type = Static ""; param_name = None }
  else if len > 4 && String.sub s 0 2 = "[[" && String.sub s (len - 2) 2 = "]]"
  then (
    (* [[optional]] parameter *)
    let name = String.sub s 2 (len - 4) in
    { raw = s; segment_type = Optional name; param_name = Some name })
  else if len > 5 && String.sub s 0 4 = "[..." && s.[len - 1] = ']'
  then (
    (* [...catchAll] parameter *)
  ...
  else { raw = s; segment_type = Static s; param_name = None }
;;

(** Parse full path into segments *)
let parse_path path =
  let parts = String.split_on_char '/' path in
  List.filter (fun s -> s <> "") parts |> List.map parse_segment
;;
...
let is_page_file name =
  Filename.check_suffix name ".vue" && not (String.contains name '_')
;;

(* Exclude _*.vue partials *)

(** Check if file is a layout *)
let is_layout_file name = Filename.check_suffix name ".vue"

(** Check if file is middleware *)
let is_middleware_file name =
  Filename.check_suffix name ".ts" || Filename.check_suffix name ".js"
;;
...
let file_to_route_path file_path =
  ...
  (* Handle index files *)
  let path =
    if Filename.basename without_ext = "index"
    then Filename.dirname without_ext
    else without_ext
  in
  (* Ensure leading slash *)
  if path = "." || path = "" then "/" else if path.[0] = '/' then path else "/" ^ path
;;
...
let discover_route ~base_dir file_path =
  ...
  let server_handler_path = Filename.concat "server/api" (route_path ^ ".ts") in
  let has_server_handler =
    Sys.file_exists (Filename.concat base_dir server_handler_path)
  in
  { file_path = full_path; route_path; file_type = Page; is_index; has_server_handler }
;;
...
let rec discover_routes_aux ~base_dir ~prefix dir =
  ...
  if not (Sys.file_exists full_dir && Sys.is_directory full_dir)
  then []
  else (
    ...
    List.concat_map
      (fun entry ->
         ...
         then discover_routes_aux ~base_dir ~prefix:rel_path entry
         else if is_page_file entry
         then [ discover_route ~base_dir rel_path ]
         else [])
      entries)
;;

(** Discover all routes from pages directory *)
let discover_routes pages_dir = discover_routes_aux ~base_dir:pages_dir ~prefix:"" ""
...
(** Create empty node *)
let empty_node segment = { segment; route = None; children = [] }
...
let rec insert_route segments route node =
  ...
  | seg :: rest ->
    let child =
      try List.find (fun n -> n.segment = seg) node.children with
      | Not_found -> empty_node seg
    in
