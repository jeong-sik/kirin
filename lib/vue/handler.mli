type mode = SPA | Hydration | SSR | Streaming
type config = {
  mode : mode;
...
val handle_hydration :
  title:string ->
  payload:Data.payload -> entry_script:string -> unit -> response
val handle_ssr : engine:Ssr.t -> url:string -> unit -> response
val handle_ssr_with_fallback :
  engine:Ssr.t ->
  url:string -> fallback:string -> unit -> response
...
val handle_streaming :
  ...
  shell:string -> chunks:Streaming.chunk list -> unit -> response
val handle_dev_proxy : port:int -> path:string -> unit -> response
val handle_api :
  handler:('a -> Action.action_result) ->
  request:'a -> unit -> response
...
val auth_middleware :
  ...
  streaming_response -> response option
...
val response_to_json :
  ...
  [> `Assoc of
       (string *
        [> `Assoc of (string * [> `String of string ]) list | `Int of int ])
       list ]
