...
(** Create shell chunk *)
let shell_chunk ~html = { chunk_type = Shell; content = html; id = None; priority = 0 }
...
(** Create script chunk *)
let script_chunk ~content = { chunk_type = Script; content; id = None; priority = 3 }

(** Create flush marker *)
let flush_chunk () = { chunk_type = Flush; content = ""; id = None; priority = 10 }

(** Create end marker *)
let end_chunk () = { chunk_type = End; content = ""; id = None; priority = 100 }
...
window.__NUXT_ISLANDS__['%s'] = %s;
      ...
      id
      chunk.content
  | Script -> Printf.sprintf "<script>%s</script>" chunk.content
  | Flush -> ""
  | End -> ""
;;
...
(** Start streaming *)
let start ctx =
  Eio.Mutex.use_rw ~protect:true ctx.mutex (fun () -> ctx.state <- Streaming)
;;
...
(** Finish streaming *)
let finish ctx =
  Eio.Mutex.use_rw ~protect:true ctx.mutex (fun () -> ctx.state <- Complete);
  encode_chunked_end ()
;;
...
let is_complete ctx =
  ...
  | Complete -> true
  | Error _ -> true
  | _ -> List.length ctx.suspense_pending = 0 && List.length ctx.islands_pending = 0
;;
...
let render_sse_event ev =
  ...
  Option.iter
    (fun e -> Buffer.add_string lines (Printf.sprintf "event: %s\n" e))
    ev.event_type;
  Option.iter
    (fun id -> Buffer.add_string lines (Printf.sprintf "id: %s\n" id))
    ev.event_id;
