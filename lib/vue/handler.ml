...
let request_info ~path ?(query = []) ?(headers = []) ?(method_ = "GET") ?body () =
  { path; query; headers; method_; body }
;;
...
let handle_hydration ~title ~payload ~entry_script () =
  let options = { Hydrate.default_options with title; scripts = [ entry_script ] } in
  let html =
    Hydrate.render
      ~options
      ~ssr_html:""
      ~payload:(Data.payload_script payload)
      ~entry_script
  in
  html_response html
;;
...
let handle_ssr ~engine ~url () =
  ...
  | Ok result ->
    { status = result.Ssr.status; headers = result.Ssr.headers; body = result.Ssr.html }
  | Error msg -> error_response ~status:500 msg
;;
...
let handle_streaming ~ctx ~shell ~chunks () =
  ...
  html_response
    ~headers:(Streaming.streaming_headers ())
...
let handle_api ~handler ~request () =
  ...
  | Action.ActionRedirect (status, url) -> redirect_response ~location:url ~status ()
  | Action.ActionError (status, message) ->
    { status
    ; headers = [ "Content-Type", "application/json" ]
    ; body =
        Yojson.Safe.to_string (`Assoc [ "error", `String message; "status", `Int status ])
    }
  | Action.ActionEmpty -> { status = 204; headers = []; body = "" }
...
let handle ~config ~request () =
  ...
  (* Check if API route *)
  if String.length url >= 4 && String.sub url 0 4 = "/api"
  then
    handle_api
      ~handler:(fun _ -> Action.ActionData (`Assoc [ "message", `String "OK" ]))
      ~request
      ()
    (* Dev mode *)
  else if config.dev_mode
  then handle_dev_proxy ~port:config.dev_port ~path:url ()
  (* Based on mode *)
  else (
    match config.mode with
    | SPA -> handle_spa ~config ~title:"Vue App" ()
    | Hydration ->
    ...
    | SSR ->
      let engine =
        Ssr.create (Option.value ~default:Ssr.default_config config.ssr_config)
      in
      handle_ssr ~engine ~url ()
    | Streaming ->
      let ctx = Streaming.create_context ?config:config.streaming_config () in
...
let catch_all_handler config = fun request -> handle ~config ~request ()
