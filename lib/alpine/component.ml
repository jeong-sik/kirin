...
(** Set init function *)
let with_init init component = { component with init = Some init }

(** Set destroy function *)
let with_destroy destroy component = { component with destroy = Some destroy }
...
let to_x_data component =
  ...
  let getters =
    component.getters
    |> List.map (fun g -> Printf.sprintf "get %s() { %s }" g.getter_name g.getter_body)
  in
  let init_fn =
    match component.init with
    | Some body -> [ Printf.sprintf "init() { %s }" body ]
    | None -> []
  in
  let destroy_fn =
    match component.destroy with
...
let to_x_data_ref component = Printf.sprintf "x-data=\"%s\"" component.name

(** Generate x-data with initial props override *)
let to_x_data_with_props component props =
  let overrides =
    props
    |> List.map (fun (k, v) -> Printf.sprintf "%s: %s" k (Yojson.Safe.to_string v))
    |> String.concat ", "
  in
...
let inline_data ~data component =
  let merged =
    ...
    |> List.map (fun p ->
      let value =
        match List.assoc_opt p.prop_name data with
        | Some v -> v
        | None -> p.prop_default
      in
