...
(** Create store *)
let create name = { name; properties = []; methods = [] }
...
let from_object name pairs =
  let props =
    pairs |> List.map (fun (k, v) -> { store_prop_name = k; store_prop_value = v })
  in
...
let accessor store prop = Printf.sprintf "$store.%s.%s" store.name prop
...
let magic_store store_name = Printf.sprintf "$store.%s" store_name
...
let to_persisted_store ?storage:(persist_storage = `Local) store =
  let storage_str =
    match persist_storage with
    | `Local -> "localStorage"
    | `Session -> "sessionStorage"
  in
  Printf.sprintf
    {|Alpine.store('%s', Alpine.$persist(%s).using(%s))|}
    store.name
    (to_object store)
    storage_str
;;
...
let property_to_json p =
  `Assoc [ "name", `String p.store_prop_name; "value", p.store_prop_value ]
;;
