...
let script_tag config =
  ...
  Printf.sprintf
    {|<script%s src="%s@%s/dist/cdn.min.js"></script>|}
    defer
    config.cdn_url
    config.version
;;
...
let plugin_scripts config =
  ...
  |> List.map (fun plugin ->
    Printf.sprintf
      {|<script src="%s@%s/dist/cdn.min.js"></script>|}
      (config.cdn_url ^ "-" ^ plugin)
      config.version)
  |> String.concat "\n"
;;

(** Generate x-cloak style *)
let cloak_style = {|<style>[x-cloak] { display: none !important; }</style>|}

(** Generate full head content *)
let head_scripts config =
  let cloak = if config.include_cloak_style then cloak_style ^ "\n" else "" in
  let plugins =
    match config.plugins with
    | [] -> ""
...
(** Empty state *)
let empty_state = { components = []; stores = []; initial_data = [] }
...
(** Add store *)
let with_store store state = { state with stores = state.stores @ [ store ] }
...
let init_script state =
  ...
  let data_assignments =
    ...
    |> List.map (fun (key, value) ->
      Printf.sprintf
        "window.__ALPINE_DATA__ = window.__ALPINE_DATA__ || {}; \
         window.__ALPINE_DATA__['%s'] = %s"
        key
...
let render_element ~tag ~directives ?(attrs = []) ?(children = "") () =
  let directive_attrs =
    directives |> List.map Directive.to_attribute |> String.concat " "
  in
  let other_attrs =
    attrs |> List.map (fun (k, v) -> Printf.sprintf "%s=\"%s\"" k v) |> String.concat " "
  in
  let all_attrs =
    String.concat " " (List.filter (fun s -> s <> "") [ directive_attrs; other_attrs ])
  in
...
let render_text_static ~data expr = eval_expression ~data expr
...
let render_for_static ~data ~item ~items ~template =
  ...
  | Some (`List arr) ->
    arr
    |> List.map (fun item_value ->
      let item_data =
        match item_value with
        | `Assoc pairs -> pairs
        | _ -> [ item, item_value ]
      in
      template item_data)
