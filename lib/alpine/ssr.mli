...
val render_for_static
  ...
  -> item:'c
  -> items:'a
  -> template:(('c * 'b) list -> string)
  -> string

val render_page : config:ssr_config -> state:ssr_state -> body:string -> string
val response : config:ssr_config -> state:ssr_state -> body:string -> Kirin.Response.t

val handler
  :  config:ssr_config
  -> get_state:('a -> ssr_state)
  -> render_body:('a -> ssr_state -> string)
  -> 'a
  -> Kirin.Response.t
