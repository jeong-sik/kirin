...
let status expected resp =
  if Testing_response.status resp <> expected then
...
let status_code expected resp =
  if Testing_response.status_code resp <> expected then
    fail (Printf.sprintf "Expected status %d but got %d"
      expected (Testing_response.status_code resp))

(** Assert success (2xx) *)
let success resp =
  if not (Testing_response.is_success resp) then
    fail (Printf.sprintf "Expected success (2xx) but got %d"
      (Testing_response.status_code resp))

(** Assert header exists *)
let header_exists name resp =
  if Option.is_none (Testing_response.header name resp) then
...
let json_path_exists path resp =
  ...
  | None -> fail "Expected JSON response body"
  | Some json ->
    if not (Testing_json_path.exists path json) then
...
let redirect_to expected resp =
  if not (Testing_response.is_redirect resp) then
