...
let log_handler : event_handler = function
  ...
  | Request_started { id; url } -> Printf.printf "[Worker] Request %d: %s\n%!" id url
  ...
  | Restarting { reason } -> Printf.printf "[Worker] Restarting: %s\n%!" reason
  | Stopped -> Printf.printf "[Worker] Stopped\n%!"
;;
...
module type POOL = sig
  ...
  val create : size:int -> config:config -> ?on_event:event_handler -> unit -> t
  val render : t -> url:string -> props:Yojson.Safe.t -> (string, string) result
...
module Render_cache = struct
  ...
  let set cache ~url ~props ~html ~ttl_seconds =
    let key = cache_key ~url ~props in
    let entry = { html; created_at = Unix.gettimeofday (); ttl_seconds } in
...
let placeholder_render ~url ~props =
  ...
  Printf.sprintf
    {|<div id="root" data-url="%s" data-props='%s'>Loading...</div>|}
    url
    props_json
;;
