...
let render options =
  ...
  addln (Meta.viewport ());
  (* Title *)
  if options.title <> "" then addln (Meta.title options.title);
  (* Meta tags from pairs *)
  if options.meta <> [] then addln (Meta.render_pairs options.meta);
  (* Stylesheets *)
  List.iter
    (fun href ->
       addln
         (Printf.sprintf {|<link rel="stylesheet" href="%s">|} (Meta.escape_html href)))
    options.styles;
  (* Extra head content *)
  ...
  addln
    (Printf.sprintf
       {|<div id="%s"%s></div>|}
       (Meta.escape_html options.root_id)
       root_class_attr);
  ...
  Option.iter
    (fun state -> addln (Data.script_tag ~var_name:"__DEHYDRATED_STATE__" state))
...
let shell ~title ?(lang = "en") ?(meta = []) ?(initial_data = None) ~manifest ~entry () =
  let styles = Manifest.css_for manifest entry in
  let script =
    match Manifest.resolve manifest entry with
    | Some file -> [ file ]
    | None -> [ entry ]
  in
  render
    { default_options with title; lang; meta; styles; scripts = script; initial_data }
;;
...
let simple_response ~title ?(meta = []) ?(initial_data = None) ~manifest ~entry () =
...
module Builder = struct
  ...
  let add_meta ~name ~content b = { b with meta = (name, content) :: b.meta }
  ...
  let body_attr ~name ~value b = { b with body_attrs = (name, value) :: b.body_attrs }
...
let with_manifest ~manifest ~entry options =
  ...
  let script =
    ...
    | None -> [ entry ]
  in
  { options with styles = options.styles @ styles; scripts = options.scripts @ script }
;;
