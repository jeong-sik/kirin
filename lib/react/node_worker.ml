...
let render worker ~url ~props =
  if worker.status = Worker.Dead
  then Result.Error "Worker is dead"
  else (
    ...
    (* Update stats *)
    let prev_total =
      worker.stats.avg_response_time_ms *. float_of_int worker.stats.requests_handled
    in
    worker.stats
    <- { worker.stats with
         requests_handled = worker.stats.requests_handled + 1
       ; avg_response_time_ms =
           (prev_total +. duration_ms) /. float_of_int (worker.stats.requests_handled + 1)
...
let restart worker =
  ...
  else worker.status <- Worker.Dead
;;
...
let render_with_restart worker ~url ~props =
  if should_restart worker then restart worker;
