...
let url ?(base_url = "") ~manifest src =
  ...
  | Some file -> if base_url = "" then "/" ^ file else base_url ^ "/" ^ file
  | None ->
    (* Fallback to original path if not in manifest *)
    if base_url = "" then "/" ^ src else base_url ^ "/" ^ src
;;
...
let script_tag ?(base_url = "") ?(module_ = true) ~manifest src =
  ...
  if module_
  then Printf.sprintf {|<script type="module" src="%s"></script>|} href
  else Printf.sprintf {|<script src="%s"></script>|} href
;;
...
let preload_all ?(base_url = "") ~manifest src =
  ...
  | Some e ->
    ...
    (* Preload CSS *)
    List.iter
      (fun css -> tags := preload_tag ~base_url ~as_type:"style" css :: !tags)
      e.css;
...
let body_tags ?(base_url = "") ~manifest ~entry () = script_tag ~base_url ~manifest entry
...
let make_config ?(base_url = "") manifest = { base_url; manifest }

(** Helper functions using config *)
let resolve_url config src = url ~base_url:config.base_url ~manifest:config.manifest src

let script config src = script_tag ~base_url:config.base_url ~manifest:config.manifest src
...
let inline_css ?(dir = "dist") css_path =
  ...
  with
  | Sys_error _ -> Printf.sprintf {|<link rel="stylesheet" href="/%s">|} css_path
;;
