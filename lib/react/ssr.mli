...
val render :
  t ->
  url:string -> ?props:Yojson.Safe.t -> unit -> (string, string) Result.t
...
val render_full :
  ...
  url:string ->
  ?props:Yojson.Safe.t ->
  unit -> (Protocol.render_result, string) Result.t
type stats = {
  total_renders : int;
  errors : int;
  timeouts : int;
...
val handler_with_fallback :
  fallback:string -> t -> Kirin.Request.t -> Kirin.Response.t
val render_hydrated :
  t ->
  url:string ->
  ?props:Yojson.Safe.t ->
  manifest:Manifest.t ->
  entry:string -> unit -> (string, string) Result.t
