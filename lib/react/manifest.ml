...
let parse json : t =
  match json with
  | `Assoc entries ->
    List.map (fun (src, entry_json) -> src, parse_entry ~src entry_json) entries
  | _ -> []
;;
...
let find (manifest : t) src = List.assoc_opt src manifest
...
let entries manifest = List.filter (fun (_, entry) -> entry.is_entry) manifest
      ...
      (fun (visited, files) s ->
         ...
         then visited, files
           ...
           | Some entry ->
             ...
             collect_imports visited' entry.imports |> fun (v, f) -> v, files' @ f
           | None -> visited, files))
      (visited, [])
      src_list
  in
...
let mem manifest src = List.mem_assoc src manifest

(** Get all source files in manifest *)
let sources manifest = List.map fst manifest
...
let pp fmt manifest =
  ...
  List.iter
    (fun (src, entry) ->
       ...
       if entry.css <> []
       then Format.fprintf fmt "    css: [%s]\n" (String.concat ", " entry.css);
       if entry.imports <> []
       then Format.fprintf fmt "    imports: [%s]\n" (String.concat ", " entry.imports))
    manifest;
  Format.fprintf fmt "}"
;;

let to_string manifest = Format.asprintf "%a" pp manifest
