...
(** Generate title tag *)
let title t = Printf.sprintf "<title>%s</title>" (escape_html t)
...
let canonical url = Printf.sprintf {|<link rel="canonical" href="%s">|} (escape_html url)
...
let robots ?(index = true) ?(follow = true) () =
  let directives =
    [ (if index then "index" else "noindex"); (if follow then "follow" else "nofollow") ]
  in
  Printf.sprintf {|<meta name="robots" content="%s">|} (String.concat ", " directives)
;;
...
let apple_touch_icon ?sizes href =
  ...
  | Some s ->
    Printf.sprintf
      ...
      (escape_html s)
      (escape_html href)
  | None -> Printf.sprintf {|<link rel="apple-touch-icon" href="%s">|} (escape_html href)
;;
...
let preconnect ?(crossorigin = false) href =
  if crossorigin
  then Printf.sprintf {|<link rel="preconnect" href="%s" crossorigin>|} (escape_html href)
  else Printf.sprintf {|<link rel="preconnect" href="%s">|} (escape_html href)
;;
...
let render_config config =
  ...
  if config.keywords <> []
  then add (meta ~name:"keywords" ~content:(String.concat ", " config.keywords));
  ...
  Option.iter
    (fun d -> add (twitter ~name:"description" ~content:d))
    config.twitter_description;
  Option.iter (fun i -> add (twitter ~name:"image" ~content:i)) config.twitter_image;
  (* Robots *)
  if not (config.robots_index && config.robots_follow)
  then add (robots ~index:config.robots_index ~follow:config.robots_follow ());
  (* Theme color *)
