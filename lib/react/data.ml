...
let named_script_tag ~name json = script_tag ~var_name:name json
...
let render_payload payload =
  let scripts = ref [] in
  Option.iter
    (fun d -> scripts := script_tag ~var_name:"__INITIAL_DATA__" d :: !scripts)
    payload.initial_data;
  Option.iter
    (fun d -> scripts := script_tag ~var_name:"__DEHYDRATED_STATE__" d :: !scripts)
    payload.dehydrated_state;
  Option.iter
    (fun d -> scripts := script_tag ~var_name:"__ROUTE_DATA__" d :: !scripts)
    payload.route_data;
  Option.iter
    (fun d -> scripts := script_tag ~var_name:"__USER_DATA__" d :: !scripts)
    payload.user_data;
  Option.iter
    (fun d -> scripts := script_tag ~var_name:"__CONFIG__" d :: !scripts)
    payload.config_data;
  List.rev !scripts |> String.concat "\n"
;;

(** Create payload from simple key-value pairs *)
let payload_from_assoc assoc = { empty_payload with initial_data = Some (`Assoc assoc) }
...
let script_tag_with_nonce ~nonce ?(var_name = "__INITIAL_DATA__") json =
  let safe_json = serialize json in
  Printf.sprintf
    {|<script nonce="%s">window.%s=%s</script>|}
    (Meta.escape_html nonce)
    var_name
    safe_json
;;
