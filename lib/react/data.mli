...
module QueryClient :
  sig
    type query = { query_key : string list; state : Yojson.Safe.t; }
    type dehydrated = {
      queries : query list;
    ...
    val add_query :
      key:string list -> data:Yojson.Safe.t -> dehydrated -> dehydrated
    val to_json :
      dehydrated ->
      [> `Assoc of (string * [> `List of Yojson.Safe.t list ]) list ]
    val to_script_tag : dehydrated -> string
  end
val script_tag_with_nonce :
  nonce:string -> ?var_name:string -> Yojson.Safe.t -> string
