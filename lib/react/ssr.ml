...
let create ?(on_event = Worker.null_handler) config =
  ...
  in
  let workers =
    Array.init config.workers (fun _ -> Node_worker.create ~on_event worker_config)
  in
  { workers
  ; config
  ; next_worker = 0
...
let get_worker engine =
  ...
  let rec find_idle i =
    ...
    then None
      ...
      if Node_worker.status w = Worker.Idle
      then (
...
let render_full engine ~url ?(props = `Assoc []) () =
  ...
  | Some worker ->
    ...
    (match Node_worker.send_request worker req with
     | Ok response -> Protocol.extract_render_result response
     | Error msg ->
       engine.total_errors <- engine.total_errors + 1;
       Result.Error msg)
;;
...
let stats engine =
  ...
  in
  let total_requests =
    Array.fold_left (fun acc s -> acc + s.Worker.requests_handled) 0 worker_stats
  in
  let avg_time =
    if total_requests > 0 then total_time /. float_of_int total_requests else 0.0
  in
  let active, idle =
    Array.fold_left
      (fun (a, i) w ->
  ...
  let cache_hit_rate =
    if cache_total > 0
    then float_of_int engine.cache_hits /. float_of_int cache_total
    else 0.0
  in
...
let health engine = Array.exists Node_worker.health_check engine.workers

(** Shutdown all workers *)
let shutdown engine = Array.iter Node_worker.close engine.workers

(** Clear render cache *)
let clear_cache engine = Worker.Render_cache.clear engine.cache
...
let handler engine =
  ...
  | Error msg -> Kirin.Response.text ~status:`Internal_server_error ("SSR Error: " ^ msg)
;;
