...
let default_dev_config = { host = "localhost"; port = 5173; https = false }
...
let serve_file file_path =
  ...
  with
  | Sys_error _ -> Kirin.Response.not_found ()
;;
...
let static_handler ~config ~manifest req =
  ...
  let file_path =
    if path = "/"
    then Filename.concat config.dist_dir "index.html"
    else Filename.concat config.dist_dir (String.sub path 1 (String.length path - 1))
  in
  if Sys.file_exists file_path && not (Sys.is_directory file_path)
  then serve_file file_path
  else if config.index_fallback
  then (
    (* SPA fallback *)
    let index_path = Filename.concat config.dist_dir "index.html" in
    if Sys.file_exists index_path
    then serve_file index_path
    else Kirin.Response.not_found ())
  else (
    ignore manifest;
...
let routes ?(dev_config = default_dev_config) ?(prod_config = default_prod_config) () =
  ...
  then (
    ...
    ignore dev_config;
    [])
  else static_routes ~config:prod_config ()
;;
...
let script_tag ~manifest ~entry () =
  if is_dev () then dev_script_tag ~entry () else Assets.script_tag ~manifest entry
;;

(** Detect appropriate CSS handling based on mode *)
let css_tags ~manifest ~entry () =
  if is_dev ()
  then "" (* Vite injects CSS in dev mode *)
  else Assets.css_tags ~manifest entry
;;
