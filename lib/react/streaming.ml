...
let create_session ?worker () =
  { state = Pending; chunks = []; is_complete = false; worker }
;;
...
let html_start ~title ~head_extra ~styles =
  let style_tags =
    List.map
      (fun href -> Printf.sprintf {|<link rel="stylesheet" href="%s">|} href)
      styles
    |> String.concat "\n"
  in
  Printf.sprintf
...
let html_end ~scripts ~initial_data =
  let script_tags =
    List.map
      (fun src -> Printf.sprintf {|<script type="module" src="%s"></script>|} src)
      scripts
    |> String.concat "\n"
  in
  let data_script =
...
let render_stream ~engine ~url ?(props = `Assoc []) ~write () =
  ...
  (* Send initial shell *)
  let shell_html = html_start ~title:"Loading..." ~head_extra:"" ~styles:[] in
  write shell_html;
  (* Render main content *)
...
module Progressive = struct
  ...
  let hydration_marker ~id = Printf.sprintf {|<!--$hydrate:%s-->|} id
...
module SSE = struct
  (** Format SSE event *)
  let event ~name ~data = Printf.sprintf "event: %s\ndata: %s\n\n" name data
  ...
  let chunk ~boundary_id ~html =
    let json =
      `Assoc [ "id", `String boundary_id; "html", `String (Base64.encode_string html) ]
    in
    event ~name:"chunk" ~data:(Yojson.Safe.to_string json)
  ;;

  (** Complete event *)
  let complete () = event ~name:"complete" ~data:""

  (** Error event *)
  let error msg = event ~name:"error" ~data:msg
  ...
  let response ~engine ~url ?(props = `Assoc []) () =
    ...
    match Ssr.render engine ~url ~props () with
    | Ok html ->
      let events = [ shell_ready html; complete () ] in
...
let kirin_stream_response ~engine ~url ?(props = `Assoc []) () =
  ...
  match Ssr.render engine ~url ~props () with
  | Ok html ->
    Kirin.Response.html html |> Kirin.Response.with_header "Transfer-Encoding" "chunked"
