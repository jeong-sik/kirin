...
let to_global_id type_name id = Base64.encode_string (type_name ^ ":" ^ id)
...
let get_args first after last before = { first; after; last; before }
...
let connection_from_list ?total_count list args =
  ...
  let decode_cursor c =
    try Base64.decode_exn c |> int_of_string with
    | Failure _ | Invalid_argument _ -> 0
  in
  let encode_cursor i = Base64.encode_string (string_of_int i) in
  (* Step 1-2: apply after/before to narrow the index range *)
  let after_idx =
    match args.after with
    | Some cursor -> decode_cursor cursor + 1
    | None -> 0
  in
  let before_idx =
    match args.before with
    | Some cursor -> decode_cursor cursor (* exclusive: up to but not including *)
    | None -> count
  in
  let range_start = after_idx in
  let range_end = min before_idx count in
  ...
  let range_start, range_end =
    ...
    | _ -> range_start, range_end
  in
  let sliced = list |> List.filteri (fun i _ -> i >= range_start && i < range_end) in
  let edges =
    List.mapi (fun i node -> { cursor = encode_cursor (range_start + i); node }) sliced
  in
...
let node_definitions () =
  let node_interface =
    Schema.interface "Node" ~fields:(fun _ ->
      [ Schema.abstract_field "id" ~typ:(Schema.non_null Schema.guid) ~args:Schema.Arg.[]
      ])
  in
  let node_field resolve_fn =
    Schema.field
      "node"
      ~typ:node_interface
      ~args:Schema.Arg.[ arg "id" ~typ:(Schema.Arg.non_null Schema.Arg.guid) ]
