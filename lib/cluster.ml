...
let create_socket _port =
  ...
  (* OCaml Unix doesn't expose SO_REUSEPORT directly in older versions, 
...
let spawn_worker id entry_point =
  ...
  | 0 ->
    ...
    (try entry_point () with
     ...
     | e -> Logger.error "[Worker %d] Crashed: %s" id (Printexc.to_string e));
    exit 0
  | pid ->
    (* Parent *)
    { pid; id }
...
let start ?(workers = 0) entry_point =
  ...
  let cluster = { workers = Hashtbl.create num_workers; master_pid = getpid () } in
