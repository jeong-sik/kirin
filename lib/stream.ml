...
let response ?(status = `OK) ?(headers = Http.Header.init ()) producer =
  ...
  let stream_producer stream =
    ...
    Fun.protect ~finally:(fun () -> Eio.Stream.add stream None) (fun () -> producer yield)
  in
...
let flow_response ?(status = `OK) ?(headers = Http.Header.init ()) _writer =
  let headers = Http.Header.add headers "transfer-encoding" "chunked" in
  (* Flow response not fully supported with new Producer type yet, fallback to String *)
...
let file_response ?filename ?content_type ?(chunk_size = default_chunk_size) path =
  ...
  let headers =
    Http.Header.init ()
    |> fun h ->
    Http.Header.add h "content-type" content_type
    |> fun h ->
    Http.Header.add
      h
      "content-disposition"
  ...
  let stream_producer stream =
    Fun.protect
      ~finally:(fun () -> Eio.Stream.add stream None)
      (fun () ->
...
let encode_chunk data =
  ...
  if len = 0 then "" else Printf.sprintf "%x\r\n%s\r\n" len data
;;
...
*)
let write_to_buffer _buf (_resp : Response.t) = () (* No-op *)
(** {1 Reading Streams} *)
...
let save_upload ~(request : Request.t) ~dest_path ?(chunk_size = default_chunk_size) () =
  ...
  (match Fs_compat.get_fs () with
   ...
   | None ->
     let fd =
       Unix.openfile dest_path [ Unix.O_WRONLY; Unix.O_CREAT; Unix.O_TRUNC ] 0o644
     in
     Fun.protect
       ~finally:(fun () -> Unix.close fd)
       (fun () ->
...
let with_header name value (t : Response.t) = Response.with_header name value t

(** Set content type *)
let with_content_type ct t = with_header "content-type" ct t

(** {1 Generator Helpers} *)

(** Create a producer that yields lines from a list *)
let of_lines lines = fun yield -> List.iter (fun line -> yield (line ^ "\n")) lines
...
let of_seq seq = fun yield -> Seq.iter yield seq
