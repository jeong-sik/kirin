...
let extract_boundary content_type =
  ...
  List.find_map
    (fun part ->
       ...
       if String.length part > 9 && String.sub part 0 9 = "boundary="
...
let parse_content_disposition header =
  ...
  List.iter
    (fun part ->
       let part = String.trim part in
       if String.length part > 5 && String.sub part 0 5 = "name="
       then (
         ...
         in
         name := Some value)
       else if String.length part > 9 && String.sub part 0 9 = "filename="
       then (
...
let find_substring haystack needle start =
  ...
  if needle_len = 0
  then Some start
  else if needle_len > haystack_len - start
  then None
  else (
    let rec search i =
...
let parse_part content =
  ...
  let content_disposition =
    List.assoc_opt "content-disposition" headers |> Option.value ~default:""
  in
  ...
  let rec split_parts acc start =
    ...
    | Some idx ->
      ...
      then (
        ...
        in
        if String.length part_content > 0
        then split_parts (part_content :: acc) (idx + String.length delimiter)
        else split_parts acc (idx + String.length delimiter))
      else (
        (* Skip to after delimiter *)
        let after_delim = idx + String.length delimiter in
        (* Check if this is end delimiter *)
        if after_delim + 2 <= String.length body && String.sub body after_delim 2 = "--"
        then List.rev acc
        else (
          (* Skip \r\n after delimiter *)
          let next_start =
            if
              after_delim + 2 <= String.length body
...
let field name t =
  List.find_map
    (fun part ->
       if part.name = name && part.filename = None then Some part.content else None)
    t.parts
;;
...
let file name t =
  List.find_map
    (fun part -> if part.name = name && part.filename <> None then Some part else None)
    t.parts
;;

(** Get all files *)
let files t = List.filter (fun part -> part.filename <> None) t.parts

(** Get all fields (non-file parts) *)
let fields t =
  List.filter_map
    (fun part -> if part.filename = None then Some (part.name, part.content) else None)
    t.parts
;;
