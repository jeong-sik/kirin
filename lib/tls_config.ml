...
(** Read file contents *)
let read_file path =
  try Ok (Fs_compat.load path) with
...
let make ~cert_file ~key_file ?ca_file ?(alpn = [ "http/1.1" ]) () =
  ...
  | Ok cert, Ok key ->
    let ca_cert =
      ...
      | None -> None
    in
    Ok { certificate = cert; private_key = key; ca_cert; alpn_protocols = alpn }
  | Error e, _ -> Error e
  | _, Error e -> Error e
;;
...
let from_pem ~certificate ~private_key ?ca_cert ?(alpn = [ "http/1.1" ]) () =
  { certificate; private_key; ca_cert; alpn_protocols = alpn }
;;
...
let validate config =
  ...
  in
  if not has_begin_cert
  then Error (Certificate_error "Invalid certificate format (missing PEM header)")
  else if not has_begin_key
  then Error (Key_error "Invalid private key format (missing PEM header)")
  else Ok ()
;;
