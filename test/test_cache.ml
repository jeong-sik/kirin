...
let test_create_custom () =
  ...
  @@ fun _env ->
  let cache =
    Kirin.Cache.create ~max_size:5 ~default_ttl:10.0 ~cleanup_interval:30.0 ()
  in
  let s = Kirin.Cache.stats cache in
...
let ttl_tests =
  ...
  ; test_case
      ...
      `Quick
      test_per_entry_ttl_overrides_default
  ]
;;
...
let test_get_or_set_miss () =
  ...
  let v =
    Kirin.Cache.get_or_set cache "k" (fun () ->
      called := true;
      "computed")
  in
...
let utility_tests =
  [ test_case "memoize" `Quick test_memoize; test_case "make_key" `Quick test_make_key ]
;;
