...
let test_router_static_match () =
  ...
  check
    ...
    "static match body"
    "matched"
    (response_body_to_string (Kirin.Response.body resp))
;;
...
let test_router_method_matching () =
  ...
  let routes =
    [ Kirin.get "/resource" get_handler; Kirin.post "/resource" post_handler ]
  in
  let router = Kirin.router routes in
  ...
  check
    ...
    "POST method"
    "POST"
    (response_body_to_string (Kirin.Response.body post_resp))
;;
...
let handler_tests = [ test_case "json api handler" `Quick test_json_api ]
...
let test_static_mime_type () =
  ...
  check
    ...
    (Kirin.Static.get_mime_type "app.js");
  check string "png" "image/png" (Kirin.Static.get_mime_type "logo.png");
  check
    string
    "json"
    "application/json; charset=utf-8"
    (Kirin.Static.get_mime_type "data.json");
  check
    string
    "unknown"
    "application/octet-stream"
    (Kirin.Static.get_mime_type "file.xyz")
;;
