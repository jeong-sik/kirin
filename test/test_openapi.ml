...
let spec_tests =
  ...
  ; ( "create spec with description"
    , `Quick
    , fun () ->
        let spec =
          Openapi.create ~title:"Test API" ~version:"1.0.0" ~description:"A test API" ()
        in
  ...
  ; ( "create spec with servers"
    ...
    , fun () ->
        ...
        in
        let spec = Openapi.create ~title:"Test API" ~version:"1.0.0" ~servers () in
...
let schema_tests =
  ...
  ; ( "object schema"
    ...
    , fun () ->
        let schema =
          Openapi.object_ [ "name", Openapi.string (); "age", Openapi.integer () ]
        in
  ...
  ; ( "object with required"
    , `Quick
    , fun () ->
        let schema = Openapi.object_ ~required:[ "name" ] [ "name", Openapi.string () ] in
...
let parameter_tests =
  [ ( "query parameter"
    , `Quick
    , fun () ->
        let param = Openapi.query_param ~name:"limit" ~schema:(Openapi.integer ()) () in
  ...
  ; ( "path parameter"
    , `Quick
    , fun () ->
        let param = Openapi.path_param ~name:"id" ~schema:(Openapi.string ()) () in
  ...
  ; ( "header parameter"
    ...
    , fun () ->
        let param =
          Openapi.header_param ~name:"X-Request-ID" ~schema:(Openapi.string ()) ()
        in
...
let response_tests =
  [ ( "json response"
    ...
    , fun () ->
        let resp =
          Openapi.response ~description:"Success" ~content:(Openapi.object_ []) ()
        in
  ...
  ; ( "operation with parameters"
    ...
    , fun () ->
        let op =
          Openapi.operation
            ~summary:"Get user"
            ~parameters:[ Openapi.path_param ~name:"id" ~schema:(Openapi.string ()) () ]
            ~responses:[ 200, Openapi.empty_response "OK" ]
            ()
        in
  ...
  [ ( "swagger ui"
    ...
    , fun () ->
        ...
        Alcotest.(check bool)
          ...
          true
          (try
             let _ = Str.search_forward (Str.regexp_string "swagger-ui") html 0 in
             true
           with
           | Not_found -> false) )
  ; ( "redoc"
    ...
    , fun () ->
        ...
        Alcotest.(check bool)
          ...
          true
          (try
             let _ = Str.search_forward (Str.regexp_string "redoc") html 0 in
             true
           with
           | Not_found -> false) )
  ]
