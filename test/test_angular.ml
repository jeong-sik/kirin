...
let test_route_def_with_guard () =
  let open Kirin_angular.Route_def in
  let route = ssr "/admin" "AdminComponent" |> with_guard "AuthGuard" in
...
let test_route_def_with_resolver () =
  let open Kirin_angular.Route_def in
  let route =
    ssr "/users/:id" "UserComponent" |> with_resolver ~key:"user" "UserResolver"
  in
...
let test_route_def_children () =
  ...
  let parent =
    empty "users"
    |> with_children [ ssr ":id" "UserComponent"; ssr "new" "NewUserComponent" ]
  in
...
let test_codegen_param_type () =
  ...
  let route =
    Route_def.ssr "/users/:id" "UserComponent"
...
let test_codegen_route_type () =
  let open Kirin_angular in
  let route = Route_def.ssr "/users" "UsersComponent" |> Route_def.with_name "users" in
...
let test_codegen_server_routes () =
  let open Kirin_angular in
  let routes =
    [ Route_def.ssr "/" "HomeComponent"; Route_def.csr "/dashboard" "DashboardComponent" ]
  in
