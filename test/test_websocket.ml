...
let test_opcode_of_int () =
  ...
  check
    ...
    "binary"
    true
    (Kirin.Websocket.opcode_of_int 0x2 = Some Kirin.Websocket.Binary);
  ...
  check
    ...
    "continuation"
    true
    (Kirin.Websocket.opcode_of_int 0x0 = Some Kirin.Websocket.Continuation);
...
let test_int_of_opcode () =
  ...
  check
    ...
    "continuation"
    0x0
    (Kirin.Websocket.int_of_opcode Kirin.Websocket.Continuation)
;;
...
let test_opcode_roundtrip () =
  ...
  List.iter
    (fun op ->
       ...
       check bool "roundtrip" true (Kirin.Websocket.opcode_of_int n = Some op))
    opcodes
;;
...
let test_close_code_values () =
  ...
  check
    ...
    "ProtocolError"
    1002
    (Kirin.Websocket.int_of_close_code Kirin.Websocket.ProtocolError);
  check
    int
    "UnsupportedData"
    1003
    (Kirin.Websocket.int_of_close_code Kirin.Websocket.UnsupportedData);
  check
    int
    "InvalidPayload"
    1007
    (Kirin.Websocket.int_of_close_code Kirin.Websocket.InvalidPayload);
  check
    int
    "PolicyViolation"
    1008
    (Kirin.Websocket.int_of_close_code Kirin.Websocket.PolicyViolation);
  check
    int
    "MessageTooBig"
    1009
    (Kirin.Websocket.int_of_close_code Kirin.Websocket.MessageTooBig);
  check
    int
    "InternalError"
    1011
    (Kirin.Websocket.int_of_close_code Kirin.Websocket.InternalError)
;;

let test_close_code_roundtrip () =
  let codes =
    [ Kirin.Websocket.Normal
    ; GoingAway
    ; ProtocolError
    ; UnsupportedData
    ; InvalidPayload
...
let test_decode_unmasked () =
  ...
  match Kirin.Websocket.decode_frame data with
  | Ok (frame, _) -> check string "payload" "Hi" frame.Kirin.Websocket.payload
  | Error msg -> fail ("unmasked decode: " ^ msg)
;;
...
let handshake_tests = [ test_case "accept key (RFC 6455)" `Quick test_accept_key ]
