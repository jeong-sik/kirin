...
let test_chained_builders () =
  let e =
    Kirin.Sse.event "tick" "data" |> Kirin.Sse.with_id "99" |> Kirin.Sse.with_retry 3000
  in
...
let test_encode_full_event () =
  let e =
    Kirin.Sse.event "msg" "payload" |> Kirin.Sse.with_id "42" |> Kirin.Sse.with_retry 5000
  in
  let encoded = Kirin.Sse.encode e in
...
let test_ping () = check string "ping format" ": ping\n\n" Kirin.Sse.ping
let ping_tests = [ test_case "ping comment" `Quick test_ping ]
