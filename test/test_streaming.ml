...
let test_empty_string_not_eos () =
  ...
  let producer yield =
    yield "before";
    yield "";
    (* Empty string should NOT be EOS *)
    yield "after"
  in
  let resp = Kirin.Stream.response producer in
  match Kirin.Response.body resp with
...
let test_producer_cancelled_on_switch_exit () =
  ...
  (try
   ...
   with
   | Failure msg when msg = "client disconnected" -> ());
...
let cancellation_tests =
  [ test_case
      "producer cancelled on switch exit"
      `Quick
      test_producer_cancelled_on_switch_exit
  ; test_case "producer normal completion" `Quick test_producer_normal_completion
  ]
;;

let () =
  run "Streaming" [ "Eio Stream", streaming_tests; "Cancellation", cancellation_tests ]
;;
