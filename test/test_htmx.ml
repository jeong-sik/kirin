...
let test_swap_to_string () =
  check
    ...
    "innerHTML"
    "innerHTML"
    (Kirin_htmx.Headers.swap_to_string Kirin_htmx.Headers.Inner_html);
  check
    string
    "outerHTML"
    "outerHTML"
    (Kirin_htmx.Headers.swap_to_string Kirin_htmx.Headers.Outer_html);
  check
    string
    "beforeend"
    "beforeend"
    (Kirin_htmx.Headers.swap_to_string Kirin_htmx.Headers.Before_end);
  check
    string
    "delete"
    "delete"
    (Kirin_htmx.Headers.swap_to_string Kirin_htmx.Headers.Delete)
;;
...
let test_hyperscript_on_click () =
  let script =
    Kirin_htmx.Hyperscript.on_click (Kirin_htmx.Hyperscript.toggle_class "active")
  in
...
let test_hyperscript_wait_then () =
  let script =
    Kirin_htmx.Hyperscript.wait_then "2s" (Kirin_htmx.Hyperscript.remove_class "loading")
  in
...
let test_oob_multi () =
  let html =
    Kirin_htmx.Oob.multi
      [ Kirin_htmx.Oob.notification ~id:"a" "A"; Kirin_htmx.Oob.notification ~id:"b" "B" ]
  in
...
let test_delete_with_confirm () =
  let html =
    Kirin_htmx.delete_with_confirm
      ~url:"/item/1"
      ~target:"#item-1"
      ~confirm_msg:"Delete?"
      "Delete"
...
let script_tests =
  [ "htmx_script", `Quick, test_htmx_script; "all_scripts", `Quick, test_all_scripts ]
;;
