  ...
  let contains_s haystack needle =
    ...
    then false
      ...
      let rec check i =
        if i > hl - nl
        then false
        else if sub haystack i nl = needle
        then true
        else check (i + 1)
      in
      check 0)
...
let assert_true cond msg = if not cond then failwith msg

let assert_contains haystack needle msg =
  if not (String.contains_s haystack needle)
  then
    failwith
      (Printf.sprintf
         "%s: '%s' not found in '%s'"
         msg
         needle
         (if String.length haystack > 100
          then String.sub haystack 0 100 ^ "..."
          else haystack))
;;
...
let test_hydrate () =
  ...
  test "render with SSR content" (fun () ->
    let html =
      Hydrate.render ~ssr_html:"<div>SSR Content</div>" Hydrate.default_options
    in
  ...
  test "lazy island" (fun () ->
    let html =
      Hydrate.lazy_island ~component_path:"./Counter.tsx" ~props_json:(`Assoc [])
    in
...
let test_protocol () =
  ...
  test "encode render request" (fun () ->
    let req : Protocol.render_request =
      { url = "/test"; props = `Assoc [ "key", `String "val" ]; meta = None }
    in
  ...
  test "decode success response" (fun () ->
    let json =
      {|{"jsonrpc":"2.0","id":1,"result":{"html":"<div>test</div>","head":"","status":200}}|}
    in
  ...
  test "decode redirect response" (fun () ->
    let json =
      {|{"jsonrpc":"2.0","id":1,"result":{"html":"","head":"","status":302,"redirect":"/login"}}|}
    in
    match Protocol.decode_render_response json with
    | Ok resp ->
  ...
  test "decode stream chunk - script" (fun () ->
    ...
    | Protocol.Script script ->
      assert_contains script "console" "should decode script chunk"
...
let test_router () =
  ...
  test "create route with children" (fun () ->
    ...
    let parent =
      Router.route ~path:"/users" ~component:"./Users.tsx" ~children:[ child ] ()
    in
...
let test_streaming () =
  ...
  test "shell with placeholder" (fun () ->
    let shell =
      Streaming.shell_with_placeholder
  ...
  test "replacement script" (fun () ->
    let script =
      Streaming.replacement_script ~suspense_id:"s1" ~content:"<div>Done</div>"
    in
  ...
  test "progressive marker" (fun () ->
    let marker =
      Streaming.progressive_marker ~component_id:"c1" ~priority:Streaming.Visible
    in
  ...
  test "error boundary fallback" (fun () ->
    let fallback =
      Streaming.error_boundary_fallback ~boundary_id:"eb1" ~error_message:"Test error"
    in
...
let test_handler () =
  ...
  test "is_vite_request - regular path" (fun () ->
    assert_true
      (not (Handler.is_vite_request "/api/users"))
      "should not detect regular path");
  ...
  test "data_routes creates list" (fun () ->
    let loaders = [ ("/users", fun ~url:_ ~params:_ -> Router.Data (`List [])) ] in
...
let test_facade () =
  ...
  test "ssr_shell helper" (fun () ->
    let html =
      Kirin_solid.ssr_shell ~title:"Test" ~entry_script:"/app.js" ~ssr_html:"<div/>" ()
    in
