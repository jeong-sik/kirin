...
let test_cookie_parse () =
  ...
  check
    ...
    "session cookie"
    (Some "abc123")
    (List.assoc_opt "session" cookies);
...
let test_cookie_build_simple () =
  let header =
    Kirin.Cookie.build_set_cookie "session" "xyz" Kirin.Cookie.default_attributes
  in
...
let test_cookie_set_on_response () =
  let resp = Kirin.html "<h1>Test</h1>" |> Kirin.set_cookie "token" "abc123" in
...
let test_cookie_delete () =
  let resp = Kirin.html "<h1>Test</h1>" |> Kirin.delete_cookie "session" in
  let set_cookie = Kirin.Response.header "set-cookie" resp in
  match set_cookie with
  | Some v ->
