...
let test_jwt_decode () =
  ...
  | Ok decoded -> check (option string) "sub matches" (Some "user-123") decoded.claims.sub
...
let test_password_different_hashes () =
  ...
  check bool "same password produces different hashes (salt)" true (hash1 <> hash2);
  check
    bool
...
let test_password_strength () =
  check
    (testable (Fmt.of_to_string Kirin_auth.Password.strength_to_string) ( = ))
    "weak password"
    Kirin_auth.Password.Weak
    (Kirin_auth.Password.check_strength "abc");
  check
    (testable (Fmt.of_to_string Kirin_auth.Password.strength_to_string) ( = ))
    "strong password"
    Kirin_auth.Password.Strong
...
let test_session_destroy () =
  ...
  check
    (option string)
    "destroyed session returns None"
    None
...
let test_oauth2_provider_google () =
  let p =
    Kirin_auth.Oauth2.Provider.google
      ~client_id:"test-id"
...
let test_oauth2_provider_github () =
  let p =
    Kirin_auth.Oauth2.Provider.github
      ~client_id:"test-id"
      ~client_secret:"test-secret"
      ~redirect_uri:"http://localhost/callback"
  in
...
let test_oauth2_pkce () =
  ...
  check
    ...
    (String.length url > 0
     &&
     try
       ignore (Str.search_forward (Str.regexp "code_challenge=") url 0);
       true
     with
     | Not_found -> false);
  check bool "state non-empty" true (String.length state > 0);
...
let test_middleware_set_get_auth_info () =
  let req = make_req "/" in
  check (option string) "no auth initially" None (Kirin_auth.Middleware.get_user_id req);
...
let test_middleware_jwt_missing_token () =
  ...
  check int "401 on missing token" 401 (Http.Status.to_int (Kirin.Response.status resp))
;;
...
let test_middleware_concurrent_isolation () =
  ...
  in
  Eio.Fiber.both
    (fun () -> ignore (Kirin_auth.Middleware.jwt ~secret:jwt_secret handler_a req_a))
    (fun () -> ignore (Kirin_auth.Middleware.jwt ~secret:jwt_secret handler_b req_b));
...
let make_test_request ?(meth = `GET) ?(headers = []) ?(body = "") path =
  ...
  let body_source =
    ...
    |> Eio.Buf_read.of_flow ~max_size:(max 1024 (String.length body + 1))
  in
  Kirin.Request.make ~raw ~body_source
;;
...
let test_any_of_no_double_execution () =
  ...
  let handler _req =
    ...
    Kirin.Response.json ~status:`Not_found (`Assoc [ "error", `String "User not found" ])
  in
  let auth_jwt = Kirin_auth.Middleware.jwt ~secret in
  let protected = Kirin_auth.Middleware.any_of [ auth_jwt ] handler in
  let req =
    make_test_request ~headers:[ "authorization", "Bearer " ^ token ] "/api/user"
  in
  let resp = protected req in
...
let test_any_of_no_execution_on_auth_failure () =
  ...
  check int "status is 401" 401 (Http.Status.to_int (Kirin.Response.status resp))
;;
...
let any_of_tests =
  ...
  ; test_case
      "no execution on auth failure"
      `Quick
      test_any_of_no_execution_on_auth_failure
