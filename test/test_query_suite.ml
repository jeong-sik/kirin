...
let test_query_select () =
  let q = Q.(select [ "id"; "name"; "email" ] |> from "users" |> build) in
...
let test_query_select_distinct () =
  let q = Q.(select_distinct [ "name" ] |> from "users" |> build) in
  check string "distinct sql" "SELECT DISTINCT name FROM users" q.sql
;;

let test_query_where () =
  let q = Q.(select [ "*" ] |> from "users" |> where "age > ?" [ Int 18 ] |> build) in
...
let test_query_order_by () =
  let q =
    Q.(
      ...
      |> build)
  in
  check string "order by sql" "SELECT * FROM users ORDER BY name ASC, age DESC" q.sql
;;

let test_query_limit_offset () =
  let q = Q.(select [ "*" ] |> from "users" |> limit 10 |> offset 20 |> build) in
  check string "limit offset sql" "SELECT * FROM users LIMIT 10 OFFSET 20" q.sql
;;
...
let test_query_insert () =
  let q =
    Q.(
      ...
      |> build)
  in
  check string "insert sql" "INSERT INTO users (name, email, age) VALUES (?, ?, ?)" q.sql;
...
let test_query_update () =
  let q =
    Q.(
      ...
      |> build)
  in
  check string "update sql" "UPDATE users SET name = ?, age = ? WHERE id = ?" q.sql;
  check int "3 params" 3 (List.length q.params)
;;

let test_query_delete () =
  let q = Q.(delete_from "users" |> where "id = ?" [ Int 1 ] |> build) in
...
let test_query_returning () =
  let q =
    Q.(
      ...
      |> build)
  in
  check string "returning sql" "INSERT INTO users (name) VALUES (?) RETURNING id" q.sql
;;
...
let test_query_paginate () =
  let q = Q.(select [ "*" ] |> from "users" |> paginate ~page:3 ~per_page:10 |> build) in
  check string "paginate sql" "SELECT * FROM users LIMIT 10 OFFSET 20" q.sql
;;

let test_query_where_id () =
  let q = Q.(select [ "*" ] |> from "users" |> where_id 42 |> build) in
  check string "where_id sql" "SELECT * FROM users WHERE id = ?" q.sql
;;

let test_query_where_in () =
  let q =
    Q.(select [ "*" ] |> from "users" |> where_in "id" [ Int 1; Int 2; Int 3 ] |> build)
  in
  check string "where_in sql" "SELECT * FROM users WHERE id IN (?, ?, ?)" q.sql;
  check int "3 params" 3 (List.length q.params)
;;

let test_query_where_null () =
  let q = Q.(select [ "*" ] |> from "users" |> where_null "deleted_at" |> build) in
  check string "where_null sql" "SELECT * FROM users WHERE deleted_at IS NULL" q.sql
;;

let test_query_where_like () =
  let q = Q.(select [ "*" ] |> from "users" |> where_like "name" "%john%" |> build) in
  check string "where_like sql" "SELECT * FROM users WHERE name LIKE ?" q.sql
;;

(* Issue #43: or_where as first clause should produce valid SQL *)
let test_query_or_where_first_clause () =
  let q = Q.(select [ "*" ] |> from "users" |> or_where "age > ?" [ Int 18 ] |> build) in
  (* Must produce "WHERE age > ?", not "WHERE OR age > ?" *)
  check string "or_where first clause sql" "SELECT * FROM users WHERE age > ?" q.sql
;;
...
let test_query_or_where_first_clause_update () =
  ...
  check
    bool
    "update or_where first clause valid"
    true
    (not
       (String.sub q.sql 0 (String.length q.sql)
        |> fun s ->
        try
          ignore (Str.search_forward (Str.regexp "WHERE OR") s 0);
          true
        with
        | Not_found -> false))
;;
...
let test_query_or_where_first_clause_delete () =
  let q = Q.(delete_from "users" |> or_where "status = ?" [ String "banned" ] |> build) in
  check
    string
    "delete or_where first clause sql"
...
let query_tests =
  ...
  ; test_case
      "or_where first clause update"
      `Quick
      test_query_or_where_first_clause_update
  ; test_case
      "or_where first clause delete"
      `Quick
      test_query_or_where_first_clause_delete
  ]
;;
...
let test_db_error_to_string () =
  ...
  check string "timeout" "Database operation timed out" (D.error_to_string D.Timeout)
;;
...
let test_db_default_config () =
  ...
  check
    ...
    "connect_timeout"
    (Some 30.0)
    D.default_config.connect_timeout
;;
