...
let test_grpc_service_with_unary () =
  let svc =
    Kirin.Grpc.service "test.Greeter"
...
let test_grpc_interceptor () =
  let _interceptor =
    Kirin.Grpc.make_interceptor "test" (fun ctx next ->
      (* Just pass through *)
      next ctx)
  in
  (* Interceptor created *)
...
let test_grpc_status_codes () =
  ...
  check
    ...
    "not found code"
    true
    (Kirin.Grpc.StatusCode.not_found = Grpc_core.Status.Not_found);
  check
    bool
    "internal code"
    true
    (Kirin.Grpc.StatusCode.internal = Grpc_core.Status.Internal)
;;
