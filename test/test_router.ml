...
let test_parse_static () =
  ...
  check
    ...
    "first is Static"
    true
    (match List.nth segs 0 with
     | Kirin.Router.Static "users" -> true
     | _ -> false);
  check
    bool
    "second is Static"
...
let test_match_multi_params () =
  let segs =
    [ Kirin.Router.Static "users"
    ; Kirin.Router.Param "uid"
    ; Kirin.Router.Static "posts"
    ; Kirin.Router.Param "pid"
    ]
  in
...
let test_dispatch_post () =
  let routes =
    [ Kirin.Router.post "/items" (fun _req -> Kirin.Response.text "created") ]
  in
...
let test_dispatch_put () =
  let routes =
    [ Kirin.Router.put "/items/:id" (fun req ->
        let id =
          match Kirin.Request.param "id" req with
          | Some v -> v
          | None -> "?"
        in
...
let test_dispatch_delete () =
  let routes =
    [ Kirin.Router.delete "/items/:id" (fun _req -> Kirin.Response.text "deleted") ]
  in
...
let test_scope_with_middleware () =
  let add_header : Kirin.Middleware.t =
    fun handler req ->
    let resp = handler req in
