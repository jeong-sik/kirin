...
let test_graphql_object_type () =
  ...
  | Ok (`Response json) ->
    let expected =
      `Assoc
        [ "data", `Assoc [ "user", `Assoc [ "id", `String "1"; "name", `String "Alice" ] ]
        ]
    in
    check
      Alcotest.string
...
let test_graphql_parse_request () =
  let body =
    {|{"query":"{ hello }","operationName":"GetHello","variables":{"id":"1"}}|}
  in
...
let test_graphql_error_helpers () =
  ...
  check
    (option (list (pair Alcotest.int Alcotest.int)))
    "locations"
    (Some [ 10, 5 ])
    err_loc.locations;
  let err_path = GQL.error_with_path "Field error" [ "user"; "name" ] in
  check (option (list Alcotest.string)) "path" (Some [ "user"; "name" ]) err_path.path
;;
...
let test_graphql_playground () =
  ...
  check
    ...
    "contains doctype"
    true
    (String.length body > 100 && String.sub body 0 15 = "<!DOCTYPE html>")
;;
...
let test_graphql_intlit_small () =
  ...
  check bool "small Intlit parsed as Int 42" true (result = `Int 42)
;;
...
let test_introspection_enabled_get () =
  ...
  check int "GET returns 200 when enabled" 200 (Kirin.Response.status_code resp)
;;
...
let test_introspection_disabled_get () =
  ...
  let req = make_test_request ~meth:`GET "/graphql" in
  let resp = mw fallback req in
  check int "GET returns 403 when disabled" 403 (Kirin.Response.status_code resp)
;;
...
let test_introspection_disabled_post_schema_query () =
  ...
  let fallback _req = Kirin.Response.text "fallback" in
  let body =
    Yojson.Safe.to_string (`Assoc [ "query", `String "{ __schema { types { name } } }" ])
  in
  let req =
    make_test_request
      ~meth:`POST
  ...
  check
    int
    "POST __schema returns 403 when disabled"
    403
...
let test_introspection_disabled_post_type_query () =
  ...
  let body =
    Yojson.Safe.to_string
      (`Assoc [ "query", `String "{ __type(name: \"Query\") { name } }" ])
  in
  let req =
    make_test_request
      ~meth:`POST
  ...
  in
  let resp = mw fallback req in
  check int "POST __type returns 403 when disabled" 403 (Kirin.Response.status_code resp)
;;
...
let test_introspection_disabled_post_normal_query () =
  ...
  let mw = Kirin.Graphql.middleware ~enable_introspection:false schema in
  let fallback _req = Kirin.Response.text "fallback" in
  let body = Yojson.Safe.to_string (`Assoc [ "query", `String "{ hello }" ]) in
  let req =
    make_test_request
      ~meth:`POST
      ~headers:[ "content-type", "application/json" ]
  ...
  check int "POST normal query returns 200" 200 (Kirin.Response.status_code resp)
;;
...
let test_introspection_default_enabled () =
  ...
  let req = make_test_request ~meth:`GET "/graphql" in
  let resp = mw fallback req in
  check int "GET returns 200 by default" 200 (Kirin.Response.status_code resp)
;;
...
let introspection_tests =
  ...
  ; test_case
      "disabled POST __schema"
      `Quick
      test_introspection_disabled_post_schema_query
  ; test_case "disabled POST __type" `Quick test_introspection_disabled_post_type_query
  ; test_case
      "disabled POST normal query"
      `Quick
      test_introspection_disabled_post_normal_query
