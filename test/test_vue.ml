...
let test_route_def_param () =
  let open Kirin_vue.Route_def in
  let route = page "/users/[id]" |> with_param "id" String in
...
let test_route_def_catch_all () =
  let open Kirin_vue.Route_def in
  let route = page "/docs/[...slug]" |> with_param "slug" Slug in
  let p = List.hd route.params in
...
let test_route_def_optional () =
  let open Kirin_vue.Route_def in
  let route = page "/user/[[id]]" |> with_param "id" (Optional String) in
  let p = List.hd route.params in
...
let test_route_def_middleware () =
  ...
  let route = page "/admin" |> with_middleware auth_mw in
...
let test_route_def_layout () =
  let open Kirin_vue.Route_def in
  let route = page "/dashboard" |> with_layout "admin" in
...
let test_protocol_decode_success () =
  let open Kirin_vue.Protocol in
  let json =
    {|{"jsonrpc":"2.0","id":1,"result":{"html":"<div>test</div>","head":"","payload":"{}"}}|}
  in
...
let test_codegen_param_type () =
  ...
  let route =
    Kirin_vue.Route_def.page "/users/[id]"
    |> Kirin_vue.Route_def.with_param "id" Kirin_vue.Route_def.String
  in
