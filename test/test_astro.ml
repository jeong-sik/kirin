...
let test_island_directive_string () =
  ...
  check
    ...
    (Kirin_astro.Island.directive_to_string
       (Kirin_astro.Island.ClientMedia "(max-width: 768px)"))
;;
...
let test_island_framework_string () =
  ...
  check string "vue" "vue" (Kirin_astro.Island.framework_to_string Kirin_astro.Island.Vue);
  check
    string
    "svelte"
    "svelte"
...
let test_island_hydration_script () =
  ...
  check bool "contains IntersectionObserver" true (String.length script > 0)
;;
...
let test_island_to_json () =
  ...
  match json with
  | `Assoc fields ->
    check bool "has component field" true (List.mem_assoc "component" fields)
...
let test_route_is_dynamic () =
  ...
  check bool "about is not dynamic" false (Kirin_astro.Route_def.is_dynamic static);
  check bool "blog slug is dynamic" true (Kirin_astro.Route_def.is_dynamic dynamic)
;;
...
let test_file_router_segment_static () =
  ...
  | Kirin_astro.File_router.Static name -> check string "segment name" "about" name
  | _ -> fail "Expected Static segment"
;;

let test_file_router_segment_dynamic () =
  match Kirin_astro.File_router.parse_segment "[slug]" with
  | Kirin_astro.File_router.Dynamic name -> check string "param name" "slug" name
...
let test_file_router_segment_catchall () =
  ...
  | Kirin_astro.File_router.CatchAll name -> check string "param name" "path" name
  | _ -> fail "Expected CatchAll segment"
;;

let test_file_router_file_to_path () =
  check string "index" "/" (Kirin_astro.File_router.file_to_route_path "index.astro");
  check string "about" "/about" (Kirin_astro.File_router.file_to_route_path "about.astro");
  check
    string
    "nested"
    "/blog/post"
...
let test_file_router_extension () =
  check bool "is astro" true (Kirin_astro.File_router.should_include "index.astro");
  check bool "is mdx" true (Kirin_astro.File_router.should_include "post.mdx");
  check bool "is not ts" false (Kirin_astro.File_router.should_include "utils.ts")
;;
...
let test_content_string_field () =
  ...
  match field with
  | Kirin_astro.Content.StringField { required } -> check bool "required" true required
...
let test_content_schema () =
  let schema =
    Kirin_astro.Content.define_schema
      "blog"
      [ "title", Kirin_astro.Content.string_ (); "draft", Kirin_astro.Content.bool_ () ]
  in
...
let test_content_collection () =
  let schema =
    Kirin_astro.Content.define_schema "blog" [ "title", Kirin_astro.Content.string_ () ]
  in
...
let test_content_entry () =
  let entry =
    Kirin_astro.Content.create_entry
      ~id:"hello-world"
      ~collection:"blog"
...
let test_meta_render () =
  ...
  check bool "contains meta" true (String.length html > 0)
;;

let test_meta_head () =
  let head = Kirin_astro.Meta.empty |> Kirin_astro.Meta.with_title "Test" in
...
let test_integration_react () =
  ...
  check
    ...
    "name"
    "react"
    i.Kirin_astro.Integration.config.Kirin_astro.Integration.name
;;
...
let test_integration_generate_config () =
  let config =
    Kirin_astro.Integration.generate_config [ Kirin_astro.Integration.react ]
  in
...
let test_view_transitions_enable () =
  let config =
    Kirin_astro.View_transitions.default_config
...
let test_view_transitions_element () =
  let elem =
    Kirin_astro.View_transitions.element
      ~name:"header"
...
let test_view_transitions_attrs () =
  ...
  check bool "has transition:name" true (String.length attrs > 0)
;;
...
let test_protocol_encode_request () =
  ...
  check bool "contains jsonrpc" true (String.length encoded > 0 && encoded.[0] = '{')
;;
...
let test_protocol_decode_success () =
  ...
  | Kirin_astro.Protocol.Success { id; _ } -> check int "id" 1 id
...
let test_ssr_with_fallback () =
  ...
  Kirin_astro.Ssr.shutdown engine;
  let html =
    Kirin_astro.Ssr.render_with_fallback engine ~url:"/test" ~fallback:"<h1>Fallback</h1>"
  in
...
let test_handler_is_static_asset () =
  check bool "js is static" true (Kirin_astro.Handler.is_static_asset "/app.js");
  check bool "css is static" true (Kirin_astro.Handler.is_static_asset "/style.css");
  check bool "html is not static" false (Kirin_astro.Handler.is_static_asset "/page.html")
;;
...
let test_codegen_schema_to_ts () =
  let schema =
    Kirin_astro.Content.define_schema
      "Blog"
      [ "title", Kirin_astro.Content.string_ (); "views", Kirin_astro.Content.number () ]
  in
  ...
  check
    ...
    (ts
     ...
     |> List.hd
     |> fun s -> String.length s > 0 && String.sub s 0 6 = "export")
;;

let test_codegen_route_to_ts () =
  let route = Kirin_astro.Route_def.static "/blog/[slug]" in
  let ts = Kirin_astro.Codegen.route_to_ts route in
  check bool "has slug param" true (String.length ts > 0)
;;
...
let test_codegen_generate_types () =
  ...
  let route = Kirin_astro.Route_def.static "/blog/[slug]" in
  let ts =
    Kirin_astro.Codegen.generate_types_file ~routes:[ route ] ~collections:[ collection ]
  in
  check bool "has Routes interface" true (String.length ts > 0)
;;

let test_codegen_routes_file () =
  let route = Kirin_astro.Route_def.static "/about" in
  let ts = Kirin_astro.Codegen.generate_routes_file [ route ] in
  check bool "has routes array" true (String.length ts > 0)
;;

let test_codegen_astro_config () =
  let ts =
    Kirin_astro.Codegen.generate_astro_config
      ~integrations:[ Kirin_astro.Integration.react ]
      ~output:Kirin_astro.Handler.Hybrid
  in
  check bool "has defineConfig" true (String.length ts > 0)
;;

let test_codegen_env_dts () =
  let ts = Kirin_astro.Codegen.generate_env_dts () in
  check bool "has ImportMetaEnv" true (String.length ts > 0)
;;
...
let test_facade_vue_island () =
  ...
  check bool "is vue" true (island.Kirin_astro.Island.framework = Kirin_astro.Island.Vue)
;;

let test_facade_client_directives () =
  check bool "client_load" true (Kirin_astro.client_load = Kirin_astro.Island.ClientLoad);
  check bool "client_idle" true (Kirin_astro.client_idle = Kirin_astro.Island.ClientIdle);
  check
    bool
    "client_visible"
    true
