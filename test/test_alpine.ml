...
let test_directive_on_modifiers () =
  let d =
    Kirin_alpine.Directive.on
      ~modifiers:[ Kirin_alpine.Directive.Prevent; Kirin_alpine.Directive.Stop ]
      "submit"
      "handle"
  in
  let attr = Kirin_alpine.Directive.to_attribute d in
...
let test_component_with_property () =
  let c =
    Kirin_alpine.Component.(
      create "test" |> with_property ~default:(`Int 0) `Number "count")
  in
...
let test_component_with_method () =
  let c =
    Kirin_alpine.Component.(create "test" |> with_method "increment" [] "this.count++")
  in
...
let test_component_inline_data () =
  let c =
    Kirin_alpine.Component.(
      create "user" |> with_property ~default:(`String "") `String "name")
  in
...
let test_store_with_property () =
  let s = Kirin_alpine.Store.(create "app" |> with_property "theme" (`String "dark")) in
  check int "has property" 1 (List.length s.properties)
;;

let test_store_from_object () =
  let s =
    Kirin_alpine.Store.from_object
      "config"
      [ "debug", `Bool true; "version", `String "1.0" ]
  in
  check int "has properties" 2 (List.length s.properties)
;;

let test_store_to_object () =
  let s = Kirin_alpine.Store.(create "app" |> with_property "count" (`Int 5)) in
...
let test_store_hydration_script () =
  let script =
    Kirin_alpine.Store.hydration_script ~store_name:"app" ~data:[ "ready", `Bool true ]
  in
...
let test_ssr_render_element () =
  let html =
    Kirin_alpine.Ssr.render_element
      ~tag:"div"
...
let test_ssr_render_cloaked () =
  let html =
    Kirin_alpine.Ssr.render_cloaked
      ~tag:"div"
...
let test_ssr_eval_expression () =
  let result =
    Kirin_alpine.Ssr.eval_expression ~data:[ "name", `String "Alice" ] "name"
  in
...
let test_facade_modifiers () =
  let d =
    Kirin_alpine.on
      ~modifiers:[ Kirin_alpine.prevent; Kirin_alpine.stop ]
      "click"
      "handle"
  in
  let attr = Kirin_alpine.Directive.to_attribute d in
