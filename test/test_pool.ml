...
let test_use_releases_on_exception () =
  ...
  @@ fun _env ->
  let pool = make_pool ~max_size:5 () in
  (try Kirin.Pool.use pool (fun _conn -> failwith "boom") with
   | Failure _ -> ());
  let s = Kirin.Pool.stats pool in
...
let test_use_pooled () =
  ...
  let pool = make_pool ~max_size:5 () in
  let info =
    Kirin.Pool.use_pooled pool (fun p -> p.Kirin.Pool.conn, p.Kirin.Pool.use_count)
  in
...
let error_tests = [ test_case "error_to_string" `Quick test_error_strings ]
...
let info_tests = [ test_case "connection_info" `Quick test_connection_info ]
