...
let test_jsonrpc_response_success () =
  let resp = J.success_response ~id:(J.Int 1) (`Assoc [ "result", `String "ok" ]) in
  check bool "has result" true (Option.is_some resp.result)
;;

let test_jsonrpc_response_error () =
  let err = J.make_error ~code:J.Invalid_params ~message:"Bad params" () in
...
let test_jsonrpc_notification () =
  let notif = J.make_notification ~method_:"notifications/test" ~params:`Null () in
...
let test_schema_string () =
  ...
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp "string") json 0);
       true
     with
     | Not_found -> false)
;;
...
let test_schema_number () =
  ...
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp "number") json 0);
       true
     with
     | Not_found -> false)
;;
...
let test_schema_object () =
  let schema = S.object_ ~required:[ "name" ] [ "name", S.string (); "age", S.int () ] in
  let json = Yojson.Safe.to_string schema in
  check
    bool
    "has type object"
    true
    (try
       ignore (Str.search_forward (Str.regexp "object") json 0);
       true
     with
     | Not_found -> false)
;;
...
let test_schema_array () =
  ...
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp "array") json 0);
       true
     with
     | Not_found -> false)
;;
...
let test_server_add_tool () =
  let server = Kirin_mcp.Server.create () in
  Kirin_mcp.Server.add_tool
    server
    ~name:"greet"
    ~description:"Greet someone"
    ~schema:(S.object_ ~required:[ "name" ] [ "name", S.string () ])
    ~handler:
      (Kirin_mcp.Server.Sync
         (fun params ->
...
let test_server_call_tool () =
  ...
  Kirin_mcp.Server.add_tool
    ...
    ~schema:(S.object_ ~required:[ "a"; "b" ] [ "a", S.int (); "b", S.int () ])
    ~handler:
      (Kirin_mcp.Server.Sync
         (fun params ->
    ...
    ();
  match
    Kirin_mcp.Server.call_tool
      server
      ~ctx
      ~name:"add"
      ~arguments:(Some (`Assoc [ "a", `Int 2; "b", `Int 3 ]))
  with
  | Ok result ->
    (match result.content with
     | [ P.Text s ] -> check string "result is 5" "5" s
...
let test_server_add_prompt () =
  let server = Kirin_mcp.Server.create () in
  Kirin_mcp.Server.add_prompt
    server
    ~name:"greeting"
    ~description:"A greeting prompt"
    ~arguments:
      [ { P.name = "style"; description = Some "Greeting style"; required = Some false } ]
    ();
    ...
    J.make_request
      ...
      ~params:
        (`Assoc
            ...
            ; "capabilities", `Assoc []
            ; ( "clientInfo"
              , `Assoc [ "name", `String "test-client"; "version", `String "1.0" ] )
            ])
      ()
  in
...
let test_server_handle_tools_list () =
  ...
  Kirin_mcp.Server.add_tool
    ...
    ~handler:(Kirin_mcp.Server.Sync (fun _ -> `Null))
    ();
  let req = J.make_request ~id:(J.Int 1) ~method_:"tools/list" () in
  let resp = Kirin_mcp.Server.handle_request server ~ctx req in
  check bool "response has result" true (Option.is_some resp.result)
;;
...
let server_tests =
  ...
  ; test_case
      "add tool with annotations/icon"
      `Quick
      test_server_add_tool_with_annotations
  ]
;;
...
let test_session_create () =
  let session = Kirin_mcp.Session.create ~server_name:"test" ~server_version:"1.0.0" () in
  check
    bool
...
let test_tasks_complete () =
  ...
  let result : P.tool_result =
    { content = [ P.Text "42" ]; is_error = None; _meta = None }
  in
...
let test_tasks_state_strings () =
  ...
  check
    ...
    "inputRequired rt"
    true
    (T.task_state_of_string "inputRequired" = Ok T.Input_required);
  check
    bool
    "input_required compat"
    true
    (T.task_state_of_string "input_required" = Ok T.Input_required);
...
let test_tasks_on_state_change () =
  ...
  let result : P.tool_result =
    { content = [ P.Text "done" ]; is_error = None; _meta = None }
  in
  ignore (T.complete_task reg ~id:task.id ~result);
...
let test_tasks_not_found () =
  ...
  let check_err label r =
    match r with
    | Error msg -> check bool label true (String.length msg > 0)
    | Ok () -> fail label
  in
...
let test_annotations_in_tool () =
  ...
  in
  let json = P.tool_to_json tool in
  check bool "has annotations" true (Yojson.Safe.Util.member "annotations" json <> `Null)
;;
...
let test_icon_in_tool () =
  ...
  in
  let json = P.tool_to_json tool in
  check bool "has icon field" true (Yojson.Safe.Util.member "icon" json <> `Null)
;;
...
let test_meta_extraction () =
  let params =
    Some
      (`Assoc
          [ "name", `String "test"; "_meta", `Assoc [ "progressToken", `String "tok-1" ] ])
  in
...
let test_capabilities_empty () =
  ...
  in
  let json = P.server_capabilities_to_json caps in
  check bool "tools is null" true (Yojson.Safe.Util.member "tools" json = `Null)
;;
...
let test_adapter_is_init_single () =
  ...
  in
  check bool "single initialize request" true (A.is_initialize_request json)
;;

let test_adapter_is_init_batch () =
  let init_req =
    `Assoc
      [ "jsonrpc", `String "2.0"; "id", `Int 1; "method", `String P.Method.initialize ]
  in
  let ping_req =
    `Assoc [ "jsonrpc", `String "2.0"; "id", `Int 2; "method", `String P.Method.ping ]
  in
  check
    bool
    "batch containing initialize"
    true
    (A.is_initialize_request (`List [ init_req; ping_req ]));
  check
    bool
    "batch without initialize"
    false
...
let test_adapter_is_init_non_init () =
  let json =
    `Assoc
      [ "jsonrpc", `String "2.0"; "id", `Int 1; "method", `String P.Method.tools_list ]
  in
  check bool "tools/list is not initialize" false (A.is_initialize_request json)
;;

let test_adapter_is_init_malformed () =
  check bool "null is not initialize" false (A.is_initialize_request `Null);
  check bool "string is not initialize" false (A.is_initialize_request (`String "hello"));
  check
    bool
    "object without method"
    false
    (A.is_initialize_request (`Assoc [ "id", `Int 1 ]));
  check bool "empty batch" false (A.is_initialize_request (`List []))
;;
...
let test_transport_session_id_streamable () =
  ...
  check
    (option string)
    "session_id after set"
    (Some "test-session-42")
    (Tr.session_id transport)
;;
...
let test_transport_queue_message_streamable () =
  ...
  match read_msg with
  | J.Notification n -> check string "method matches" "test/ping" n.method_
...
let test_governance_audit_eviction () =
  ...
  for i = 0 to 4 do
    let event =
      G.tool_call_event ~tool_name:(Printf.sprintf "tool_%d" i) ~outcome:"success" ()
    in
    G.record_event log event
  done;
  let events = G.recent_events log 10 in
...
let test_governance_audit_to_json () =
  ...
  let log = G.create_audit_log ~max_events:10 () in
  let event =
    G.tool_call_event ~tool_name:"json_tool" ~outcome:"error" ~session_id:"sess-1" ()
  in
  G.record_event log event;
  ...
  Eio.Fiber.all
    [ (fun () ->
        for i = 0 to 19 do
          let event =
            G.tool_call_event
              ~tool_name:(Printf.sprintf "fiber_a_%d" i)
              ~outcome:"success"
              ()
          in
          G.record_event log event
    ...
    ; (fun () ->
        for i = 0 to 19 do
          let event =
            G.tool_call_event
              ~tool_name:(Printf.sprintf "fiber_b_%d" i)
              ~outcome:"success"
              ()
          in
          G.record_event log event
...
let test_logging_set_level_missing_params () =
  ...
  let ctx = Kirin_mcp.Server.{ sw; clock } in
  let server = Kirin_mcp.Server.create () in
  let req = J.make_request ~id:(J.Int 1) ~method_:"logging/setLevel" () in
  let resp = Kirin_mcp.Server.handle_request server ~ctx req in
