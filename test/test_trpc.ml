...
let test_eio name f = test name (fun () -> Eio_main.run @@ fun _env -> f ())
...
let assert_true msg condition = if not condition then failwith msg
...
let test_procedure_types () =
  ...
  test "procedure_type_to_string subscription" (fun () ->
    assert_eq
      "subscription"
      "subscription"
      (Procedure.procedure_type_to_string Subscription))
;;
...
let test_type_info () =
  ...
  test "type_info_to_typescript optional" (fun () ->
    assert_eq
      "ts"
      "string | undefined"
      (Procedure.type_info_to_typescript (TOptional TString)))
;;
...
let test_batch () =
  test "parse single call" (fun () ->
    let json =
      `Assoc [ "id", `Int 1; "path", `String "test.hello"; "input", `String "world" ]
    in
  ...
  test "is_batch array" (fun () -> assert_true "is batch" (Batch.is_batch (`List [])));
  ...
  test "batch_response_to_json multiple" (fun () ->
    let responses =
      [ Batch.success ~id:1 (`String "a"); Batch.success ~id:2 (`String "b") ]
    in
    match Batch.batch_response_to_json responses with
...
let test_subscription () =
  ...
  test "encode Data message" (fun () ->
    let msg =
      Subscription.encode_message ~id:"sub_1" (Subscription.Data (`String "hello"))
    in
    match msg with
    | `Assoc fields ->
      assert_true "has id" (List.mem_assoc "id" fields);
  ...
  test "encode Error message" (fun () ->
    let msg =
      Subscription.encode_message
        ~id:"sub_1"
        (Subscription.Err { code = -1; message = "test error" })
    in
    match msg with
    | `Assoc fields -> assert_true "has error" (List.mem_assoc "error" fields)
    | _ -> failwith "expected object");
...
let test_codegen () =
  ...
  test "generate_schema" (fun () ->
    ...
    match schema with
    | `Assoc fields -> assert_true "has procedures" (List.mem_assoc "procedures" fields)
    | _ -> failwith "expected object")
;;
  ...
  test "nested router paths" (fun () ->
    let user_router =
      ...
      |> Router.query
           ...
           (fun _ctx id -> Ok (`Assoc [ "id", `String id ]))
    in
    let main_router = Router.create () |> Router.merge ~prefix:"user" user_router in
