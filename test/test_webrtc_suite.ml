...
let test_webrtc_peer_create () =
  ...
  check bool "state is new" true (WR.Peer.get_state peer = WR.Peer.New)
;;
...
let test_webrtc_peer_ice_servers () =
  let ice_servers =
    ...
    ; { WR.urls = [ "turn:turn.example.com:3478" ]
      ; username = Some "user"
...
let test_webrtc_ice_candidate () =
  ...
  let cand_str =
    Yojson.Safe.Util.member "candidate" cand_obj |> Yojson.Safe.Util.to_string
  in
...
let tests =
  ...
  ; test_case
      "signaling decode error"
      `Quick
      (with_eio test_webrtc_signaling_decode_error)
