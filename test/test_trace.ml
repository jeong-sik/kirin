...
let test_parse_traceparent_invalid_short_tid () =
  ...
  check (option reject) "invalid tid" None (Kirin.Trace.parse_traceparent header)
;;
...
let test_parse_traceparent_reject_uppercase () =
  let header = "00-0AF7651916CD43DD8448EB211C80319C-b7ad6b7169203331-01" in
  check (option reject) "uppercase trace_id" None (Kirin.Trace.parse_traceparent header)
;;

let test_parse_traceparent_reject_non_hex () =
  let header = "00-zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz-b7ad6b7169203331-01" in
  check (option reject) "non-hex chars" None (Kirin.Trace.parse_traceparent header)
;;

let test_parse_traceparent_reject_wrong_length () =
  let header = "00-0af765-b7ad6b7169203331-01" in
  check (option reject) "short trace_id" None (Kirin.Trace.parse_traceparent header)
;;
...
let test_to_traceparent () =
  let span =
    Kirin.Trace.start ~name:"op" ~trace_id:"0123456789abcdef0123456789abcdef" ()
  in
  ...
  check
    ...
    "contains trace_id"
    true
    (String.length tp >= 36 && String.sub tp 3 32 = "0123456789abcdef0123456789abcdef");
  check
    bool
    "ends with -01"
    true
    (String.length tp >= 2 && String.sub tp (String.length tp - 2) 2 = "01")
;;
...
let test_ids_are_hex () =
  ...
  String.iter (fun c -> check bool (Printf.sprintf "hex char '%c'" c) true (is_hex c)) id
;;
  ...
  run
    ...
    ; ( "w3c_trace_context"
      , [ test_case "parse valid traceparent" `Quick test_parse_traceparent_valid
        ; test_case
            "reject short trace_id"
            `Quick
            test_parse_traceparent_invalid_short_tid
        ...
        ; test_case
            "reject wrong length"
            `Quick
            test_parse_traceparent_reject_wrong_length
