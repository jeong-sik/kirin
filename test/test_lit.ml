...
let test_element_create () =
  let elem =
    Kirin_lit.Element.create ~tag_name:"my-element" ~class_name:"MyElement" "Hello World"
  in
...
let test_element_valid_tag_name () =
  ...
  check
    ...
    "valid multi-hyphen"
    true
    (Kirin_lit.Element.is_valid_tag_name "my-custom-element");
...
let test_element_to_javascript () =
  ...
  check
    ...
    true
    (contains js "customElements.define('my-counter'")
;;

let test_element_to_json () =
  let elem =
    Kirin_lit.Element.create ~tag_name:"test-elem" ~class_name:"TestElem" "<div></div>"
  in
  let json = Kirin_lit.Element.to_json elem in
  match json with
  | `Assoc pairs -> check bool "has tagName" true (List.mem_assoc "tagName" pairs)
...
let test_shadow_dom_create () =
  let shadow =
    Kirin_lit.Shadow_dom.create ~mode:Kirin_lit.Shadow_dom.Open "<slot></slot>"
  in
...
let test_shadow_dom_declarative () =
  let shadow =
    Kirin_lit.Shadow_dom.create ~mode:Kirin_lit.Shadow_dom.Open "<p>Content</p>"
  in
...
let test_shadow_dom_closed () =
  let shadow = Kirin_lit.Shadow_dom.create ~mode:Kirin_lit.Shadow_dom.Closed "Secret" in
  let html = Kirin_lit.Shadow_dom.to_declarative_shadow_dom shadow in
...
let test_shadow_dom_styles () =
  let shadow =
    Kirin_lit.Shadow_dom.create ~styles:[ ":host { display: block; }" ] "<slot></slot>"
  in
  let html = Kirin_lit.Shadow_dom.to_declarative_shadow_dom shadow in
...
let test_shadow_dom_slots () =
  let slot =
    Kirin_lit.Shadow_dom.render_slot (Kirin_lit.Shadow_dom.named_slot "content" ())
  in
...
let test_shadow_dom_render_component () =
  ...
  let html =
    Kirin_lit.Shadow_dom.render_component
      ~tag_name:"my-comp"
      ~attributes:[ "id", "test" ]
      shadow
  in
...
let test_ssr_render_with_props () =
  ...
  | Ok html -> check bool "has prop" true (contains html "name=\"Test\"")
  | Error msg -> fail ("Render failed: " ^ msg)
;;
...
let test_ssr_polyfill () =
  ...
  match Kirin_lit.Ssr.render engine ~tag_name:"my-element" () with
  | Ok html -> check bool "has polyfill" true (contains html "attachShadow")
  | Error _ -> fail "Should have rendered"
;;
...
let test_ssr_no_polyfill () =
  ...
  let engine = Kirin_lit.Ssr.create config in
  match Kirin_lit.Ssr.render engine ~tag_name:"my-element" () with
  | Ok html -> check bool "no polyfill" false (contains html "attachShadow")
  | Error _ -> fail "Should have rendered"
;;
...
let test_ssr_fallback () =
  let engine = Kirin_lit.Ssr.create Kirin_lit.Ssr.default_config in
  let html =
    Kirin_lit.Ssr.render_with_fallback
      engine
      ~tag_name:"my-element"
...
let test_ssr_page () =
  let engine = Kirin_lit.Ssr.create Kirin_lit.Ssr.default_config in
  let components =
    [ "my-header", `Null, []; "my-content", `Null, []; "my-footer", `Null, [] ]
  in
...
let test_facade_element () =
  let elem = Kirin_lit.element ~tag_name:"test-el" ~class_name:"TestEl" "<div></div>" in
