...
let test_request_tests =
  ...
  ; ( "with headers"
    ...
    , fun () ->
        let req =
          Testing.Test_request.get "/api"
          |> Testing.Test_request.with_header "Authorization" "Bearer token"
          |> Testing.Test_request.with_header "Accept" "application/json"
        in
  ...
  ; ( "with query params"
    ...
    , fun () ->
        let req =
          Testing.Test_request.get "/api/users"
          |> Testing.Test_request.with_query "page" "1"
          |> Testing.Test_request.with_query "limit" "10"
        in
  ...
  ; ( "with json body"
    ...
    , fun () ->
        let req =
          Testing.Test_request.post_json "/api/users" (`Assoc [ "name", `String "Alice" ])
        in
        Alcotest.(check (option string))
          "content-type"
  ...
  ; ( "with bearer token"
    ...
    , fun () ->
        let req =
          Testing.Test_request.get "/api"
          |> Testing.Test_request.with_bearer_token "mytoken"
        in
        Alcotest.(check (option string))
          "auth header"
          (Some "Bearer mytoken")
...
let test_response_tests =
  ...
  ; ( "is client error"
    ...
    , fun () ->
        let resp =
          { Testing.Test_response.status = `Not_found; headers = []; body = "" }
        in
        Alcotest.(check bool)
          "is client error"
          true
          (Testing.Test_response.is_client_error resp) )
  ; ( "is server error"
    , `Quick
    , fun () ->
        let resp =
          { Testing.Test_response.status = `Internal_server_error
          ; headers = []
          ; body = ""
          }
        in
        Alcotest.(check bool)
          "is server error"
          true
          (Testing.Test_response.is_server_error resp) )
  ; ( "get header"
    , `Quick
    , fun () ->
        let resp =
          { Testing.Test_response.status = `OK
...
let json_path_tests =
  ...
  ; ( "nested key"
    , `Quick
    , fun () ->
        let json = `Assoc [ "user", `Assoc [ "name", `String "Bob" ] ] in
  ...
  ; ( "array index"
    , `Quick
    , fun () ->
        let json = `Assoc [ "users", `List [ `String "A"; `String "B"; `String "C" ] ] in
    ...
    , fun () ->
        let json =
          `Assoc
            [ ( "data"
              , `Assoc
                  [ "items", `List [ `Assoc [ "id", `Int 1 ]; `Assoc [ "id", `Int 2 ] ] ]
              )
            ]
        in
...
let assert_tests =
  ...
  ; ( "assert body contains"
    ...
    , fun () ->
        let resp =
          { Testing.Test_response.status = `OK; headers = []; body = "Hello, World!" }
        in
        Testing.Assert.body_contains "World" resp )
  ; ( "assert body equals"
    , `Quick
    , fun () ->
        let resp = { Testing.Test_response.status = `OK; headers = []; body = "exact" } in
...
let mock_tests =
  [ ( "mock endpoint"
    ...
    , fun () ->
        ...
        Alcotest.(check int) "status" 200 (Testing.Test_response.status_code resp) )
  ; ( "mock no match returns 404"
    , `Quick
    , fun () ->
        let mock =
          Testing.Mock.create ()
          |> Testing.Mock.on ~meth:(Some `GET) ~path:"/api/users" ~status:`OK ~body:"OK"
        in
  ...
  ; ( "mock call count"
    ...
    , fun () ->
        let mock =
          Testing.Mock.create ()
          |> Testing.Mock.on ~meth:(Some `GET) ~path:"/api/test" ~status:`OK ~body:"OK"
        in
        ...
        Alcotest.(check int)
          "call count"
          2
          (Testing.Mock.call_count ~path:"/api/test" mock) )
  ; ( "mock reset"
    , `Quick
    , fun () ->
        let mock =
...
let utility_tests =
  ...
  ; ( "random email"
    ...
    , fun () ->
        ...
        Alcotest.(check bool) "contains @" true (String.contains email '@') )
  ; ( "random int in range"
    , `Quick
    , fun () ->
