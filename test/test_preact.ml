...
let test_effect_creation () =
  let e =
    Kirin_preact.Signals.make_effect ~cleanup:true ~name:"logger" [ "count"; "name" ]
  in
...
let test_import_generation () =
  let import =
    Kirin_preact.Signals.generate_import
...
let test_vite_aliases () =
  ...
  check
    ...
    "has react-dom"
    true
    (string_contains ~sub:"'react-dom': 'preact/compat'" aliases)
;;
...
let test_hook_import () =
  let import =
    Kirin_preact.Compat.hook_import
      [ Kirin_preact.Compat.UseState; Kirin_preact.Compat.UseEffect ]
  in
...
let test_manifest_resolve () =
  let json =
    `Assoc
      [ "index.html", `Assoc [ "file", `String "assets/index.js"; "isEntry", `Bool true ]
      ]
  in
  let m = Kirin_preact.Manifest.parse json in
...
let test_manifest_preload () =
  let json =
    `Assoc
      [ "main.tsx", `Assoc [ "file", `String "assets/main.js"; "isEntry", `Bool true ] ]
  in
  let m = Kirin_preact.Manifest.parse json in
...
let test_manifest_script_tag () =
  let json = `Assoc [ "entry.tsx", `Assoc [ "file", `String "assets/entry.js" ] ] in
  let m = Kirin_preact.Manifest.parse json in
...
let test_hydrate_render () =
  let options =
    { Kirin_preact.Hydrate.default_options with title = "Test App"; root_id = "root" }
  in
...
let test_hydrate_initial_data () =
  let options =
    { Kirin_preact.Hydrate.default_options with
      title = "App"
...
let test_hydrate_mode () =
  check
    ...
    "native mode"
    "native"
    (Kirin_preact.Hydrate.mode_to_string Kirin_preact.Hydrate.Native);
  check
    string
    "compat mode"
    "compat"
    (Kirin_preact.Hydrate.mode_to_string Kirin_preact.Hydrate.Compat)
;;
...
let test_protocol_decode_success () =
  ...
  | Ok (Kirin_preact.Protocol.RpcSuccess s) -> check int "id" 1 s.success_id
...
let test_protocol_decode_error () =
  ...
  match Kirin_preact.Protocol.decode_response json with
  | Ok (Kirin_preact.Protocol.RpcError e) -> check int "code" (-32600) e.error.code
...
let test_ssr_render_with_fallback () =
  let engine = Kirin_preact.Ssr.create Kirin_preact.Ssr.default_config in
  let html =
    Kirin_preact.Ssr.render_with_fallback
      engine
      ~url:"/test"
