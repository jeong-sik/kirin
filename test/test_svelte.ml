...
let test_eio name f =
  Alcotest.test_case name `Quick (fun () -> Eio_main.run @@ fun _env -> f ())
;;
...
let route_def_tests =
  ...
  ; test "add param" (fun () ->
      let route =
        Route_def.page "/users/[id]" |> Route_def.with_param "id" Route_def.Int
      in
      Alcotest.(check int) "param count" 1 (List.length route.Route_def.params))
  ; test "add optional param" (fun () ->
      let route =
        Route_def.page "/posts/[[slug]]"
        |> Route_def.with_optional_param "slug" Route_def.String
      in
  ...
  ; test "add rest param" (fun () ->
      let route = Route_def.page "/files/[...path]" |> Route_def.with_rest_param "path" in
      let param = List.hd route.Route_def.params in
      Alcotest.(check bool)
        "is rest"
        true
  ...
  ; test "route group" (fun () ->
      let route = Route_def.page "/(auth)/login" |> Route_def.in_group "auth" in
...
let file_router_tests =
  ...
  ; test "parse optional segment" (fun () ->
      ...
      Alcotest.(check bool)
        ...
        true
        (match seg.File_router.segment_type with
         | File_router.OptionalParam _ -> true
         | _ -> false))
  ...
  ; test "sveltekit file types" (fun () ->
      ...
      Alcotest.(check bool)
        ...
        true
        (match server with
         | Some File_router.PageServer -> true
         | _ -> false))
...
let loader_tests =
  ...
  ; test "create loader result error" (fun () ->
      ...
      Alcotest.(check bool)
        ...
        true
        (match result with
         | Loader.LoadError _ -> true
         | _ -> false))
  ; test "loader context" (fun () ->
      let ctx =
        Loader.create_context ~url:"/test" ~params:[ "id", "1" ] ~route_id:"test" ()
      in
      Alcotest.(check string) "url" "/test" ctx.Loader.url)
  ; test "get param" (fun () ->
      let ctx =
        Loader.create_context ~url:"/test" ~params:[ "id", "123" ] ~route_id:"test" ()
      in
...
let action_tests =
  ...
  ; test "get all values" (fun () ->
      let form = [ "tags", Action.Multiple [ Action.Text "a"; Action.Text "b" ] ] in
...
let preload_tests =
  ...
  ; test "preload css hint" (fun () ->
      ...
      Alcotest.(check bool) "contains as style" true (String.length tag > 0))
  ; test "prefetch hint" (fun () ->
      let tag = Preload.prefetch_hint "/page" in
      Alcotest.(check bool) "contains prefetch" true (String.length tag > 0))
...
let meta_tests =
  ...
  ; test "render description tag" (fun () ->
      ...
      Alcotest.(check bool) "has content" true (String.length tag > 0))
  ; test "builder pattern" (fun () ->
      let html =
        Meta.create () |> Meta.title "Test" |> Meta.description "A test" |> Meta.build
      in
      Alcotest.(check bool) "has content" true (String.length html > 0))
  ; test "page helper" (fun () ->
      ...
      Alcotest.(check bool) "has content" true (String.length html > 0))
  ; test "og tags" (fun () ->
      let b = Meta.create () |> Meta.og ~title:"Test" () in
      let html = Meta.build b in
  ...
  ; test "canonical tag" (fun () ->
      ...
      Alcotest.(check bool) "has href" true (String.sub tag 0 5 = "<link"))
...
let data_tests =
  ...
  ; test "serialize escapes script" (fun () ->
      ...
      let str = Data.serialize json in
      Alcotest.(check bool) "escaped" true (not (String.sub str 0 2 = "</")))
  ; test "script tag" (fun () ->
      let json = `Assoc [ "user", `Int 1 ] in
      let script = Data.script_tag json in
      Alcotest.(check bool) "has script" true (String.sub script 0 7 = "<script"))
  ; test "route data" (fun () ->
...
let hydrate_tests =
  [ test "spa shell" (fun () ->
      ...
      Alcotest.(check bool) "has doctype" true (String.sub html 0 9 = "<!DOCTYPE"))
  ; test "with ssr" (fun () ->
      let html =
        Hydrate.with_ssr
          ~title:"Test"
          ...
          ()
      in
      Alcotest.(check bool) "has ssr" true (String.length html > 0))
  ; test "render options" (fun () ->
      let opts =
        Hydrate.{ default_options with title = "Test"; scripts = [ "/app.js" ] }
      in
      let html = Hydrate.render opts in
      Alcotest.(check bool) "has content" true (String.length html > 0))
  ; test "sveltekit shell" (fun () ->
      let html =
        Hydrate.sveltekit_shell
          ~title:"Test"
          ~entry_script:"/app.js"
          ~css_files:[ "/app.css" ]
          ()
  ...
  ; test "streaming placeholder" (fun () ->
      ...
      Alcotest.(check bool) "has template" true (String.sub html 0 9 = "<template"))
  ]
;;
...
let protocol_tests =
  [ test "encode render request" (fun () ->
      ...
      Alcotest.(check bool) "has jsonrpc" true (String.length json > 0))
  ; test "encode request" (fun () ->
      let _id, json = Protocol.encode_request "test" (`Assoc []) in
      Alcotest.(check bool) "has method" true (String.length json > 0))
  ; test "encode notification" (fun () ->
      let json = Protocol.encode_notification "ping" (`Assoc []) in
      Alcotest.(check bool) "no id" true (not (String.sub json 0 6 = "{\"id\":")))
  ; test "decode response" (fun () ->
      let json =
        Yojson.Safe.from_string {|{"jsonrpc":"2.0","id":1,"result":{"ok":true}}|}
      in
...
let worker_tests =
  [ test "create worker" (fun () ->
      let worker =
        Worker.create ~bundle:"test.js" ~max_requests:1000 ~memory_limit_mb:200 ()
      in
      (* Freshly created worker starts in Ready state, which is healthy *)
      Alcotest.(check bool) "is healthy" true (Worker.is_healthy worker))
  ...
  ; test "needs restart" (fun () ->
      let worker =
        Worker.create ~bundle:"test.js" ~max_requests:10 ~memory_limit_mb:200 ()
      in
      Alcotest.(check bool) "no restart needed" false (Worker.needs_restart worker))
  ]
;;
...
let streaming_tests =
  ...
  ; test "shell generation" (fun () ->
      ...
      Alcotest.(check bool) "has doctype" true (String.length html > 10))
  ...
  ; test "hydration boundary" (fun () ->
      let html =
        Streaming.hydration_boundary
          ~id:"comp1"
          ~priority:Streaming.Idle
          ~fallback:None
          "<div/>"
      in
  ...
  ; test "placeholder" (fun () ->
      ...
      Alcotest.(check bool) "template" true (String.sub html 0 9 = "<template"))
  ; test "sse event" (fun () ->
      let event = Streaming.sse_event ~event:"test" ~data:"hello" in
      Alcotest.(check bool) "has event" true (String.sub event 0 6 = "event:"))
...
let codegen_tests =
  ...
  ; test "generate hooks server" (fun () ->
      ...
      Alcotest.(check bool) "has Handle" true (String.length ts > 0))
  ; test "generate hooks client" (fun () ->
      let ts = Codegen.generate_hooks_client in
      Alcotest.(check bool) "has HandleClientError" true (String.length ts > 0))
  ]
;;
