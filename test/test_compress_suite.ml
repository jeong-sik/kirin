...
let test_compress_skip_for_images () =
  ...
  check
    ...
    "compress json"
    false
    (Kirin.Compress.skip_compression_for "application/json")
;;
...
let test_compress_middleware () =
  ...
  let req = make_test_request ~headers:[ "accept-encoding", "gzip, deflate" ] "/" in
  let resp = with_compress req in
  check
    (option string)
    "content-encoding"
    (Some "gzip")
    (Kirin.Response.header "content-encoding" resp);
  check
    bool
    "response compressed"
    true
    (String.length (response_body_to_string (Kirin.Response.body resp)) < 2000)
;;
