...
let test_loader_pure () =
  let ctx : Kirin_remix.Loader.loader_context =
    { request_url = "/"; request_method = "GET"; params = []; headers = []; cookies = [] }
  in
  let loader = Kirin_remix.Loader.pure (`String "hello") in
  match loader ctx with
  | Kirin_remix.Loader.Data json -> check json_string "data" (`String "hello") json
...
let test_loader_error () =
  ...
  match loader ctx with
  | Kirin_remix.Loader.ServerError msg -> check string "error" "Something went wrong" msg
...
let test_loader_to_json () =
  let result =
    Kirin_remix.Loader.to_json_result (Kirin_remix.Loader.Data (`String "test"))
  in
...
let test_action_required_field () =
  ...
  check
    ...
    "present"
    (Some "John")
    (Kirin_remix.Action.required_field ctx "name");
...
let test_route_segments () =
  ...
  | [ Kirin_remix.Route.Static "posts"
    ; Kirin_remix.Route.Dynamic "slug"
    ; Kirin_remix.Route.Splat
    ] -> ()
...
let test_route_with_loader () =
  ...
  let loader (ctx : Kirin_remix.Loader.loader_context) =
    Kirin_remix.Loader.Data (`String ctx.request_url)
  in
...
let test_route_match_dynamic () =
  ...
  | Some m -> check (option string) "id param" (Some "123") (List.assoc_opt "id" m.params)
  | None -> fail "expected match"
;;
...
let test_ssr_create_context () =
  let ctx =
    Kirin_remix.Ssr.create_context
      ~url:"/users"
      ~matches:[]
...
let test_ssr_hydration_script () =
  let script =
    Kirin_remix.Ssr.hydration_script
...
let test_ssr_render_document () =
  let config = Kirin_remix.Ssr.default_config in
  let context = Kirin_remix.Ssr.create_context ~url:"/" ~matches:[] ~loader_data:[] () in
...
let test_ssr_error_boundary () =
  let html =
    Kirin_remix.Ssr.render_error_boundary ~error:"Something failed" ~route:"/users"
  in
...
let test_ssr_catch_boundary () =
  let html =
    Kirin_remix.Ssr.render_catch_boundary ~status:404 ~message:"Page not found"
  in
...
let test_ssr_context_json () =
  let context =
    Kirin_remix.Ssr.create_context ~url:"/test" ~matches:[] ~loader_data:[] ()
  in
...
let test_facade_pure () =
  ...
  match loader ctx with
  | Kirin_remix.Loader.Data json -> check json_string "data" (`String "facade") json
  | _ -> fail "expected Data"
;;
...
let test_facade_redirect () =
  ...
  | Kirin_remix.Loader.Redirect (url, _) -> check string "url" "/home" url
  | _ -> fail "expected Redirect"
;;
