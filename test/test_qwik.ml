...
let test_loader_execute () =
  ...
  let ctx =
    context_of_request ~url:"/test" ~params:[] ~query:[] ~headers:[] ~cookies:[] ()
  in
...
let test_action_execute () =
  ...
  let ctx =
    context_of_request ~url:"/test" ~params:[] ~headers:[] ~cookies:[] ~form:empty_form ()
  in
...
let test_meta_render () =
  let open Kirin_qwik.Meta in
  let head =
    empty |> with_title "Test" |> with_meta ~name:"description" ~content:"Desc"
  in
...
let test_codegen_routes_file () =
  let open Kirin_qwik in
  let routes = [ Route_def.ssr "/" |> Route_def.with_name "home" ] in
...
let test_codegen_types_file () =
  let open Kirin_qwik in
  let routes = [ Route_def.ssr "/users" |> Route_def.with_name "users" ] in
