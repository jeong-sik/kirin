  ...
  , fun () ->
      ...
      Fun.protect
        ~finally:(fun () ->
          ...
          (try Sys.remove file_path with
           | _ -> ());
          (try Unix.rmdir (Filename.concat root "a/b") with
           | _ -> ());
          (try Unix.rmdir (Filename.concat root "a") with
           | _ -> ());
...
let () = Alcotest.run "Fs_compat" [ "save", [ test_save_creates_parent_dirs ] ]
