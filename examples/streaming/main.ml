...
let csv_handler _req =
  Stream.response
    ~headers:
      (Http.Header.init ()
       |> fun h ->
       Http.Header.add h "content-disposition" "attachment; filename=\"data.csv\"")
    (fun yield ->
       (* Header *)
...
let upload_handler req =
  ...
  if body_size = 0
  then text "No file uploaded. Use POST with file content."
  else (
    (* For demo, just count bytes without saving *)
    let chunks_count = ref 0 in
    Stream.read_chunks ~request:req ~chunk_size:8192 (fun _chunk -> incr chunks_count);
    json
      (`Assoc
...
let json_stream_handler _req =
  Stream.response (fun yield ->
    ...
    for i = 1 to 1000 do
      let item =
        Printf.sprintf
          {|  {"id": %d, "name": "Item %d", "value": %.2f}|}
          i
          i
          (Random.float 100.0)
      in
      if i < 1000 then yield (item ^ ",\n") else yield (item ^ "\n")
    done;
    yield "]\n")
...
<body>
  ...
  <h2>Features</h2>
  <ul>
    <li>✅ <strong>Memory Efficient</strong> - Files never fully loaded into memory</li>
    <li>✅ <strong>Chunked Transfer</strong> - Data streams as it's generated</li>
    <li>✅ <strong>Progress Tracking</strong> - Callbacks for upload/download progress</li>
    <li>✅ <strong>Backpressure</strong> - Eio flow-based automatic rate control</li>
  </ul>
...
let () =
  ...
  in
  Printf.printf "🦒 Kirin Streaming Demo\n";
  Printf.printf "========================\n";
