...
let encode_hello_reply name = Printf.sprintf "Hello, %s!" name
...
let greeter_service =
  ...
  |> Kirin.Grpc.server_streaming "SayHelloStream" (fun request ->
    ...
    for i = 1 to 3 do
      Kirin.Grpc.stream_add stream (Printf.sprintf "Hello %d, %s!" i name)
    done;
...
let _http_routes =
  Kirin.router
    [ Kirin.get "/" (fun _ -> Kirin.html "<h1>Kirin HTTP + gRPC Server</h1>")
...
let () =
  ...
  Printf.printf
    "Test gRPC: grpcurl -plaintext localhost:50051 helloworld.Greeter/SayHello\n";
  Printf.printf "\n";
