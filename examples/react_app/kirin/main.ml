...
let user_to_json u =
  `Assoc [ "id", `Int u.id; "name", `String u.name; "email", `String u.email ]
;;

(** API Routes *)
let api_users _req = Kirin.json (`List (List.map user_to_json users))
...
let level2_handler manifest req =
  ...
  let initial_data =
    Some (`Assoc [ "currentUser", user_to_json user; "path", `String path ])
  in
...
let () =
  ...
  let handler =
    ...
    | "static" | "level1" ->
      ...
      Kirin.router
        ([ Kirin.get "/api/users" api_users; Kirin.get "/api/users/:id" api_user ]
         @ level1_routes ())
    | "hydration" | "level2" ->
      (* Level 2: Hydration with SEO *)
      let manifest =
        match Kirin_react.Manifest.load "dist/.vite/manifest.json" with
        | Ok m -> m
    ...
    | _ -> failwith "Unknown mode. Use: static, hydration, or ssr"
  in
  Printf.printf "Server running at http://localhost:3000\n%!";
  Kirin.start ~port:3000 @@ Kirin.logger @@ handler
;;
