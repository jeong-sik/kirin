...
module Store = struct
  ...
  let posts : (string, string * string * string * string * int) Hashtbl.t =
    Hashtbl.create 64
  ;;
  ...
  let find_agent_by_key api_key = Hashtbl.find_opt agents api_key
  let name_exists name = Hashtbl.mem agent_names name
  ...
  let get_posts ?(limit = 25) () =
    Hashtbl.fold
      (fun id (agent_id, title, content, created_at, score) acc ->
  ...
  let find_post id = Hashtbl.find_opt posts id
...
let with_auth handler req =
  ...
  | Some api_key ->
    (match Store.find_agent_by_key api_key with
     ...
     | Some (agent_id, agent_name, _) -> handler ~agent_id ~agent_name req)
;;
...
let register_agent req =
  ...
  | Ok body ->
    ...
    let description =
      body |> member "description" |> to_string_option |> Option.value ~default:""
    in
...
let get_me ~agent_id ~agent_name _req =
  Kirin.json
    (`Assoc
        [ "success", `Bool true
        ; "agent", `Assoc [ "id", `String agent_id; "name", `String agent_name ]
        ])
;;
...
let create_post ~agent_id ~agent_name:_ req =
  ...
  | Ok body ->
    ...
    let content =
      body |> member "content" |> to_string_option |> Option.value ~default:""
    in
    let id = generate_uuid () in
    ...
    Kirin.json
      (`Assoc
          [ "success", `Bool true
          ; "post", `Assoc [ "id", `String id; "title", `String title ]
          ])
;;

let get_posts req =
  let limit =
    Kirin.query_opt "limit" req
  ...
  let posts_json =
    List.map
      (fun (id, _agent_id, title, content, created_at, score) ->
         `Assoc
           [ "id", `String id
           ; "title", `String title
...
let upvote_post ~agent_id:_ ~agent_name:_ req =
  let id = Kirin.param "id" req in
  if Store.vote_post id 1
  then Kirin.json_string {|{"success":true,"message":"Upvoted! 🦞"}|}
  else Kirin.json_string ~status:`Not_found {|{"success":false,"error":"Post not found"}|}
;;

let downvote_post ~agent_id:_ ~agent_name:_ req =
  let id = Kirin.param "id" req in
  if Store.vote_post id (-1)
  then Kirin.json_string {|{"success":true,"message":"Downvoted"}|}
  else Kirin.json_string ~status:`Not_found {|{"success":false,"error":"Post not found"}|}
;;
...
let routes =
  Kirin.router
    [ Kirin.get "/" (fun _ ->
        Kirin.json (`Assoc [ "name", `String "Agent Hub"; "version", `String "0.1.0" ]))
...
let () =
  ...
  Kirin.start ~port:8989 @@ Kirin.logger @@ Kirin.cors () @@ routes
;;
