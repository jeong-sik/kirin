...
let cpu_heavy_handler _req =
  ...
  Kirin.Logger.info
    ~ctx:[ "result", `Int result; "duration_sec", `Float duration ]
    "CPU heavy task finished on Domain %d"
    (Domain.self () :> int);
  Kirin.json
    (`Assoc
        [ "result", `Int result
...
let routes =
  Kirin.router
    [ Kirin.get "/" (fun _ ->
        Kirin.html "<h1>Kirin Multicore Scaling</h1><p>Check logs for JSON output</p>")
    ; Kirin.get "/heavy" cpu_heavy_handler
    ]
;;

let app = Kirin.Logger.middleware @@ routes

let () =
  (* Configure JSON logging *)
  Kirin.Logger.configure ~format:`Json ~min_level:Kirin.Logger.Info ();
  Printf.printf
    "Starting Kirin Multicore Server (Cores detected: %d)\n%!"
