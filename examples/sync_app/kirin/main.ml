...
module MemDb = struct
  ...
  let process_push client_id mutations =
    Printf.printf
      "Received push from %s: %d mutations\n"
      client_id
  ...
  let process_pull client_id _from_version =
    ...
    let patches =
      Hashtbl.fold (fun k v acc -> { op = `Put; key = k; value = Some v } :: acc) data []
    in
    let last_m =
      Hashtbl.find_opt client_last_mutation client_id |> Option.value ~default:0
    in
...
let () = Kirin.start ~port:9000 @@ Kirin.logger @@ Kirin.cors () @@ routes
