...
module Bench = struct
  ...
  let run ~name ~warmup_iterations ~sample_count ~alloc_iterations f =
    ...
    let throughput_ops =
      ...
      then float sample_count /. (total_time_ns /. 1_000_000_000.0)
      else 0.0
    in
    (* Calculate latencies *)
    ...
    { name
    ; _iterations = sample_count
    ; _total_time_ns = total_time_ns
    ; throughput_ops
    ; latency_p50_ns
    ; latency_p95_ns
    ; latency_p99_ns
  ...
  let print_results results =
    ...
    List.iter
      (fun r ->
         Printf.printf
           "| %-30s | %12s | %10.0f | %10.0f | %10.0f | %10.1f | %10.1f |\n"
           r.name
...
let make_request ?(meth = `GET) ?(path = "/") ?(headers = []) ?(body = "") () =
  let raw_headers = Http.Header.of_list (("host", "localhost") :: headers) in
  let resource = path in
  let raw = Http.Request.make ~meth ~headers:raw_headers resource in
  let body_source =
    Eio.Flow.string_source body |> Eio.Buf_read.of_flow ~max_size:max_int
  in
  Kirin.Request.make ~raw ~body_source
;;

(** Sample JSON data for serialization benchmarks *)
let small_json = `Assoc [ "message", `String "Hello, World!"; "status", `Int 200 ]
  ...
  `Assoc
    [ ( "users"
      , `List
          ...
          ; `Assoc
              [ "id", `Int 2; "name", `String "Bob"; "email", `String "bob@example.com" ]
          ; `Assoc
              [ "id", `Int 3
              ; "name", `String "Charlie"
          ...
          ; `Assoc
              [ "id", `Int 5; "name", `String "Eve"; "email", `String "eve@example.com" ]
          ] )
    ; ( "metadata"
      , `Assoc
          [ "page", `Int 1
...
let large_json =
  ...
  `Assoc
    [ "users", `List users
    ; "metadata", `Assoc [ "page", `Int 1; "per_page", `Int 100; "total", `Int 10000 ]
    ]
;;
...
let many_routes =
  List.init 100 (fun i -> Kirin.get (Printf.sprintf "/route%d" i) handler_hello_text)
...
let silent_logger : Kirin.middleware =
  fun handler req ->
...
let combined_middleware =
  Kirin.pipeline [ silent_logger; timing_middleware; cors_middleware ]
;;
...
let mock_static_middleware : Kirin.middleware =
  ...
  fun handler req ->
    ...
    if String.length path >= 7 && String.sub path 0 7 = "/static"
    then
...
let () =
  ...
  let req_with_encoding =
    make_request ~path:"/" ~headers:[ "accept-encoding", "gzip, deflate" ] ()
  in
  (* ===== Handler Benchmarks ===== *)
  ...
  let router_results =
    ...
    ; run "router/100routes_last" (fun () ->
        router_many (make_request ~path:"/route99" ()))
  ...
  let response_results =
    ...
    ; run "response/with_header" (fun () ->
        Kirin.Response.with_header "X-Custom" "value" base_resp)
    ; run "response/with_3_headers" (fun () ->
        Kirin.Response.with_headers [ "X-A", "1"; "X-B", "2"; "X-C", "3" ] base_resp)
  ...
  Kirin_react.Worker.Render_cache.set
    render_cache
    ~url:"/cached"
    ~props:(`Assoc [])
  ...
  let react_results =
    [ (* Manifest operations *)
      run "react/manifest_parse" (fun () -> Kirin_react.Manifest.parse manifest_json)
    ...
    ; (* Meta tag generation *)
      run "react/meta_title" (fun () -> ignore (Kirin_react.Meta.title "Test Page"))
    ; run "react/meta_og_tags" (fun () ->
        ignore
          (Kirin_react.Meta.og ~property:"title" ~content:"Test"
    ...
    ; (* Render cache operations *)
      run "react/cache_get_miss" (fun () ->
        ignore
          (Kirin_react.Worker.Render_cache.get
             render_cache
             ~url:"/miss"
             ~props:(`Assoc [])))
